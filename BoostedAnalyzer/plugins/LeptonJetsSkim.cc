...
bool LeptonJetsSkim::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  selectedElectrons.erase(
      std::remove_if(
          selectedElectrons.begin(), selectedElectrons.end(),
          [&](pat::Electron ele) {
  ...
  selectedMuons.erase(
      std::remove_if(
          selectedMuons.begin(), selectedMuons.end(),
          [&](pat::Muon mu) {
            return !(mu.pt() >= muonPtMin_ && fabs(mu.eta()) <= muonEtaMax_ &&
                     muon::isLooseMuon(mu) && mu.passed(pat::Muon::PFIsoLoose));
          }),
      selectedMuons.end());
  ...
  if (n_ak4jets < minJetsAK4_ && n_ak8jets < minJetsAK8_ &&
      n_ak15jets < minJetsAK15_)
    return false;
  if (n_ak4jets > maxJetsAK4_ || n_ak8jets > maxJetsAK8_ ||
      n_ak15jets > maxJetsAK15_)
    return false;
  // met selection in general
  if (hMETs->at(0).pt() < metPtMin_ && hPuppiMETs->at(0).pt() < metPtMin_ &&
      hadr_recoil.pt() < metPtMin_ && hadr_recoil_puppi.pt() < metPtMin_)
    return false;
...
// module  ------------
void LeptonJetsSkim::fillDescriptions(
