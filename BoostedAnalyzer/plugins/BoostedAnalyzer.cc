...
class BoostedAnalyzer : public edm::EDAnalyzer {
 ...
 private:
  ...
  std::map<string, float> GetWeights(
      const GenEventInfoProduct& genEventInfo, const LHEEventProduct& lheInfo,
  ...
  std::map<string, float> GetCSVWeights(
      const std::vector<pat::Jet>& selectedJets,
      const Systematics::Type& systype = Systematics::NA);

  static std::string outfileName(const std::string& basename,
...
BoostedAnalyzer::BoostedAnalyzer(const edm::ParameterSet& iConfig)
    :  // initialize some helpers with consumes collector (allows to access data
      ...
      dumpAlwaysEvents{
          iConfig.getParameter<std::vector<int>>("dumpAlwaysEvents")},
      ...
      selectedPhotonsToken{consumes<pat::PhotonCollection>(
          iConfig.getParameter<edm::InputTag>("selectedPhotons"))},

      genInfoToken{consumes<GenEventInfoProduct>(
          iConfig.getParameter<edm::InputTag>("genInfo"))},
      lheInfoToken{consumes<LHEEventProduct>(
          iConfig.getParameter<edm::InputTag>("lheInfo"))},
      lheInfoToken_source{consumes<LHEEventProduct>(
          iConfig.getParameter<edm::InputTag>("lheInfo_source"))},
...

{
  ...
  for (const auto& tag :
       iConfig.getParameter<std::vector<edm::InputTag>>("selectedJetsAK8")) {
    selectedJetsAK8_Tokens.push_back(consumes<std::vector<pat::Jet>>(tag));
  }
  for (const auto& tag :
       iConfig.getParameter<std::vector<edm::InputTag>>("selectedJetsAK15")) {
  ...
  for (auto& treewriter : treewriters) {
    if (std::find(processorNames.begin(), processorNames.end(),
    ...
    if (std::find(processorNames.begin(), processorNames.end(),
                  "essentialRecoVarProcessor") != processorNames.end()) {
      treewriter->AddTreeProcessor(new essentialRecoVarProcessor(),
                                   "essentialRecoVarProcessor");
    }
    if (std::find(processorNames.begin(), processorNames.end(),
                  "PhotonVarProcessor") != processorNames.end()) {
      treewriter->AddTreeProcessor(new PhotonVarProcessor(),
                                   "PhotonVarProcessor");
    }
    ...
    if (std::find(processorNames.begin(), processorNames.end(),
      ...
      treewriter->AddTreeProcessor(new essentialMVAVarProcessor(),
                                   "essentialMVAVarProcessor");
    }
    if (std::find(processorNames.begin(), processorNames.end(),
    ...
    if (std::find(processorNames.begin(), processorNames.end(),
                  "AK8JetProcessor") != processorNames.end()) {
      treewriter->AddTreeProcessor(new AK8JetProcessor(&helper),
                                   "AK8JetProcessor");
    }
    if (std::find(processorNames.begin(), processorNames.end(),
                  "SelectionTagProcessor") != processorNames.end()) {
      treewriter->AddTreeProcessor(new SelectionTagProcessor(),
...
void BoostedAnalyzer::analyze(const edm::Event& iEvent,
  ...
  helper.GetCorrectedBoostedJets(idBoostedJets, iEvent, iSetup, h_genJets, syst,
  true, true);
  ...
  boosted::Ak4ClusterCollection ak4Cluster = Ak4Cluster::GetAk4Cluster(ak4Jets,
  0); boosted::Ak4ClusterCollection selectedAk4Cluster =
  Ak4Cluster::GetSelectedAk4Cluster(ak4Cluster, 200., "A");
  */
  // Fill Event Info Object
  EventInfo eventInfo(iEvent, h_beamSpot, h_hcalNoiseSummary, h_puInfo,
                      firstVertexIsGood, *h_rho);
  ...
  if (!isData) {
    ...
    // cout << "DarkMatterEvent MET: " << genDarkMatterEvent.ReturnNaiveMET() <<
    // endl;
  }
  ...
  const auto eventweights =
      GetWeights(*h_genInfo, *h_lheInfo, eventInfo, selectedPVs,
                 *h_selectedElectronsLoose, *h_selectedMuonsLoose, genTopEvt);
  for (size_t isys = 0; isys < jetSystematics.size(); isys++) {
    auto weights = eventweights;
    auto csvweights =
        GetCSVWeights(*(hs_selectedJets.at(isys)), jetSystematics.at(isys));
    weights.insert(csvweights.begin(), csvweights.end());
    inputs.emplace_back(
        eventInfo, triggerInfo, filterInfo, selectedPVs, *h_selectedMuons,
        *h_selectedMuonsDL, *h_selectedMuonsLoose, *h_selectedElectrons,
        *h_selectedElectronsDL, *h_selectedElectronsLoose, *h_selectedPhotons,
        *(hs_selectedJets.at(isys)), *(hs_selectedJetsLoose.at(isys)),
        *(hs_selectedJetsAK8.at(isys)), *(hs_selectedJetsAK15.at(isys)),
        (*(hs_correctedMETs.at(isys))).at(0),
        // selectedBoostedJets[isys],
        // selectedAk4Cluster,
        genTopEvt, genDarkMatterEvent, *h_genJets, sampleType, higgsdecay,
        weights, iEvent, iSetup, jetSystematics.at(isys), selectionTags, era);
  }
  ...
  for (size_t i_sys = 0; i_sys < jetSystematics.size(); i_sys++) {
    ...
    for (size_t i_sel = 0; i_sel < selections.size() && selected; i_sel++) {
      ...
      if (!taggingSelection) {
        ...
        selected = true;
        next_event = false;
        selectionTags[selectionNames.at(i_sel)] =
            selections.at(i_sel)->IsSelected(inputs[i_sys], cutflows[i_sys]);
      }
    }
...
float BoostedAnalyzer::GetTopPtWeight(const float& toppt1,
                                      const float& toppt2) {
...
std::map<string, float> BoostedAnalyzer::GetWeights(
    const GenEventInfoProduct& genInfo, const LHEEventProduct& lheInfo,
    const EventInfo& eventInfo, const reco::VertexCollection& selectedPVs,
    const std::vector<pat::Electron>& selectedElectrons,
  ...
  for (auto sfit = selectedScaleFactors.begin();
       sfit != selectedScaleFactors.end(); sfit++) {
...
std::map<string, float> BoostedAnalyzer::GetCSVWeights(
    const std::vector<pat::Jet>& selectedJets,
    const Systematics::Type& systype) {
  std::map<string, float> weights;
  ...
  csvweight = csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                         systype, csvWgtHF, csvWgtLF, csvWgtCF);

  weights["Weight_CSV"] = csvweight;

  if (systype == Systematics::NA) {  // only do these for the nominal samples
    weights["Weight_CSVLFup"] =
        csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                   Systematics::CSVLFup, csvWgtHF, csvWgtLF,
                                   csvWgtCF) /
        csvweight;
    weights["Weight_CSVLFdown"] =
        csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                   Systematics::CSVLFdown, csvWgtHF, csvWgtLF,
                                   csvWgtCF) /
        csvweight;
    weights["Weight_CSVHFup"] =
        csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                   Systematics::CSVHFup, csvWgtHF, csvWgtLF,
                                   csvWgtCF) /
        csvweight;
    weights["Weight_CSVHFdown"] =
        csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                   Systematics::CSVHFdown, csvWgtHF, csvWgtLF,
                                   csvWgtCF) /
        csvweight;
    weights["Weight_CSVHFStats1up"] =
        csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                   Systematics::CSVHFStats1up, csvWgtHF,
                                   csvWgtLF, csvWgtCF) /
        csvweight;
    weights["Weight_CSVHFStats1down"] =
        csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                   Systematics::CSVHFStats1down, csvWgtHF,
                                   csvWgtLF, csvWgtCF) /
        csvweight;
    weights["Weight_CSVLFStats1up"] =
        csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                   Systematics::CSVLFStats1up, csvWgtHF,
                                   csvWgtLF, csvWgtCF) /
        csvweight;
    weights["Weight_CSVLFStats1down"] =
        csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                   Systematics::CSVLFStats1down, csvWgtHF,
                                   csvWgtLF, csvWgtCF) /
        csvweight;
    weights["Weight_CSVHFStats2up"] =
        csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                   Systematics::CSVHFStats2up, csvWgtHF,
                                   csvWgtLF, csvWgtCF) /
        csvweight;
    weights["Weight_CSVHFStats2down"] =
        csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                   Systematics::CSVHFStats2down, csvWgtHF,
                                   csvWgtLF, csvWgtCF) /
        csvweight;
    weights["Weight_CSVLFStats2up"] =
        csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                   Systematics::CSVLFStats2up, csvWgtHF,
                                   csvWgtLF, csvWgtCF) /
        csvweight;
    weights["Weight_CSVLFStats2down"] =
        csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                   Systematics::CSVLFStats2down, csvWgtHF,
                                   csvWgtLF, csvWgtCF) /
        csvweight;
    weights["Weight_CSVCErr1up"] =
        csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                   Systematics::CSVCErr1up, csvWgtHF, csvWgtLF,
                                   csvWgtCF) /
        csvweight;
    weights["Weight_CSVCErr1down"] =
        csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                   Systematics::CSVCErr1down, csvWgtHF,
                                   csvWgtLF, csvWgtCF) /
        csvweight;
    weights["Weight_CSVCErr2up"] =
        csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                   Systematics::CSVCErr2up, csvWgtHF, csvWgtLF,
                                   csvWgtCF) /
        csvweight;
    weights["Weight_CSVCErr2down"] =
        csvReweighter.getCSVWeight(jetPts, jetEtas, jetCSVs, jetFlavors,
                                   Systematics::CSVCErr2down, csvWgtHF,
                                   csvWgtLF, csvWgtCF) /
        csvweight;
  }
...
std::string BoostedAnalyzer::outfileName(const std::string& basename,
                                         const Systematics::Type& sysType) {
...
void BoostedAnalyzer::beginRun(edm::Run const& iRun,
...
void BoostedAnalyzer::endRun(edm::Run const& iRun,
                             edm::EventSetup const& iSetup) {
  genweights.Clear();
}

// ------------ method called when starting a luminosity block ------------
void BoostedAnalyzer::beginLuminosityBlock(edm::LuminosityBlock const& iBlock,
                                           edm::EventSetup const& iSetup) {}

// ------------ method fills 'descriptions' with the allowed parameters for the
// module  ------------
void BoostedAnalyzer::fillDescriptions(
