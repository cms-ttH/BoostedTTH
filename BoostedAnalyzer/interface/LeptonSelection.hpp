...
class LeptonSelection: public Selection{
  ...
  public:
    ...
    LeptonSelection(std::vector<std::string> electronTriggers, std::vector<std::string> muonTriggers, std::string channel="both", int step=-1);
    LeptonSelection(std::string electronTrigger="any", std::string muonTrigger="any", std::string channel="both", int step=-1);
    ~LeptonSelection();
