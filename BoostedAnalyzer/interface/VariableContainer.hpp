...
public:
  ...
  void FillIntVar( const TString& name, int value, bool checkIfExists=true );
  void InitVars( const TString& name, float defaultValue, const TString& nEntryVariable, int maxentries =100 );
  void InitVars( const TString& name, const TString& nEntryVariable, int maxentries=100 );
  ...
  int* GetIntVarPointer( const TString& name); 
  int GetIntVar( const TString& name); 
