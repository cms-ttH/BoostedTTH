...
class VariableContainer {
 public:
  ...
  void InitVar(const TString& name, float defaultValue,
               const std::string& type = "F");
  void FillVar(const TString& name, double value);
  void FillFloatVar(const TString& name, float value,
                    bool checkIfExists = true);
  void FillIntVar(const TString& name, long value, bool checkIfExists = true);
  void InitVars(const TString& name, float defaultValue,
                const TString& nEntryVariable, int maxentries = 100);
  void InitVars(const TString& name, const TString& nEntryVariable,
                int maxentries = 100);
