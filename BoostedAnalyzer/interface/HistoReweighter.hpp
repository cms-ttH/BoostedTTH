...
class HistoReweighter {
 public:
  ...
  HistoReweighter(const char* filename, const char* nom_histo_name,
                  const char* denom_histo_name, bool normalize = true);
  HistoReweighter(const char* filename, const char* nom_histo_name,
                  const char* denom_file, const char* denom_histo_name,
                  bool normalize = true);
  float GetWeight(float value, bool interpolate = false);

 private:
  void CreateWeightHisto(TH1* nom_histo, TH1* denom_histo,
                         bool normalize = true);
  TH1* nomHisto;
  TH1* denomHisto;
  float min;
