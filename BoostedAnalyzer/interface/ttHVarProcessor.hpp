...
class ttHVarProcessor : public TreeProcessor {
   public:
    ttHVarProcessor(BoostedRecoType recotype_, MiniAODHelper* helper_, TopTag::Mode topTaggerMode_, TopTag::SubjetAssign subjetAssign_,
                    std::string topTaggerfilePath_, const boosted::SubjetType subjettype_, HiggsTag::Mode higgsTaggerMode_, std::string higgsTaggerFilePath_,
                    std::string prefix = "", bool doMEM = true);
    ~ttHVarProcessor();
