...
class GenWeights {
   public:
    GenWeights();
    void GetGenWeights(std::map< std::string, float >& weights, const LHEEventProduct& LHEEvent) const;
    bool GetLHAPDFWeight(std::map< std::string, float >& weights, const GenEventInfoProduct& genInfos);
