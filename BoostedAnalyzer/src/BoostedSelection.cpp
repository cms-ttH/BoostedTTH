...
BoostedSelection::BoostedSelection(int minTopTags_, int minHiggsTags_)
  : toptagger(TopTag::Likelihood, TopTag::CSV, "toplikelihoodtaggerhistos.root")
  , minTopTags(minTopTags_)
...
void
BoostedSelection::InitCutflow(Cutflow& cutflow)
{

  cutflow.AddStep(">= " + to_string(minTopTags) + " top-tag");
  cutflow.AddStep(">= " + to_string(minHiggsTags) +
                  " non-overlapping jet with higgs-tag");

  initialized = true;
}

bool
BoostedSelection::IsSelected(const InputCollections& input, Cutflow& cutflow)
{
  if (!initialized)
    cerr << "BoostedSelection not initialized" << endl;

  int n_toptags = 0;
  int n_higgstags = 0;

  vector<boosted::BoostedJet> syncTopJets;
  for (auto topjet = input.selectedBoostedJets.begin();
       topjet != input.selectedBoostedJets.end();
       topjet++) {
    // pt and eta requirements on top jet
    if (!(topjet->fatjet.pt() > 200. && abs(topjet->fatjet.eta()) < 2.))
      continue;
  ...
  if (n_toptags >= minTopTags) {
    cutflow.EventSurvivedStep(">= " + to_string(minTopTags) + " top-tag",
                              input.weights.at("Weight"));
  } else {
    return false;
  }

  for (auto higgsJet = input.selectedBoostedJets.begin();
       higgsJet != input.selectedBoostedJets.end();
       ++higgsJet) {
    // pt and eta requirements on higgs jet
    if (!(higgsJet->fatjet.pt() > 200. && abs(higgsJet->fatjet.eta()) < 2))
      continue;
  ...
  if (n_higgstags >= minHiggsTags) {
    cutflow.EventSurvivedStep(">= " + to_string(minHiggsTags) +
                                " non-overlapping jet with higgs-tag",
                              input.weights.at("Weight"));
  } else {
    return false;
