...
BoostedSelection::BoostedSelection(int minTopTags_, int minHiggsTags_)
    : toptagger(TopTag::Likelihood, TopTag::CSV,
...
bool BoostedSelection::IsSelected(const InputCollections& input,
                                  Cutflow& cutflow) {
  ...
  for (auto topjet = input.selectedBoostedJets.begin();
       topjet != input.selectedBoostedJets.end(); topjet++) {
  ...
  for (auto higgsJet = input.selectedBoostedJets.begin();
       higgsJet != input.selectedBoostedJets.end(); ++higgsJet) {
  ...
  if (n_higgstags >= minHiggsTags) {
    cutflow.EventSurvivedStep(
        ">= " + to_string(minHiggsTags) + " non-overlapping jet with higgs-tag",
        input.weights.at("Weight"));
  } else {
    return false;
  }
