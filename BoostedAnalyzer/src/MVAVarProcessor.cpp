...
MVAVarProcessor::MVAVarProcessor() { mem = new MEMClassifier(); }
...
void MVAVarProcessor::Init(const InputCollections& input,
...
void MVAVarProcessor::Process(const InputCollections& input,
                              VariableContainer& vars) {
  ...
  for (std::vector<pat::Jet>::const_iterator itJet = input.selectedJets.begin();
    ...
    if (CSVHelper::PassesCSV(*itJet, btagger, CSVHelper::CSVwp::Medium,
                             input.era)) {
      selectedTaggedJets.push_back(*itJet);
    } else {
      selectedUntaggedJets.push_back(*itJet);
    }
    if (CSVHelper::PassesCSV(*itJet, btagger, CSVHelper::CSVwp::Loose,
                             input.era)) {
      selectedTaggedJetsL.push_back(*itJet);
    }
    if (CSVHelper::PassesCSV(*itJet, btagger, CSVHelper::CSVwp::Tight,
                             input.era)) {
  ...
  if (input.selectedElectrons.size() > 0 || input.selectedMuons.size() > 0) {
    primLepVec = BoostedUtils::GetPrimLepVec(input.selectedElectrons,
                                             input.selectedMuons);
  ...
  vars.FillVar("Evt_CSV_Min",
               csvJetsSorted.size() > 0 ? csvJetsSorted.back() : -.1);
  ...
  vars.FillVar("Evt_CSV_Min_Tagged",
               csvTaggedJets.size() > 0 ? csvTaggedJets.back() : -.1);

  float averageCSVTagged = 0;
  for (std::vector<float>::iterator itCSVTagged = csvTaggedJets.begin();
       itCSVTagged != csvTaggedJets.end(); ++itCSVTagged) {
  ...
  if (input.selectedJets.size() > 1) {
    int idClosestJet1 = -1;
    int idClosestJet2 = -1;
    float minDrJets = BoostedUtils::GetClosestJetIDs(
        idClosestJet1, idClosestJet2, input.selectedJets);
    math::XYZTLorentzVector closestJetVec1 =
        input.selectedJets[idClosestJet1].p4();
    math::XYZTLorentzVector closestJetVec2 =
        input.selectedJets[idClosestJet2].p4();
    vars.FillVar("Evt_M_MinDeltaRJets", (closestJetVec1 + closestJetVec2).M());
    vars.FillVar("Evt_Dr_MinDeltaRJets", minDrJets);
    vars.FillVar("Evt_Pt_MinDeltaRJets",
                 (closestJetVec1 + closestJetVec2).Pt());
  }
  // Tagged Jets
  if (selectedTaggedJets.size() > 1) {
    int idClosestTaggedJet1 = -1;
    int idClosestTaggedJet2 = -1;
    float minDrTaggedJets = BoostedUtils::GetClosestJetIDs(
        idClosestTaggedJet1, idClosestTaggedJet2, selectedTaggedJets);
    math::XYZTLorentzVector closestTaggedJetVec1 =
        selectedTaggedJets[idClosestTaggedJet1].p4();
    math::XYZTLorentzVector closestTaggedJetVec2 =
        selectedTaggedJets[idClosestTaggedJet2].p4();
    vars.FillVar("Evt_M_MinDeltaRTaggedJets",
                 (closestTaggedJetVec1 + closestTaggedJetVec2).M());
    vars.FillVar("Evt_Dr_MinDeltaRTaggedJets", minDrTaggedJets);
    vars.FillVar("Evt_Pt_MinDeltaRTaggedJets",
                 (closestTaggedJetVec1 + closestTaggedJetVec2).Pt());
  }
  // Untagged Jets
  if (selectedUntaggedJets.size() > 1) {
    int idClosestUntaggedJet1 = -1;
    int idClosestUntaggedJet2 = -1;
    float minDrUntaggedJets = BoostedUtils::GetClosestJetIDs(
        idClosestUntaggedJet1, idClosestUntaggedJet2, selectedUntaggedJets);
    math::XYZTLorentzVector closestUntaggedJetVec1 =
        selectedUntaggedJets[idClosestUntaggedJet1].p4();
    math::XYZTLorentzVector closestUntaggedJetVec2 =
        selectedUntaggedJets[idClosestUntaggedJet2].p4();
    vars.FillVar("Evt_M_MinDeltaRUntaggedJets",
                 (closestUntaggedJetVec1 + closestUntaggedJetVec2).M());
    vars.FillVar("Evt_Dr_MinDeltaRUntaggedJets", minDrUntaggedJets);
    vars.FillVar("Evt_Pt_MinDeltaRUntaggedJets",
                 (closestUntaggedJetVec1 + closestUntaggedJetVec2).Pt());
  }
  // Jet and Lepton
  if (input.selectedJets.size() > 1 &&
      (input.selectedElectrons.size() > 0 || input.selectedMuons.size() > 0)) {
    int idClosestJet = -1;
    float minDrLepJet = BoostedUtils::GetClosestLepJetID(
        idClosestJet, primLepVec, input.selectedJets);
    math::XYZTLorentzVector closestJetVec =
        input.selectedJets[idClosestJet].p4();
  ...
  if (selectedTaggedJets.size() > 1 &&
    ...
    float minDrLepTaggedJet = BoostedUtils::GetClosestLepJetID(
        idClosestTaggedJet, primLepVec, selectedTaggedJets);
    math::XYZTLorentzVector closestTaggedJetVec =
        selectedTaggedJets[idClosestTaggedJet].p4();
    vars.FillVar("Evt_M_MinDeltaRLeptonTaggedJet",
                 (primLepVec + closestTaggedJetVec).M());
  ...
  for (std::vector<math::XYZTLorentzVector>::iterator itJetVec1 =
           jetvecs.begin();
       itJetVec1 != jetvecs.end(); ++itJetVec1) {
    mJetsAverage += itJetVec1->M();
    eJetsAverage += itJetVec1->E();
    ptJetsAverage += itJetVec1->Pt();
    etaJetsAverage += itJetVec1->Eta();

    for (std::vector<math::XYZTLorentzVector>::iterator itJetVec2 =
             itJetVec1 + 1;
         itJetVec2 != jetvecs.end(); ++itJetVec2) {
  ...
  for (std::vector<pat::Jet>::iterator itTaggedJet1 =
           selectedTaggedJets.begin();
       itTaggedJet1 != selectedTaggedJets.end(); ++itTaggedJet1) {
    ...
    for (std::vector<pat::Jet>::iterator itTaggedJet2 = itTaggedJet1 + 1;
         itTaggedJet2 != selectedTaggedJets.end(); ++itTaggedJet2) {
      ...
      if (fabs(m2TaggedJets.back() - 125) <
          fabs(m2TaggedJetsClosestTo125 - 125))
      ...
      dktTaggedJetsAverage +=
          BoostedUtils::DeltaKt(taggedJetVec1, taggedJetVec2);
  ...
  if (m2TaggedJets.size() > 0)
    vars.FillVar("Evt_M_MedianTaggedJets",
                 m2TaggedJets.at(m2TaggedJets.size() / 2));
  ...
  for (std::vector<pat::Jet>::iterator itUntaggedJet1 =
           selectedUntaggedJets.begin();
       itUntaggedJet1 != selectedUntaggedJets.end(); ++itUntaggedJet1) {
    ...
    for (std::vector<pat::Jet>::iterator itUntaggedJet2 = itUntaggedJet1 + 1;
         itUntaggedJet2 != selectedUntaggedJets.end(); ++itUntaggedJet2) {
      math::XYZTLorentzVector untaggedJetVec2 = itUntaggedJet2->p4();
      m2UntaggedJetsAverage += (untaggedJetVec1 + untaggedJetVec2).M();
      detaUntaggedJetsAverage +=
          fabs(untaggedJetVec1.Eta() - untaggedJetVec2.Eta());
      drUntaggedJetsAverage +=
          BoostedUtils::DeltaR(untaggedJetVec1, untaggedJetVec2);
      dktUntaggedJetsAverage +=
          BoostedUtils::DeltaKt(untaggedJetVec1, untaggedJetVec2);
      nPairsUntaggedJets++;
    }
  }
  ...
  for (int maxjets = 2; maxjets <= 6; maxjets++) {
    ...
    for (std::vector<math::XYZTLorentzVector>::iterator itJetVec1 =
             jetvecs.begin();
         itJetVec1 != jetvecs.end() && itJetVec1 - jetvecs.begin() < maxjets;
         ++itJetVec1) {
      for (std::vector<math::XYZTLorentzVector>::iterator itJetVec2 =
               itJetVec1 + 1;
           itJetVec2 != jetvecs.end() && itJetVec2 - jetvecs.begin() < maxjets;
           ++itJetVec2) {
        m2JetsAverage += (*itJetVec1 + *itJetVec2).M();
        drJetsAverage += BoostedUtils::DeltaR(*itJetVec1, *itJetVec2);
        dktJetsAverage += BoostedUtils::DeltaKt(*itJetVec1, *itJetVec2);
  ...
  std::sort(jetsByCSV.begin(), jetsByCSV.end(),
            BoostedUtils::FirstHasHigherCSV);
  for (int maxjets = 2; maxjets <= 4; maxjets++) {
    ...
    for (std::vector<pat::Jet>::iterator itJet1 = jetsByCSV.begin();
         itJet1 != jetsByCSV.end() && itJet1 - jetsByCSV.begin() < maxjets;
         ++itJet1) {
      math::XYZTLorentzVector jetVec1 = itJet1->p4();

      for (std::vector<pat::Jet>::iterator itJet2 = itJet1 + 1;
           itJet2 != jetsByCSV.end() && itJet2 - jetsByCSV.begin() < maxjets;
           ++itJet2) {
  ...
  // DeltaEta of Jets
  vars.FillVar("Evt_Jet_MaxDeta_Jets",
               BoostedUtils::GetJetAverageJetEtaMax(input.selectedJets,
                                                    input.selectedJets));
  vars.FillVar("Evt_TaggedJet_MaxDeta_Jets",
               BoostedUtils::GetJetAverageJetEtaMax(input.selectedJets,
                                                    selectedTaggedJets));
  vars.FillVar("Evt_TaggedJet_MaxDeta_TaggedJets",
               BoostedUtils::GetJetAverageJetEtaMax(selectedTaggedJets,
                                                    selectedTaggedJets));
  ...
  for (std::vector<math::XYZTLorentzVector>::iterator itJetVec =
           jetvecs.begin();
       itJetVec != jetvecs.end(); ++itJetVec) {
  ...
  TLorentzVector metvecTL(input.correctedMET.corPx(pat::MET::Type1XY),
                          input.correctedMET.corPy(pat::MET::Type1XY), 0,
  ...
  float best_higgs_mass = bdtvar.getBestHiggsMass(
      primLepVecTL, metvecTL, jetvecsTL, csvJets, minChi, dRbb, bjet1, bjet2,
      jet_loose_vecsTL, jetCSV_loose);
  ...
  bdtvar.study_tops_bb_syst(
      input.correctedMET.corPt(pat::MET::Type1XY),
      input.correctedMET.corP4(pat::MET::Type1XY).phi(), dummy_metv,
      primLepVecTL, jets_vvdouble, csvJets, minChiStudy, chi2lepW, chi2leptop,
      chi2hadW, chi2hadtop, mass_lepW, mass_leptop, mass_hadW, mass_hadtop,
      dRbbStudy, testquant1, testquant2, testquant3, testquant4, testquant5,
      testquant6, testquant7, b1, b2);
  ...
  for (uint i = 0; i < input.selectedJets.size(); i++) {
    jettvecs.push_back(
        BoostedUtils::GetTLorentzVector(input.selectedJets[i].p4()));
  ...
  if (input.selectedJets.size() > 3)
    eth_blr = mem->GetBTagLikelihoodRatio(jettvecs, jetcsvs, out_best_perm,
                                          out_P_4b, out_P_2b);
