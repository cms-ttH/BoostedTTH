...
void MVAVarProcessor::Process(const InputCollections& input, VariableContainer& vars)
{
    ...
    for (std::vector< pat::Jet >::const_iterator itJet = input.selectedJets.begin(); itJet != input.selectedJets.end(); ++itJet) {
        if (CSVHelper::PassesCSV(*itJet, btagger, CSVHelper::CSVwp::Medium, input.era)) { selectedTaggedJets.push_back(*itJet); }
        else {
            selectedUntaggedJets.push_back(*itJet);
        }
        if (CSVHelper::PassesCSV(*itJet, btagger, CSVHelper::CSVwp::Loose, input.era)) { selectedTaggedJetsL.push_back(*itJet); }
        if (CSVHelper::PassesCSV(*itJet, btagger, CSVHelper::CSVwp::Tight, input.era)) { selectedTaggedJetsT.push_back(*itJet); }
    }
    ...
    float averageCSV = 0;
    for (std::vector< double >::iterator itCSV = csvJetsSorted.begin(); itCSV != csvJetsSorted.end(); ++itCSV) { averageCSV += fmax(*itCSV, 0); }
    averageCSV /= csvJetsSorted.size();
    vars.FillVar("Evt_CSV_Average", averageCSV);

    float csvDev = 0;
    for (std::vector< double >::iterator itCSV = csvJetsSorted.begin(); itCSV != csvJetsSorted.end(); ++itCSV) { csvDev += fabs(pow(*itCSV - averageCSV, 2)); }
    ...
    for (int maxjets = 2; maxjets <= 6; maxjets++) {
        ...
        for (std::vector< math::XYZTLorentzVector >::iterator itJetVec1 = jetvecs.begin(); itJetVec1 != jetvecs.end() && itJetVec1 - jetvecs.begin() < maxjets;
             ++itJetVec1) {
            for (std::vector< math::XYZTLorentzVector >::iterator itJetVec2 = itJetVec1 + 1;
                 itJetVec2 != jetvecs.end() && itJetVec2 - jetvecs.begin() < maxjets; ++itJetVec2) {
                m2JetsAverage += (*itJetVec1 + *itJetVec2).M();
                drJetsAverage += BoostedUtils::DeltaR(*itJetVec1, *itJetVec2);
                dktJetsAverage += BoostedUtils::DeltaKt(*itJetVec1, *itJetVec2);
                nPairsJets++;
    ...
    for (std::vector< math::XYZTLorentzVector >::iterator itJetVec = jetvecs.begin(); itJetVec != jetvecs.end(); ++itJetVec) {
        ...
        if (drmax_lj < dr_lj) { drmax_lj = dr_lj; }
        if (detamax_lj < deta_lj) { detamax_lj = deta_lj; }
        if (drmax_j1j < dr_j1j) { drmax_j1j = dr_j1j; }
        if (drmax_j2j < dr_j2j) { drmax_j2j = dr_j2j; }
        if (drmax_j3j < dr_j3j) { drmax_j3j = dr_j3j; }
        if (drmax_j4j < dr_j4j) { drmax_j4j = dr_j4j; }
        if (detamax_j1j < deta_j1j) { detamax_j1j = deta_j1j; }
        if (detamax_j2j < deta_j2j) { detamax_j2j = deta_j2j; }
        if (detamax_j3j < deta_j3j) { detamax_j3j = deta_j3j; }
        if (detamax_j4j < deta_j4j) { detamax_j4j = deta_j4j; }
        math::XYZTLorentzVector p4all;
        for (std::vector< math::XYZTLorentzVector >::iterator itJetVec = jetvecs.begin(); itJetVec != jetvecs.end(); ++itJetVec) { p4all += *itJetVec; }
        ...
        if (costhetamax_jcm < fabs(costheta_jcm)) { costhetamax_jcm = fabs(costheta_jcm); }
    }
    ...
    double         minChiStudy, chi2lepW, chi2leptop, chi2hadW, chi2hadtop, mass_lepW, mass_leptop, mass_hadW, mass_hadtop, dRbbStudy, testquant1, testquant2,
    ...
    double                      out_P_2b = -1;
    double                      eth_blr  = -1;
    if (input.selectedJets.size() > 3) eth_blr = mem->GetBTagLikelihoodRatio(jettvecs, jetcsvs, out_best_perm, out_P_4b, out_P_2b);
