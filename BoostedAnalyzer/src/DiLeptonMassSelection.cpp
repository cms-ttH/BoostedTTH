...
DiLeptonMassSelection::DiLeptonMassSelection(float minMass_,
                                             float maxMass_,
                                             bool invertCut_,
                                             bool cutForDifferentFlavors_)
  : minMass(minMass_)
  , maxMass(maxMass_)
  , invertCut(invertCut_)
  , cutForDifferentFlavors(cutForDifferentFlavors_)
{
}
...
void
DiLeptonMassSelection::InitCutflow(Cutflow& cutflow)
{
  if (cutForDifferentFlavors)
    selectionName = "Any Flavor ";
  else
    selectionName = "Same Flavor ";
...
bool
DiLeptonMassSelection::IsSelected(const InputCollections& input,
                                  Cutflow& cutflow)
{
  if (!initialized)
    cerr << "DiLeptonMassSelection not initialized" << endl;
  ...
  */
  ...
  // cout << "elel_mass: " << elel_mass << "   mumu_mass: " << mumu_mass  << "
  // elmu_mass: " << elmu_mass << endl;

  if (!invertCut) {
    // both/all dilepton masses are outside of the window
    if ((elel_mass < minMass || elel_mass > maxMass) &&
        (mumu_mass < minMass || mumu_mass > maxMass) &&
        (cutForDifferentFlavors &&
         (elmu_mass < minMass || elmu_mass > maxMass))) {
      return false;
    }
  }
  if (invertCut) {
    ...
    if ((elel_mass > minMass && elel_mass < maxMass) ||
        (mumu_mass > minMass && mumu_mass < maxMass) ||
        (cutForDifferentFlavors && elmu_mass > minMass &&
         elmu_mass < maxMass)) {
      return false;
    }
  }
