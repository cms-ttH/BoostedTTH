...
DiLeptonMassSelection::DiLeptonMassSelection(float minMass_, float maxMass_, bool invertCut_, bool cutForDifferentFlavors_) :
    minMass(minMass_),
    maxMass(maxMass_),
...
bool DiLeptonMassSelection::IsSelected(const InputCollections& input, Cutflow& cutflow)
{
    ...
    */
    ...
    for (auto e = input.selectedElectronsLoose.begin(); e != input.selectedElectronsLoose.end(); e++) {
        if (leadingele) {
            if (BoostedUtils::DeltaR(elevecs[0], e->p4()) < 0.001) { continue; }
        }
    ...
    for (auto mu = input.selectedMuonsLoose.begin(); mu != input.selectedMuonsLoose.end(); mu++) {
        if (leadingmu) {
            if (BoostedUtils::DeltaR(muvecs[0], mu->p4()) < 0.001) { continue; }
        }

        muvecs.push_back(mu->p4());
    }

    if ((elevecs.size() + muvecs.size()) < 2) { return false; }

    if (leadingele && elevecs.size() >= 2) { elel_mass = (elevecs[0] + elevecs[1]).M(); }
    if (leadingmu && muvecs.size() >= 2) { mumu_mass = (muvecs[0] + muvecs[1]).M(); }
    ...
    if (!invertCut) {
        ...
        if ((elel_mass < minMass || elel_mass > maxMass) && (mumu_mass < minMass || mumu_mass > maxMass) &&
            (cutForDifferentFlavors && (elmu_mass < minMass || elmu_mass > maxMass))) {
            return false;
        }
    }
    if (invertCut) {
