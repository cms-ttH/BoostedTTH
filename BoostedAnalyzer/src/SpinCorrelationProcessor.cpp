...
TLorentzVector SpinCorrelationProcessor::Boost(
    const TLorentzVector in_vec, Int_t frame_number,
    const TLorentzVector vec_top_, const TLorentzVector vec_antitop_) {
...
// frame
void SpinCorrelationProcessor::SetAllVectors(
    TLorentzVector& vec_top_, TLorentzVector& vec_antitop_,
    TLorentzVector& vec_b_, TLorentzVector& vec_antib_,
    TLorentzVector& vec_lepton_, TLorentzVector& vec_antilepton_,
    TLorentzVector& vec_d_, TLorentzVector& vec_antid_, int identifier) {
  ...
  switch (identifier) {
    case 4:
      ...
      vec_antilepton_ =
          Boost(vec_antilepton_tmp, 1, vec_top_tmp,
                vec_antitop_tmp);  // boost antilepton in ttbar cm system
      vec_lepton_ = Boost(
          vec_lepton_, 3, Boost(vec_top_tmp, 1, vec_top_tmp, vec_antitop_tmp),
          Boost(vec_antitop_tmp, 1, vec_top_tmp,
                vec_antitop_tmp));  // boost lepton in antitop rest frame
      vec_antilepton_ =
          Boost(vec_antilepton_, 2,
                Boost(vec_top_tmp, 1, vec_top_tmp, vec_antitop_tmp),
                Boost(vec_antitop_tmp, 1, vec_top_tmp,
      ...
      vec_antib_ =
          Boost(vec_antib_tmp, 1, vec_top_tmp,
                vec_antitop_tmp);  // boost anti b quark in ttbar cm system
      vec_b_ =
          Boost(vec_b_, 2, Boost(vec_top_tmp, 1, vec_top_tmp, vec_antitop_tmp),
                Boost(vec_antitop_tmp, 1, vec_top_tmp,
                      vec_antitop_tmp));  // boost b quark in top rest frame
      vec_antib_ = Boost(
          vec_antib_, 3, Boost(vec_top_tmp, 1, vec_top_tmp, vec_antitop_tmp),
          Boost(vec_antitop_tmp, 1, vec_top_tmp,
      ...
      vec_d_ =
          Boost(vec_d_, 2, Boost(vec_top_tmp, 1, vec_top_tmp, vec_antitop_tmp),
                Boost(vec_antitop_tmp, 1, vec_top_tmp,
                      vec_antitop_tmp));  // boost d quark in top rest frame
      vec_antid_ = Boost(
          vec_antid_, 3, Boost(vec_top_tmp, 1, vec_top_tmp, vec_antitop_tmp),
          Boost(vec_antitop_tmp, 1, vec_top_tmp,
    ...
    default:
      vec_lepton_ =
          Boost(vec_lepton_tmp, identifier, vec_top_tmp,
                vec_antitop_tmp);  // boost lepton in corresponding system
      vec_antilepton_ =
          Boost(vec_antilepton_tmp, identifier, vec_top_tmp,
      ...
      vec_antitop_ =
          Boost(vec_antitop_tmp, identifier, vec_top_tmp,
...
// number
double SpinCorrelationProcessor::GetVars(
    const TLorentzVector vec_top_, const TLorentzVector vec_antitop_,
    const TLorentzVector vec_b_, const TLorentzVector vec_antib_,
    const TLorentzVector vec_lepton_, const TLorentzVector vec_antilepton_,
  ...
  switch (var_number) {
    ...
    case 12:
      ...
      out = TMath::Cos((vec_lepton_.Vect()).Angle(vec_antitop_.Vect())) *
            TMath::Cos((vec_antilepton_.Vect()).Angle(vec_top_.Vect()));
      break;
    case 13:
    ...
    case 23:
      ...
      break;
    case 24:
      out = ((vec_b_.Vect()).Cross(vec_antib_.Vect())).Dot(vec_lepton_.Vect()) /
            10000.;
      break;
    case 25:
      out = ((vec_b_.Vect()).Cross(vec_antib_.Vect()))
                .Dot(vec_antilepton_.Vect()) /
            10000.;
      break;
    case 26:
...
Interpretation* SpinCorrelationProcessor::GetBestLR(
    int& njets, int& ntags, std::vector<TLorentzVector>& jetvecs,
    std::vector<float>& jetcsvs, TLorentzVector& lepvec, TVector2& metvec,
    bool& flag, float& best_lr) {
  bool use_chi2 = true;
  if (flag) {
    if (ntags < 4) {
      if (njets > 5) {
...
reco::GenParticle SpinCorrelationProcessor::MatchPartontoJet(
    vector<reco::GenParticle>& Partons, pat::Jet& Jet) {
  float DeltaRtmp;
...
SpinCorrelationProcessor::SpinCorrelationProcessor()
    : generator(
          InterpretationGenerator(IntType::tt, 0, 10, -9999, 9999, 0.4941)) {}
SpinCorrelationProcessor::~SpinCorrelationProcessor() {}

void SpinCorrelationProcessor::Init(const InputCollections& input,
  ...
  */
  ...
  for (auto it_frames = frames.begin(); it_frames != frames.end();
       ++it_frames) {
    for (auto it_variables = variables.begin(); it_variables != variables.end();
         ++it_variables) {
      for (auto it_type = var_type.begin(); it_type != var_type.end();
           ++it_type) {
        vars.InitVar((*it_type) + "__" + (*it_frames) + "__" + (*it_variables),
                     -9., "F");
      }
...
void SpinCorrelationProcessor::Process(const InputCollections& input,
                                       VariableContainer& vars) {
  if (!initialized) cerr << "tree processor not initialized" << endl;
  // cout << "Spin Correlation Processor process started " << endl;

  std::map<TString, int>
      frames;  // map which contains the frames which are used and some number
  ...
  for (auto it_type = var_type.begin(); it_type != var_type.end(); ++it_type) {
    ...
    std::map<TString, int>
        variables;  // map which later contains the variables which are used in
    ...
    bool dilepton_reco_flag =
        false;  // flag to signalize that for this event at least the dilepton
    ...
    if (gen_evt_filled && dec_type.EqualTo("GEN")) {
      ...
      */
      ...
      if (isSL) {
        if (lep[0].pdgId() > 0) {
          if (lep[0].pdgId() == 15 && no_tau_selection) {
            continue;
          }
          vec_lepton = lep[0].p4();
      ...
      // pdgid<0 are antileptons
        ...
        if (lep[1].pdgId() > 0) {
          if (lep[1].pdgId() == 15 && no_tau_selection) {
            continue;
          }
          vec_lepton = lep[1].p4();
    ...
    // requirements, the recontruction of the RECO variables can begin
    else if (input.selectedJets.size() >= 4 && dec_type.EqualTo("RECO") &&
             (input.selectedElectrons.size() + input.selectedMuons.size()) ==
                 1 &&
             !((input.selectedElectronsLoose.size() +
                input.selectedMuonsLoose.size()) >= 2)) {
      ...
      std::vector<TLorentzVector> jetvecs = BoostedUtils::GetTLorentzVectors(
      ...
      for (auto j = input.selectedJets.begin(); j != input.selectedJets.end();
        ...
        if (CSVHelper::PassesCSV(*j, "DeepJet", CSVHelper::CSVwp::Medium,
                                 input.era))
          ntags++;
      }
      TLorentzVector lepvec =
          BoostedUtils::GetTLorentzVector(BoostedUtils::GetPrimLepVec(
              input.selectedElectrons, input.selectedMuons));
      TVector2 metvec(input.correctedMET.px(), input.correctedMET.py());
      float best_lr = -9999.;
      bool flag = true;
      Interpretation* best_int_lr = GetBestLR(njets, ntags, jetvecs, jetcsvs,
                                              lepvec, metvec, flag, best_lr);
      if (!(best_lr > -9998.)) {
        best_lr = -9999.;
        flag = false;
        best_int_lr = GetBestLR(njets, ntags, jetvecs, jetcsvs, lepvec, metvec,
                                flag, best_lr);
      }
      ...
      */
      if (best_lr > -9998.) {
        ...
        if (input.selectedElectrons.size() == 1) {
          ...
          if (leptonflag > 0) {
            // cout << "Electron! " << endl;
            vec_lepton.SetPxPyPzE(input.selectedElectrons[0].px(),
                                  input.selectedElectrons[0].py(),
                                  input.selectedElectrons[0].pz(),
                                  input.selectedElectrons[0].energy());
            vec_antilepton = vec_zero;
          } else if (leptonflag < 0) {
            // cout << "Positron! " << endl;
            vec_antilepton.SetPxPyPzE(input.selectedElectrons[0].px(),
                                      input.selectedElectrons[0].py(),
                                      input.selectedElectrons[0].pz(),
                                      input.selectedElectrons[0].energy());
            vec_lepton = vec_zero;
          ...
          if (leptonflag > 0) {
            // cout << "Muon! " << endl;
            vec_lepton.SetPxPyPzE(
                input.selectedMuons[0].px(), input.selectedMuons[0].py(),
                input.selectedMuons[0].pz(), input.selectedMuons[0].energy());
            vec_antilepton = vec_zero;
          } else if (leptonflag < 0) {
            // cout << "AntiMuon! " << endl;
            vec_antilepton.SetPxPyPzE(
                input.selectedMuons[0].px(), input.selectedMuons[0].py(),
                input.selectedMuons[0].pz(), input.selectedMuons[0].energy());
            vec_lepton = vec_zero;
          }
        ...
        if (leptonflag > 0) {
          vec_antitop.SetPxPyPzE(
              best_int_lr->TopLep().Px(), best_int_lr->TopLep().Py(),
              best_int_lr->TopLep().Pz(), best_int_lr->TopLep().E());
          vec_top.SetPxPyPzE(
              best_int_lr->TopHad().Px(), best_int_lr->TopHad().Py(),
              best_int_lr->TopHad().Pz(), best_int_lr->TopHad().E());
          vec_antib.SetPxPyPzE(
              best_int_lr->BLep().Px(), best_int_lr->BLep().Py(),
              best_int_lr->BLep().Pz(), best_int_lr->BLep().E());
          vec_b.SetPxPyPzE(best_int_lr->BHad().Px(), best_int_lr->BHad().Py(),
                           best_int_lr->BHad().Pz(), best_int_lr->BHad().E());
        } else if (leptonflag < 0) {
          vec_antitop.SetPxPyPzE(
              best_int_lr->TopHad().Px(), best_int_lr->TopHad().Py(),
              best_int_lr->TopHad().Pz(), best_int_lr->TopHad().E());
          vec_top.SetPxPyPzE(
              ...
              best_int_lr->TopLep().Pz(), best_int_lr->TopLep().E());
          vec_antib.SetPxPyPzE(
              best_int_lr->BHad().Px(), best_int_lr->BHad().Py(),
              best_int_lr->BHad().Pz(), best_int_lr->BHad().E());
          vec_b.SetPxPyPzE(best_int_lr->BLep().Px(), best_int_lr->BLep().Py(),
                           best_int_lr->BLep().Pz(), best_int_lr->BLep().E());
        } else {
          continue;
        }
        ...
        */
        if (dR_top < dR_max && dR_antitop < dR_max && dR_b < dR_max &&
            dR_antib < dR_max) {
          // cout << "every particle is matched correctly" << endl;
          vars.FillVar("RECO_flag_after_match", 1);
        }

        else if (dR_top < dR_max && dR_antitop < dR_max &&
      ...
      if (input.selectedElectronsLoose.size() == 2) {
        if (input.selectedElectronsLoose[0].charge() < 0) {
          vec_lepton.SetPxPyPzE(input.selectedElectronsLoose[0].px(),
                                input.selectedElectronsLoose[0].py(),
                                input.selectedElectronsLoose[0].pz(),
                                input.selectedElectronsLoose[0].energy());
        } else if (input.selectedElectronsLoose[0].charge() > 0) {
          vec_antilepton.SetPxPyPzE(input.selectedElectronsLoose[0].px(),
                                    input.selectedElectronsLoose[0].py(),
                                    input.selectedElectronsLoose[0].pz(),
                                    input.selectedElectronsLoose[0].energy());
        }
        if (input.selectedElectronsLoose[1].charge() < 0) {
          vec_lepton.SetPxPyPzE(input.selectedElectronsLoose[1].px(),
                                input.selectedElectronsLoose[1].py(),
                                input.selectedElectronsLoose[1].pz(),
                                input.selectedElectronsLoose[1].energy());
        } else if (input.selectedElectronsLoose[1].charge() > 0) {
          vec_antilepton.SetPxPyPzE(input.selectedElectronsLoose[1].px(),
                                    input.selectedElectronsLoose[1].py(),
                                    input.selectedElectronsLoose[1].pz(),
                                    input.selectedElectronsLoose[1].energy());
        }
        ...
        if (input.selectedMuonsLoose[0].charge() < 0) {
          vec_lepton.SetPxPyPzE(input.selectedMuonsLoose[0].px(),
                                input.selectedMuonsLoose[0].py(),
                                input.selectedMuonsLoose[0].pz(),
                                input.selectedMuonsLoose[0].energy());
        } else if (input.selectedMuonsLoose[0].charge() > 0) {
          vec_antilepton.SetPxPyPzE(input.selectedMuonsLoose[0].px(),
                                    input.selectedMuonsLoose[0].py(),
                                    input.selectedMuonsLoose[0].pz(),
                                    input.selectedMuonsLoose[0].energy());
        }
        if (input.selectedMuonsLoose[1].charge() < 0) {
          vec_lepton.SetPxPyPzE(input.selectedMuonsLoose[1].px(),
                                input.selectedMuonsLoose[1].py(),
                                input.selectedMuonsLoose[1].pz(),
                                input.selectedMuonsLoose[1].energy());
        } else if (input.selectedMuonsLoose[1].charge() > 0) {
          vec_antilepton.SetPxPyPzE(input.selectedMuonsLoose[1].px(),
                                    input.selectedMuonsLoose[1].py(),
                                    input.selectedMuonsLoose[1].pz(),
                                    input.selectedMuonsLoose[1].energy());
        }
      ...
      if (isSL) {
        ...
        if (leptonflag == 1) {
          ...
          if (nwhadjets > 1) {
            vec_antitop =
                hadtopjets[0].p4() + hadWjets[0].p4() + hadWjets[1].p4();
          } else if (nwhadjets == 1) {
    ...
    for (auto it_frames = frames.begin(); it_frames != frames.end();
      ...
      TLorentzVector vec_antilepton_ =
          BoostedUtils::GetTLorentzVector(vec_antilepton);
      TLorentzVector vec_top_ = BoostedUtils::GetTLorentzVector(vec_top);
      TLorentzVector vec_antitop_ =
          BoostedUtils::GetTLorentzVector(vec_antitop);
      ...
      */
      ...
      SetAllVectors(vec_top_, vec_antitop_, vec_b_, vec_antib_, vec_lepton_,
                    vec_antilepton_, vec_d_, vec_antid_, it_frames->second);
      ...
      */
      ...
      for (auto it_variables = variables.begin();
           it_variables != variables.end(); ++it_variables) {
        if (dilepton_reco_flag) {
          if ((it_frames->second) == 0) {
            vars.FillVar(
                *it_type + "__" + it_frames->first + "__" + it_variables->first,
                GetVars(vec_top_, vec_antitop_, vec_b_, vec_antib_, vec_lepton_,
                        vec_antilepton_, vec_d_, vec_antid_,
                        it_variables->second));
          }
        } else {
          vars.FillVar(
