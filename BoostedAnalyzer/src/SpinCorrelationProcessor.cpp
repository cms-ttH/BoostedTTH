...
TLorentzVector
SpinCorrelationProcessor::Boost(const TLorentzVector in_vec,
                                Int_t frame_number,
...
// frame
void
SpinCorrelationProcessor::SetAllVectors(TLorentzVector& vec_top_,
                                        TLorentzVector& vec_antitop_,
                                        TLorentzVector& vec_b_,
                                        TLorentzVector& vec_antib_,
                                        TLorentzVector& vec_lepton_,
                                        TLorentzVector& vec_antilepton_,
                                        TLorentzVector& vec_d_,
                                        TLorentzVector& vec_antid_,
                                        int identifier)
{
  ...
  switch (identifier) {
    case 4:
      vec_lepton_ = Boost(vec_lepton_tmp,
                          1,
                          vec_top_tmp,
                          vec_antitop_tmp); // boost lepton in ttbar cm system
      vec_antilepton_ =
        Boost(vec_antilepton_tmp,
              1,
              vec_top_tmp,
...
// number
double
SpinCorrelationProcessor::GetVars(const TLorentzVector vec_top_,
                                  const TLorentzVector vec_antitop_,
                                  const TLorentzVector vec_b_,
                                  const TLorentzVector vec_antib_,
                                  const TLorentzVector vec_lepton_,
                                  const TLorentzVector vec_antilepton_,
                                  const TLorentzVector vec_d_,
                                  const TLorentzVector vec_antid_,
                                  Int_t var_number)
{
  double out = -99.;
  ...
  switch (var_number) {
    ...
    case 25:
      out =
        ((vec_b_.Vect()).Cross(vec_antib_.Vect())).Dot(vec_antilepton_.Vect()) /
        10000.;
      break;
    case 26:
...
Interpretation*
SpinCorrelationProcessor::GetBestLR(int& njets,
                                    int& ntags,
...
reco::GenParticle
SpinCorrelationProcessor::MatchPartontoJet(vector<reco::GenParticle>& Partons,
                                           pat::Jet& Jet)
{
  float DeltaRtmp;
...
SpinCorrelationProcessor::SpinCorrelationProcessor()
  : generator(InterpretationGenerator(IntType::tt, 0, 10, -9999, 9999, 0.8484))
{
}
SpinCorrelationProcessor::~SpinCorrelationProcessor()
{
...
void
SpinCorrelationProcessor::Init(const InputCollections& input,
  ...
  for (auto it_frames = frames.begin(); it_frames != frames.end();
    ...
    for (auto it_variables = variables.begin(); it_variables != variables.end();
      ...
      for (auto it_type = var_type.begin(); it_type != var_type.end();
           ++it_type) {
        vars.InitVar(
          (*it_type) + "__" + (*it_frames) + "__" + (*it_variables), -9., "F");
      }
...
void
SpinCorrelationProcessor::Process(const InputCollections& input,
                                  VariableContainer& vars)
{
  if (!initialized)
  ...
  std::map<TString, int> frames; // map which contains the frames which are used
                                 // and some number for later identification
  ...
  for (auto it_type = var_type.begin(); it_type != var_type.end(); ++it_type) {
    ...
    std::map<TString, int> variables; // map which later contains the variables
                                      // which are used in the event and some
    ...
    // requirements, the recontruction of the RECO variables can begin
      ...
      float best_lr = -9999.;
      bool flag = true;
      Interpretation* best_int_lr = GetBestLR(
        njets, ntags, jetvecs, jetcsvs, lepvec, metvec, flag, best_lr);
      if (!(best_lr > -9998.)) {
        best_lr = -9999.;
        flag = false;
        best_int_lr = GetBestLR(
          njets, ntags, jetvecs, jetcsvs, lepvec, metvec, flag, best_lr);
      }
      ...
      */
      if (best_lr > -9998.) {
        ...
        if (input.selectedElectrons.size() == 1) {
          ...
          if (leptonflag > 0) {
            // cout << "Muon! " << endl;
            vec_lepton.SetPxPyPzE(input.selectedMuons[0].px(),
                                  input.selectedMuons[0].py(),
                                  input.selectedMuons[0].pz(),
                                  input.selectedMuons[0].energy());
            vec_antilepton = vec_zero;
          } else if (leptonflag < 0) {
            // cout << "AntiMuon! " << endl;
            vec_antilepton.SetPxPyPzE(input.selectedMuons[0].px(),
                                      input.selectedMuons[0].py(),
                                      input.selectedMuons[0].pz(),
                                      input.selectedMuons[0].energy());
            vec_lepton = vec_zero;
        ...
        if (leptonflag > 0) {
          vec_antitop.SetPxPyPzE(best_int_lr->TopLep().Px(),
                                 best_int_lr->TopLep().Py(),
                                 best_int_lr->TopLep().Pz(),
                                 best_int_lr->TopLep().E());
          vec_top.SetPxPyPzE(best_int_lr->TopHad().Px(),
                             best_int_lr->TopHad().Py(),
                             best_int_lr->TopHad().Pz(),
                             best_int_lr->TopHad().E());
          vec_antib.SetPxPyPzE(best_int_lr->BLep().Px(),
                               best_int_lr->BLep().Py(),
                               best_int_lr->BLep().Pz(),
                               best_int_lr->BLep().E());
          vec_b.SetPxPyPzE(best_int_lr->BHad().Px(),
                           best_int_lr->BHad().Py(),
                           best_int_lr->BHad().Pz(),
                           best_int_lr->BHad().E());
        } else if (leptonflag < 0) {
          vec_antitop.SetPxPyPzE(best_int_lr->TopHad().Px(),
                                 best_int_lr->TopHad().Py(),
                                 best_int_lr->TopHad().Pz(),
                                 best_int_lr->TopHad().E());
          vec_top.SetPxPyPzE(best_int_lr->TopLep().Px(),
                             best_int_lr->TopLep().Py(),
                             best_int_lr->TopLep().Pz(),
                             best_int_lr->TopLep().E());
          vec_antib.SetPxPyPzE(best_int_lr->BHad().Px(),
                               best_int_lr->BHad().Py(),
                               best_int_lr->BHad().Pz(),
                               best_int_lr->BHad().E());
          vec_b.SetPxPyPzE(best_int_lr->BLep().Px(),
                           best_int_lr->BLep().Py(),
                           best_int_lr->BLep().Pz(),
                           best_int_lr->BLep().E());
        } else {
        ...
        some dedicated processor would be better for this
        if(dR_top<dR_max) {
    ...
    for (auto it_frames = frames.begin(); it_frames != frames.end();
      ...
      */
      ...
      for (auto it_variables = variables.begin();
           it_variables != variables.end();
           ++it_variables) {
        if (dilepton_reco_flag) {
          if ((it_frames->second) == 0) {
            vars.FillVar(*it_type + "__" + it_frames->first + "__" +
                           it_variables->first,
                         GetVars(vec_top_,
                                 vec_antitop_,
                                 vec_b_,
