...
// function to do some basic boosts on a TLorentzVectors using the number of the
// corresponding frame and the top/antitop 4-vectors
TLorentzVector SpinCorrelationProcessor::Boost(const TLorentzVector in_vec, Int_t frame_number, const TLorentzVector vec_top_,
...
// function which sets all the 4-vectors of the event according to the desired
// frame
void SpinCorrelationProcessor::SetAllVectors(TLorentzVector& vec_top_, TLorentzVector& vec_antitop_, TLorentzVector& vec_b_, TLorentzVector& vec_antib_,
                                             TLorentzVector& vec_lepton_, TLorentzVector& vec_antilepton_, TLorentzVector& vec_d_, TLorentzVector& vec_antid_,
                                             int identifier)
{
    ...
    switch (identifier) {
        case 4:
            vec_lepton_     = Boost(vec_lepton_tmp, 1, vec_top_tmp,
                                vec_antitop_tmp);  // boost lepton in ttbar cm system
            vec_antilepton_ = Boost(vec_antilepton_tmp, 1, vec_top_tmp,
                                    vec_antitop_tmp);  // boost antilepton in ttbar cm system
            vec_lepton_     = Boost(vec_lepton_, 3, Boost(vec_top_tmp, 1, vec_top_tmp, vec_antitop_tmp),
                                Boost(vec_antitop_tmp, 1, vec_top_tmp,
                                      vec_antitop_tmp));  // boost lepton in antitop rest frame
            vec_antilepton_ = Boost(vec_antilepton_, 2, Boost(vec_top_tmp, 1, vec_top_tmp, vec_antitop_tmp),
                                    Boost(vec_antitop_tmp, 1, vec_top_tmp,
                                          vec_antitop_tmp));  // boost antilepton in top rest frame
            vec_b_          = Boost(vec_b_tmp, 1, vec_top_tmp,
                           vec_antitop_tmp);  // boost b quark in ttbar cm system
            vec_antib_      = Boost(vec_antib_tmp, 1, vec_top_tmp,
                               vec_antitop_tmp);  // boost anti b quark in ttbar cm system
            vec_b_          = Boost(vec_b_, 2, Boost(vec_top_tmp, 1, vec_top_tmp, vec_antitop_tmp),
                           Boost(vec_antitop_tmp, 1, vec_top_tmp,
                                 vec_antitop_tmp));  // boost b quark in top rest frame
            vec_antib_      = Boost(vec_antib_, 3, Boost(vec_top_tmp, 1, vec_top_tmp, vec_antitop_tmp),
                               Boost(vec_antitop_tmp, 1, vec_top_tmp,
                                     vec_antitop_tmp));  // boost anti b quark in antitop rest frame
            vec_top_        = Boost(vec_top_tmp, 1, vec_top_tmp,
                             vec_antitop_tmp);  // boost top in ttbar cm frame
            vec_antitop_    = Boost(vec_antitop_tmp, 1, vec_top_tmp,
                                 vec_antitop_tmp);  // boost antitop in ttbar cm frame
            vec_d_          = Boost(vec_d_tmp, 1, vec_top_tmp,
                           vec_antitop_tmp);  // boost d quark in ttbar cm frame
            vec_antid_      = Boost(vec_antid_tmp, 1, vec_top_tmp,
                               vec_antitop_tmp);  // boost antid in ttbar cm frame
            vec_d_          = Boost(vec_d_, 2, Boost(vec_top_tmp, 1, vec_top_tmp, vec_antitop_tmp),
                           Boost(vec_antitop_tmp, 1, vec_top_tmp,
                                 vec_antitop_tmp));  // boost d quark in top rest frame
            vec_antid_      = Boost(vec_antid_, 3, Boost(vec_top_tmp, 1, vec_top_tmp, vec_antitop_tmp),
                               Boost(vec_antitop_tmp, 1, vec_top_tmp,
                                     vec_antitop_tmp));  // boost antid in antitop rest frame
            ...
            break;
        default:
            vec_lepton_     = Boost(vec_lepton_tmp, identifier, vec_top_tmp,
                                vec_antitop_tmp);  // boost lepton in corresponding system
            vec_antilepton_ = Boost(vec_antilepton_tmp, identifier, vec_top_tmp,
                                    vec_antitop_tmp);  // boost antilepton in corresponding system
            vec_top_        = Boost(vec_top_tmp, identifier, vec_top_tmp,
                             vec_antitop_tmp);  // boost top in corresponding system
            vec_antitop_    = Boost(vec_antitop_tmp, identifier, vec_top_tmp,
                                 vec_antitop_tmp);  // boost antitop in corresponding system
            vec_b_          = Boost(vec_b_tmp, identifier, vec_top_tmp,
                           vec_antitop_tmp);  // boost b quark in corresponding system
            vec_antib_      = Boost(vec_antib_tmp, identifier, vec_top_tmp,
                               vec_antitop_tmp);  // boost anti b quark in corresponding system
            vec_d_          = Boost(vec_d_tmp, identifier, vec_top_tmp,
                           vec_antitop_tmp);  // bost downtype quark in corresponding system
            vec_antid_      = Boost(vec_antid_tmp, identifier, vec_top_tmp,
                               vec_antitop_tmp);  // boost downtype antiquark in corresponding system
            break;
    }
...
// this function calculates all desired variables using their identification
// number
double SpinCorrelationProcessor::GetVars(const TLorentzVector vec_top_, const TLorentzVector vec_antitop_, const TLorentzVector vec_b_,
                                         const TLorentzVector vec_antib_, const TLorentzVector vec_lepton_, const TLorentzVector vec_antilepton_,
                                         const TLorentzVector vec_d_, const TLorentzVector vec_antid_, Int_t var_number)
{
    double out = -99.;
    ...
    switch (var_number) {
        ...
        case 2:
            ...
            if (out > TMath::Pi()) { out = 2 * TMath::Pi() - out; }
            break;
        case 3:
            // cos(l lbar)
        ...
        case 13:
            ...
            break;
        case 14: out = TMath::Cos((vec_lepton_.Vect()).Angle(vec_antid_.Vect())); break;
        case 15: out = TMath::Cos((vec_antilepton_.Vect()).Angle(vec_d_.Vect())); break;
        case 16: out = vec_top_.M(); break;
        case 17: out = vec_antitop_.M(); break;
        case 18: out = vec_b_.M(); break;
        case 19: out = vec_antib_.M(); break;
        case 20: out = vec_lepton_.M(); break;
        case 21: out = vec_antilepton_.M(); break;
        case 22: out = TMath::Abs(vec_lepton_.Eta() - vec_b_.Eta()); break;
        case 23: out = TMath::Abs(vec_antilepton_.Eta() - vec_antib_.Eta()); break;
        case 24: out = ((vec_b_.Vect()).Cross(vec_antib_.Vect())).Dot(vec_lepton_.Vect()) / 10000.; break;
        case 25: out = ((vec_b_.Vect()).Cross(vec_antib_.Vect())).Dot(vec_antilepton_.Vect()) / 10000.; break;
        case 26: out = TMath::Cos((vec_d_.Vect()).Angle(vec_antid_.Vect())); break;
        default: cerr << "no identifier for used variable" << endl;
    }
    return out;
}

Interpretation* SpinCorrelationProcessor::GetBestLR(int& njets, int& ntags, std::vector< TLorentzVector >& jetvecs, std::vector< float >& jetcsvs,
                                                    TLorentzVector& lepvec, TVector2& metvec, bool& flag, float& best_lr)
{
    bool use_chi2 = true;
    if (flag) {
        if (ntags < 4) {
            if (njets > 5) { generator.SetVars(IntType::tth, 10); }
            else {
        ...
        if (ntags < 4) {
            if (njets > 5) { generator.SetVars(IntType::tt, 8); }
            else {
                generator.SetVars(IntType::tth, 10);
            }
        }
    ...
    for (uint i = 0; i < nints; i++) {
        ...
        if (flag) {
            if (ntags < 4) {
                if (njets > 5) {
                    if (!use_chi2) { lr = quality.TTHLikelihood_comb(*(ints[i])); }
                    else if (use_chi2) {
                        lr = quality.TTHChi2(*(ints[i]));
                    }
                }
                else {
                    if (!use_chi2) { lr = quality.TTLikelihood_comb(*(ints[i])); }
                    else if (use_chi2) {
...
void SpinCorrelationProcessor::Process(const InputCollections& input, VariableContainer& vars)
{
    ...
    std::map< TString, int > frames;  // map which contains the frames which are used and some number
                                      // for later identification
    std::vector< TString > var_type;  // just string vector which contains GEN and
                                      // RECO to use it in loop
    ...
    for (auto it_type = var_type.begin(); it_type != var_type.end(); ++it_type) {
        ...
        std::map< TString, int > variables;  // map which later contains the variables which are used in
                                             // the event and some number for later identification
        ...
        bool dilepton_reco_flag = false;  // flag to signalize that for this event at least the dilepton
                                          // variables in the lab frame could be used, which is of course
                                          // possible even without top-event reconstruction

        // cout << "Gen Top Event is filled? " << gen_evt_filled << endl;

        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // if top evt is filled and the loop is at var_type GEN, then the
        // reconstruction for GEN variables can begin
        if (gen_evt_filled && dec_type.EqualTo("GEN")) {
            ...
            */
            ...
            // in SL case, the lepton/antilepton which is not present will be assigned
            // with a (0,0,0,0) 4-vector, pdgid>0 ->leptons pdgid<0 antileptons
            if (isSL) {
                if (lep[0].pdgId() > 0) {
            ...
            // in DL case, the assignment is straight forward. pdgid>0 are leptons,
            // pdgid<0 are antileptons
            else if (isDL) {
                if (lep[0].pdgId() > 0) {
                    if (lep[0].pdgId() == 15 && no_tau_selection) { continue; }
                    vec_lepton = lep[0].p4();
            ...
            /*
              ...
              for(auto it=q1.begin();it!=q1.end();++it) {
                if(abs(it->pdgId())==1 || abs(it->pdgId())==3 || abs(it->pdgId())==5)
            { vec_d=it->p4(); }
              }
            ...
            */
            // now save the top/antitop, b/antib vectors for later comparison with the
            // RECO vectors
            vec_top_tmp     = vec_top;
        ...
        /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // if the loop is in var_type RECO and the events satisfies the
        // requirements, the recontruction of the RECO variables can begin
        else if (input.selectedJets.size() >= 4 && dec_type.EqualTo("RECO") && (input.selectedElectrons.size() + input.selectedMuons.size()) == 1 &&
                 !((input.selectedElectronsLoose.size() + input.selectedMuonsLoose.size()) >= 2)) {
            // cout << "N_jets>=4 and decay type RECO and Electrons+Muons==1" << endl;
            // this is the code of Hannes ttbar reconstruction processor to create
            // some likelihood/chi2 interpretations of the event
            ...
            Interpretation** ints =
            generator.GenerateTTHInterpretations(jetvecs,jetcsvs,lepvec,metvec); uint
            nints = generator.GetNints(); Interpretation* best_int_lr=0; float
            best_lr=-99999.;
            //    float best_chi2=-99999;
            for(uint i=0; i<nints; i++){
                // likelihood ratio good ttbar reco / combinatorial background
            ...
            */
            if (best_lr > -9998.) {
                ...
                // now check if the SL event has an electron- or muon-lepton and whether
                // it is a lepton or antilepton
                if (input.selectedElectrons.size() == 1) {
                    leptonflag = -input.selectedElectrons[0].charge();
                    if (leptonflag > 0) {
                        // cout << "Electron! " << endl;
                        vec_lepton.SetPxPyPzE(input.selectedElectrons[0].px(), input.selectedElectrons[0].py(), input.selectedElectrons[0].pz(),
                                              input.selectedElectrons[0].energy());
                        vec_antilepton = vec_zero;
                    }
                    else if (leptonflag < 0) {
                        // cout << "Positron! " << endl;
                        vec_antilepton.SetPxPyPzE(input.selectedElectrons[0].px(), input.selectedElectrons[0].py(), input.selectedElectrons[0].pz(),
                                                  input.selectedElectrons[0].energy());
                        vec_lepton = vec_zero;
                    }
                }
                    ...
                    if (leptonflag > 0) {
                        // cout << "Muon! " << endl;
                        vec_lepton.SetPxPyPzE(input.selectedMuons[0].px(), input.selectedMuons[0].py(), input.selectedMuons[0].pz(),
                                              input.selectedMuons[0].energy());
                        vec_antilepton = vec_zero;
                    }
                    else if (leptonflag < 0) {
                        // cout << "AntiMuon! " << endl;
                        vec_antilepton.SetPxPyPzE(input.selectedMuons[0].px(), input.selectedMuons[0].py(), input.selectedMuons[0].pz(),
                                                  input.selectedMuons[0].energy());
                        vec_lepton = vec_zero;
                    }
                }
                ...
                // depending on the outcome of the SL event classification, the
                // corresponding 4-vectors are set
                if (leptonflag > 0) {
                ...
                // now do a Delta R Matching of the reconstructed 4-vectors with the
                // saved GEN vectors
                float dR_max     = 0.4;
                ...
                /*
                // set some flags which are of interest for the reconstruction -> maybe
                some dedicated processor would be better for this if(dR_top<dR_max) {
                ...
                */
                if (dR_top < dR_max && dR_antitop < dR_max && dR_b < dR_max && dR_antib < dR_max) {
                 ...
                 dR_b_switch<dR_max && dR_antib_switch<dR_max) {
        ...
        else if (input.selectedJetsLoose.size() >= 2 && dec_type.EqualTo("RECO") &&
                 ((input.selectedElectronsLoose.size() + input.selectedMuonsLoose.size()) == 2) &&
                 !((input.selectedElectrons.size() + input.selectedMuons.size()) == 1)) {
            isSL = false;
            isDL = true;
            // isAH=false;
            // cout << "DiLepton Reconstruction ! " << endl;
            dilepton_reco_flag = true;
            // now check if there are 2 electrons or 2 muons or one of both and assign
            // the correct 4 vectors
            if (input.selectedElectronsLoose.size() == 2) {
                if (input.selectedElectronsLoose[0].charge() < 0) {
                    vec_lepton.SetPxPyPzE(input.selectedElectronsLoose[0].px(), input.selectedElectronsLoose[0].py(), input.selectedElectronsLoose[0].pz(),
                                          input.selectedElectronsLoose[0].energy());
                }
                else if (input.selectedElectronsLoose[0].charge() > 0) {
                    vec_antilepton.SetPxPyPzE(input.selectedElectronsLoose[0].px(), input.selectedElectronsLoose[0].py(), input.selectedElectronsLoose[0].pz(),
                                              input.selectedElectronsLoose[0].energy());
                }
                if (input.selectedElectronsLoose[1].charge() < 0) {
                    vec_lepton.SetPxPyPzE(input.selectedElectronsLoose[1].px(), input.selectedElectronsLoose[1].py(), input.selectedElectronsLoose[1].pz(),
                                          input.selectedElectronsLoose[1].energy());
                }
                else if (input.selectedElectronsLoose[1].charge() > 0) {
                    vec_antilepton.SetPxPyPzE(input.selectedElectronsLoose[1].px(), input.selectedElectronsLoose[1].py(), input.selectedElectronsLoose[1].pz(),
                                              input.selectedElectronsLoose[1].energy());
                }
            }
            else if (input.selectedMuonsLoose.size() == 2) {
                if (input.selectedMuonsLoose[0].charge() < 0) {
                    vec_lepton.SetPxPyPzE(input.selectedMuonsLoose[0].px(), input.selectedMuonsLoose[0].py(), input.selectedMuonsLoose[0].pz(),
                                          input.selectedMuonsLoose[0].energy());
                }
                else if (input.selectedMuonsLoose[0].charge() > 0) {
                    vec_antilepton.SetPxPyPzE(input.selectedMuonsLoose[0].px(), input.selectedMuonsLoose[0].py(), input.selectedMuonsLoose[0].pz(),
                                              input.selectedMuonsLoose[0].energy());
                }
                if (input.selectedMuonsLoose[1].charge() < 0) {
                    vec_lepton.SetPxPyPzE(input.selectedMuonsLoose[1].px(), input.selectedMuonsLoose[1].py(), input.selectedMuonsLoose[1].pz(),
                                          input.selectedMuonsLoose[1].energy());
                }
                else if (input.selectedMuonsLoose[1].charge() > 0) {
                    vec_antilepton.SetPxPyPzE(input.selectedMuonsLoose[1].px(), input.selectedMuonsLoose[1].py(), input.selectedMuonsLoose[1].pz(),
                                              input.selectedMuonsLoose[1].energy());
                }
            }
            else {
            ...
            // some gen info used, because only the effect of wrong permutations shall
            // be examined
            ...
            if (isSL) { DecayProducts.push_back(input.genTopEvt.GetWQuarks()); }
            DecayProducts.push_back(input.genTopEvt.GetAllTopLepDecayQuarks());
            if (isSL) { DecayProducts.push_back(input.genTopEvt.GetAllTopHadDecayQuarks()); }
            // cout << "decay products vector size = " << DecayProducts.size() <<
            // endl;
            vector< pat::Jet > Jets;
            if (isSL) { Jets = input.selectedJets; }
            else if (isDL) {
            ...
            for (auto& Jet : Jets) {
                ...
                for (auto& Quarks : DecayProducts) { MatchingQuarks.push_back(MatchPartontoJet(Quarks, Jet)); }
                size_t iQ = 0;
                if (isSL) { iQ = 0; }
                else if (isDL) {
                    iQ = 1;
            ...
            //////////////////////////////////////////////////////
            // after gen quark jet matching use the gen info for the rest, leptons and
            // neutrinos
            if (isSL) {
                ...
                if (leptonflag == 1) {
                    if (nwhadjets > 1) { vec_top = hadtopjets[0].p4() + hadWjets[0].p4() + hadWjets[1].p4(); }
                    else if (nwhadjets == 1) {
                    ...
                    if (nwhadjets > 1) { vec_antitop = hadtopjets[0].p4() + hadWjets[0].p4() + hadWjets[1].p4(); }
                    else if (nwhadjets == 1) {
                ...
                if (!(nleptopjets == 2 && nhadtopjets == 0 && nwhadjets == 0)) { continue; }
                ...
                if (nu[0].pdgId() > 0) { vec_nu = nu[0].p4(); }
                else if (nu[0].pdgId() < 0) {
                    vec_antinu = nu[0].p4();
                }
                if (nu[1].pdgId() > 0) { vec_nu = nu[1].p4(); }
                else if (nu[1].pdgId() < 0) {
                    vec_antinu = nu[1].p4();
                }
                if (pdgids[0] > 0) { vec_b = leptopjets[0].p4(); }
                else if (pdgids[0] < 0) {
                    vec_antib = leptopjets[0].p4();
                }
                if (pdgids[1] > 0) { vec_b = leptopjets[1].p4(); }
                else if (pdgids[1] < 0) {
        ...
        // if neither GEN nor RECO part worked the loop ends here
            ...
            // cout << *it_type << " nothing could be done in this round of the loop!!
            // " << endl;
            continue;
        }

        // now with the reconstructed 4-vectors the calculation of the variables
        // starts
        for (auto it_frames = frames.begin(); it_frames != frames.end(); ++it_frames) {
            ...
            */
            ...
            // the SetAllVectors function uses the reconstructed 4-vectors and changes
            // them according to the desired frame/boost using the number of the frame
            ...
            // if SL event: depending on lepton or antilepton, the variables which
            // make sense are defined for the event
            if (isSL) {
            ...
            // same for DL events
                ...
                // variables["M_l"]=20;
                // variables["M_lbar"]=21;
                // these variables will only be filled if the dilepton reco flag isnt
                // set, because they can only be filled if the whole event is
                // reconstructed which cannot be done in DL case
            ...
            */
            // now the GetVars function calculates the desired variable depending on
            // the number of the variable using the 4-vectors of the event
            for (auto it_variables = variables.begin(); it_variables != variables.end(); ++it_variables) {
                if (dilepton_reco_flag) {
                    if ((it_frames->second) == 0) {
                        vars.FillVar(
                            *it_type + "__" + it_frames->first + "__" + it_variables->first,
                            GetVars(vec_top_, vec_antitop_, vec_b_, vec_antib_, vec_lepton_, vec_antilepton_, vec_d_, vec_antid_, it_variables->second));
                    }
                }
                else {
                    vars.FillVar(*it_type + "__" + it_frames->first + "__" + it_variables->first,
                                 GetVars(vec_top_, vec_antitop_, vec_b_, vec_antib_, vec_lepton_, vec_antilepton_, vec_d_, vec_antid_, it_variables->second));
                    // cout << *it_type+"__"+it_frames->first+"__"+it_variables->first <<
                    // " filled with " <<
                    // GetVars(vec_top_,vec_antitop_,vec_b_,vec_antib_,vec_lepton_,vec_antilepton_,vec_d_,vec_antid_,it_variables->second)
                    // << endl;
                }
