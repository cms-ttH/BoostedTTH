...
void
DiJetVarProcessor::Init(const InputCollections& input, VariableContainer& vars)
{
...
void
DiJetVarProcessor::Process(const InputCollections& input,
                           VariableContainer& vars)
{
  if (!initialized)
    throw cms::Exception("BadProcessor") << "DiJetVarProcessor not initialized";
  ...
  for (std::vector<pat::Jet>::const_iterator itJet = jets.begin();
    ...
    if (itJet->genJet() != 0) {
      // Info on jet flavour:
      // https://indico.cern.ch/event/448366/session/0/contribution/5/attachments/1159737/1669162/BTVSWNews_24_9_2015.pdf
...
void
DiJetVarProcessor::fillSecondaryVertexInfo(VariableContainer& vars,
                                           const pat::Jet& jet,
                                           const size_t iJet) const
{
  ...
  // jet.tagInfoLabels().begin(); it != jet.tagInfoLabels().end(); ++it) {
  //   std::cout << *it << std::endl;
  // }

  if (jet.hasTagInfo(jetTagInfoSV_)) {
    std::cout << "Jet has SVInfo" << std::endl;
    const reco::SecondaryVertexTagInfo* svti =
      jet.tagInfoSecondaryVertex(jetTagInfoSV_);
    if (svti != 0) {
