...
ttHVarProcessor::ttHVarProcessor(BoostedRecoType recotype_, MiniAODHelper* helper_, TopTag::Mode topTaggerMode_, TopTag::SubjetAssign subjetAssign_,
                                 std::string topTaggerfilePath_, const boosted::SubjetType subjettype_, HiggsTag::Mode higgsTaggerMode_,
                                 std::string higgsTaggerFilePath_, std::string prefix_, bool doMEM_) :
    recotype(recotype_),
    prefix(prefix_),
    btagger("DeepJet"),
...
void ttHVarProcessor::FillHiggsCandidateVars(VariableContainer& vars, BoostedttHEvent& ttHEvent)
{
    ...
    if (fabs(mtoplep_higgs1 - 173) < fabs(mtoplep_higgs2 - 173) && fabs(mtoplep_higgs1 - 173) < fabs(mtoplep_higgs3 - 173) &&
        fabs(mtoplep_higgs1 - 173) < fabs(mtoplep_higgs4 - 173))
        vars.FillVar(prefix + "HiggsCandidate_M_TopLep", mtoplep_higgs1);
    else if (fabs(mtoplep_higgs2 - 173) < fabs(mtoplep_higgs1 - 173) && fabs(mtoplep_higgs2 - 173) < fabs(mtoplep_higgs3 - 173) &&
             fabs(mtoplep_higgs2 - 173) < fabs(mtoplep_higgs4 - 173))
        vars.FillVar(prefix + "HiggsCandidate_M_TopLep", mtoplep_higgs2);
    else if (fabs(mtoplep_higgs3 - 173) < fabs(mtoplep_higgs1 - 173) && fabs(mtoplep_higgs3 - 173) < fabs(mtoplep_higgs2 - 173) &&
             fabs(mtoplep_higgs3 - 173) < fabs(mtoplep_higgs4 - 173))
        vars.FillVar(prefix + "HiggsCandidate_M_TopLep", mtoplep_higgs3);
    else if (fabs(mtoplep_higgs4 - 173) < fabs(mtoplep_higgs1 - 173) && fabs(mtoplep_higgs4 - 173) < fabs(mtoplep_higgs2 - 173) &&
...
void ttHVarProcessor::FillTopHadCandidateVars(VariableContainer& vars, BoostedttHEvent& ttHEvent)
{
    ...
    if (topHadCandVec.Pt() > 0 && lepCandVec.Pt() > 0) { vars.FillVar(prefix + "TopHadCandidate_Dr_Lepton", BoostedUtils::DeltaR(lepCandVec, topHadCandVec)); }
...
void ttHVarProcessor::FillMCVars(VariableContainer& vars, BoostedttHEvent& ttHEvent, const InputCollections& input)
{
    ...
    if (higgsCandVec2.Pt() > 0) {
        if (higgs_mc.Pt() > 0) vars.FillVar(prefix + "HiggsCandidate_Dr_Higgs", BoostedUtils::DeltaR(higgs_mc, higgsCandVec2));
        ...
        if (tophad_mc.size() == 1) {
            ...
            vars.FillVar(prefix + "HiggsCandidate_Dr_B_TopHad",
                         fmin(BoostedUtils::DeltaR(bhad_mc[0], higgsCandVec2), BoostedUtils::DeltaR(bhad_mc[1], higgsCandVec2)));
            vars.FillVar(prefix + "HiggsCandidate_Dr_TopHad",
                         fmin(BoostedUtils::DeltaR(tophad_mc[0], higgsCandVec2), BoostedUtils::DeltaR(tophad_mc[1], higgsCandVec2)));
        }
        ...
        if (toplep_mc.size() == 1) {
            ...
            vars.FillVar(prefix + "HiggsCandidate_Dr_B_TopLep",
                         fmin(BoostedUtils::DeltaR(blep_mc[0], higgsCandVec2), BoostedUtils::DeltaR(blep_mc[1], higgsCandVec2)));
            vars.FillVar(prefix + "HiggsCandidate_Dr_TopLep",
                         fmin(BoostedUtils::DeltaR(toplep_mc[0], higgsCandVec2), BoostedUtils::DeltaR(toplep_mc[1], higgsCandVec2)));
        }
    }

    if (topHadCandVec.Pt() > 0) {
        if (higgs_mc.Pt() > 0) vars.FillVar(prefix + "TopHadCandidate_Dr_Higgs", BoostedUtils::DeltaR(higgs_mc, topHadCandVec));
        ...
        if (tophad_mc.size() == 1)
            ...
            vars.FillVar(prefix + "TopHadCandidate_Dr_TopHad",
                         fmin(BoostedUtils::DeltaR(tophad_mc[0], topHadCandVec), BoostedUtils::DeltaR(tophad_mc[1], topHadCandVec)));
        ...
        if (toplep_mc.size() == 1)
            ...
            vars.FillVar(prefix + "TopHadCandidate_Dr_TopLep",
                         fmin(BoostedUtils::DeltaR(toplep_mc[0], topHadCandVec), BoostedUtils::DeltaR(toplep_mc[1], topHadCandVec)));
    }

    if (topLepCandVec.Pt() > 0) {
        if (higgs_mc.Pt() > 0) vars.FillVar(prefix + "TopLepCandidate_Dr_Higgs", BoostedUtils::DeltaR(higgs_mc, topLepCandVec));
        ...
        if (tophad_mc.size() == 1)
            ...
            vars.FillVar(prefix + "TopLepCandidate_Dr_TopHad",
                         fmin(BoostedUtils::DeltaR(tophad_mc[0], topLepCandVec), BoostedUtils::DeltaR(tophad_mc[1], topLepCandVec)));
        ...
        if (toplep_mc.size() == 1)
            ...
            vars.FillVar(prefix + "TopLepCandidate_Dr_TopLep",
                         fmin(BoostedUtils::DeltaR(toplep_mc[0], topLepCandVec), BoostedUtils::DeltaR(toplep_mc[1], topLepCandVec)));
    }
