...
ttHVarProcessor::ttHVarProcessor(
    BoostedRecoType recotype_, MiniAODHelper* helper_,
    TopTag::Mode topTaggerMode_, TopTag::SubjetAssign subjetAssign_,
    std::string topTaggerfilePath_, const boosted::SubjetType subjettype_,
    HiggsTag::Mode higgsTaggerMode_, std::string higgsTaggerFilePath_,
    std::string prefix_, bool doMEM_)
    : recotype(recotype_),
      prefix(prefix_),
      btagger("DeepJet"),
...
void ttHVarProcessor::Init(const InputCollections& input,
...
void ttHVarProcessor::Process(const InputCollections& input,
                              VariableContainer& vars) {
  ...
  switch (recotype) {
    ...
    case BoostedRecoType::BoostedTopAk4HiggsFromAk4C: {
      ttHEvent.BoostedTopAk4HiggsFromAk4CEventRec(toptagger, subjettype,
                                                  higgstagger);
      break;
    }
    default: {
...
void ttHVarProcessor::FillHiggsCandidateVars(VariableContainer& vars,
  ...
  if (higgsB1Cand.pt() > 0) {
    ...
    vars.FillVar(prefix + "HiggsCandidate_B1_CSV",
                 CSVHelper::GetJetCSV(higgsB1Cand, btagger));
  }
  ...
  if (higgsB2Cand.pt() > 0) {
    ...
    vars.FillVar(prefix + "HiggsCandidate_B2_CSV",
                 CSVHelper::GetJetCSV(higgsB2Cand, btagger));
  }
  ...
  if (higgsGCand.pt() > 0) {
    ...
    vars.FillVar(prefix + "HiggsCandidate_G_CSV",
                 CSVHelper::GetJetCSV(higgsGCand, btagger));
  }
...
void ttHVarProcessor::FillTopHadCandidateVars(VariableContainer& vars,
                                              BoostedttHEvent& ttHEvent) {
  // Get Objects
  ...
  if (topHadBCand.pt() > 0) {
    ...
    vars.FillVar(prefix + "TopHadCandidate_B_CSV",
                 CSVHelper::GetJetCSV(topHadBCand, btagger));
  }
  ...
  if (topHadW1Cand.pt() > 0) {
    ...
    vars.FillVar(prefix + "TopHadCandidate_W1_CSV",
                 CSVHelper::GetJetCSV(topHadW1Cand, btagger));
  }
  ...
  if (topHadW2Cand.pt() > 0) {
    ...
    vars.FillVar(prefix + "TopHadCandidate_W2_CSV",
                 CSVHelper::GetJetCSV(topHadW2Cand, btagger));
  }
...
void ttHVarProcessor::FillTopLepCandidateVars(VariableContainer& vars,
                                              BoostedttHEvent& ttHEvent) {
  // Get Objects
  ...
  if (topLepBCand.pt() > 0.001) {
    ...
    vars.FillVar(prefix + "TopLepCandidate_B_CSV",
                 CSVHelper::GetJetCSV(topLepBCand, btagger));
  }
...
void ttHVarProcessor::FillAk5JetsVars(VariableContainer& vars,
                                      BoostedttHEvent& ttHEvent) {
  // Fill Variables
...
void ttHVarProcessor::FillCombinationVars(VariableContainer& vars,
                                          BoostedttHEvent& ttHEvent) {
  // Get Objects
  // Higgs Candidate
  math::XYZTLorentzVector higgsCandVec2 = ttHEvent.GetHiggsCandVec2();
...
void ttHVarProcessor::FillMCVars(VariableContainer& vars,
                                 BoostedttHEvent& ttHEvent,
  ...
  if (input.genTopEvt.IsFilled() && input.genTopEvt.TTxIsFilled()) {
    ...
    if (additional_bs.size() > 0) b1_mc = additional_bs[0].p4();
    if (additional_bs.size() > 1) b2_mc = additional_bs[1].p4();
  }
...
ttHEvent, const InputCollections& input){ if(!initialized) cerr << "tree
processor not initialized" << endl;
...
itJet++){ if (BoostedUtils::DeltaR(*itJet,topHadBCand)>0.3  &&
...
unmatchedJets.end(),BoostedUtils::FirstHasHigherCSV);
  ...
  if(doBoostedMEM){
    ...
    for(auto itJet=input.selectedJets.begin(); itJet!=input.selectedJets.end();
itJet++){ if(itJet-input.selectedJets.begin()==int(maxJets)) break;
