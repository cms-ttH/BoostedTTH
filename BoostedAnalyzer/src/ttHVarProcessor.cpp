...
ttHVarProcessor::ttHVarProcessor(BoostedRecoType recotype_,
                                 MiniAODHelper* helper_,
                                 TopTag::Mode topTaggerMode_,
                                 TopTag::SubjetAssign subjetAssign_,
                                 std::string topTaggerfilePath_,
                                 const boosted::SubjetType subjettype_,
                                 HiggsTag::Mode higgsTaggerMode_,
                                 std::string higgsTaggerFilePath_,
                                 std::string prefix_,
                                 bool doMEM_)
  : recotype(recotype_)
  , prefix(prefix_)
  , btagger("pfCombinedInclusiveSecondaryVertexV2BJetTags")
  , toptagger(topTaggerMode_, subjetAssign_, topTaggerfilePath_)
  , subjettype(subjettype_)
  , higgstagger(higgsTaggerMode_, higgsTaggerFilePath_)
  , doMEM(doMEM_)
{
}
...
void
ttHVarProcessor::Init(const InputCollections& input, VariableContainer& vars)
{
...
void
ttHVarProcessor::Process(const InputCollections& input, VariableContainer& vars)
{
  ...
  switch (recotype) {
    ...
    case BoostedRecoType::BoostedTopAk4HiggsFromAk4C: {
      ttHEvent.BoostedTopAk4HiggsFromAk4CEventRec(
        toptagger, subjettype, higgstagger);
      break;
    }
    default: {
...
void
ttHVarProcessor::InitHiggsCandidateVars(VariableContainer& vars)
{
...
void
ttHVarProcessor::InitTopHadCandidateVars(VariableContainer& vars)
{
...
void
ttHVarProcessor::InitTopLepCandidateVars(VariableContainer& vars)
{
...
void
ttHVarProcessor::InitAk5JetsVars(VariableContainer& vars)
{
...
void
ttHVarProcessor::InitCombinationVars(VariableContainer& vars)
{
...
void
ttHVarProcessor::InitMCVars(VariableContainer& vars)
{
...
void
ttHVarProcessor::FillHiggsCandidateVars(VariableContainer& vars,
                                        BoostedttHEvent& ttHEvent)
{
  ...
  if (ttHEvent.GetUseAk4Cluster()) {
    if (higgsCandFromAk4C.fatjet.Pt() > 0) {
      vars.FillVar(prefix + "HiggsCandidate_E",
                   higgsCandFromAk4C.fatjet.energy());
      vars.FillVar(prefix + "HiggsCandidate_Pt", higgsCandFromAk4C.fatjet.pt());
      vars.FillVar(prefix + "HiggsCandidate_M",
                   higgsCandFromAk4C.fatjet.mass());
      vars.FillVar(prefix + "HiggsCandidate_Eta",
                   higgsCandFromAk4C.fatjet.eta());
      vars.FillVar(prefix + "HiggsCandidate_Phi",
                   higgsCandFromAk4C.fatjet.phi());
    }

    if (higgsCandFromAk4C.fatjet.Pt() > 0 && lepCandVec.Pt() > 0)
      vars.FillVar(prefix + "HiggsCandidate_Dr_Lepton",
                   BoostedUtils::DeltaR(lepCandVec, higgsCandFromAk4C.fatjet));
  } else {
    if (higgsCand.fatjet.pt() > 0) {
      ...
      if (higgsCand.filterjets.size() > 0)
        vars.FillVar(prefix + "HiggsCandidate_Filterjet1_Pt",
                     higgsCand.filterjets[0].pt());
      if (higgsCand.filterjets.size() > 1)
        vars.FillVar(prefix + "HiggsCandidate_Filterjet2_Pt",
                     higgsCand.filterjets[1].pt());
    }

    if (higgsCand.fatjet.pt() > 0) {
      vars.FillVar(prefix + "HiggsCandidate_Subjetiness1",
                   higgsCand.tau1Filtered);
      vars.FillVar(prefix + "HiggsCandidate_Subjetiness2",
                   higgsCand.tau2Filtered);
      vars.FillVar(prefix + "HiggsCandidate_Subjetiness3",
                   higgsCand.tau3Filtered);
    }

    if (higgsCand.fatjet.pt() > 0 && lepCandVec.Pt() > 0)
      vars.FillVar(prefix + "HiggsCandidate_Dr_Lepton",
                   BoostedUtils::DeltaR(lepCandVec, higgsCand.fatjet.p4()));
  }
  if (higgsB1Cand.pt() > 0) {
    ...
    vars.FillVar(prefix + "HiggsCandidate_B1_CSV",
                 MiniAODHelper::GetJetCSV(higgsB1Cand, btagger));
  }
  ...
  if (higgsB2Cand.pt() > 0) {
    ...
    vars.FillVar(prefix + "HiggsCandidate_B2_CSV",
                 MiniAODHelper::GetJetCSV(higgsB2Cand, btagger));
  }
  ...
  if (higgsGCand.pt() > 0) {
    ...
    vars.FillVar(prefix + "HiggsCandidate_G_CSV",
                 MiniAODHelper::GetJetCSV(higgsGCand, btagger));
  }
  ...
  vars.FillVar(prefix + "HiggsCandidate_CosThetaStar_Filterjet12",
               BoostedUtils::CosThetaStar(higgsB1Cand.p4(), higgsB2Cand.p4()));
  vars.FillVar(prefix + "HiggsCandidate_CosThetaStar_Filterjet13",
               BoostedUtils::CosThetaStar(higgsB1Cand.p4(), higgsGCand.p4()));
  vars.FillVar(prefix + "HiggsCandidate_CosThetaStar_Filterjet23",
               BoostedUtils::CosThetaStar(higgsB2Cand.p4(), higgsGCand.p4()));
  ...
  if (higgsB2Cand.pt() > 0) {
    ...
    mblep = fmin((lepCandVec + higgsB1Cand.p4()).M(),
                 (lepCandVec + higgsB2Cand.p4()).M());
  }

  vars.FillVar(prefix + "HiggsCandidate_M_Lepton_B", mblep);
  if (fabs(mtoplep_higgs1 - 173) < fabs(mtoplep_higgs2 - 173) &&
      fabs(mtoplep_higgs1 - 173) < fabs(mtoplep_higgs3 - 173) &&
      fabs(mtoplep_higgs1 - 173) < fabs(mtoplep_higgs4 - 173))
    vars.FillVar(prefix + "HiggsCandidate_M_TopLep", mtoplep_higgs1);
  else if (fabs(mtoplep_higgs2 - 173) < fabs(mtoplep_higgs1 - 173) &&
           fabs(mtoplep_higgs2 - 173) < fabs(mtoplep_higgs3 - 173) &&
           fabs(mtoplep_higgs2 - 173) < fabs(mtoplep_higgs4 - 173))
    vars.FillVar(prefix + "HiggsCandidate_M_TopLep", mtoplep_higgs2);
  else if (fabs(mtoplep_higgs3 - 173) < fabs(mtoplep_higgs1 - 173) &&
           fabs(mtoplep_higgs3 - 173) < fabs(mtoplep_higgs2 - 173) &&
           fabs(mtoplep_higgs3 - 173) < fabs(mtoplep_higgs4 - 173))
    vars.FillVar(prefix + "HiggsCandidate_M_TopLep", mtoplep_higgs3);
  else if (fabs(mtoplep_higgs4 - 173) < fabs(mtoplep_higgs1 - 173) &&
           fabs(mtoplep_higgs4 - 173) < fabs(mtoplep_higgs2 - 173) &&
           fabs(mtoplep_higgs4 - 173) < fabs(mtoplep_higgs3 - 173))
    vars.FillVar(prefix + "HiggsCandidate_M_TopLep", mtoplep_higgs4);
}

void
ttHVarProcessor::FillTopHadCandidateVars(VariableContainer& vars,
                                         BoostedttHEvent& ttHEvent)
{
  ...
  vars.FillVar(prefix + "TopHadCandidate_TopMVAOutput",
               ttHEvent.GetTopHadCandTag());
  ...
  if (topHadBCand.pt() > 0) {
    ...
    vars.FillVar(prefix + "TopHadCandidate_B_CSV",
                 MiniAODHelper::GetJetCSV(topHadBCand, btagger));
  }
  ...
  if (topHadW1Cand.pt() > 0) {
    ...
    vars.FillVar(prefix + "TopHadCandidate_W1_CSV",
                 MiniAODHelper::GetJetCSV(topHadW1Cand, btagger));
  }
  ...
  if (topHadW2Cand.pt() > 0) {
    ...
    vars.FillVar(prefix + "TopHadCandidate_W2_CSV",
                 MiniAODHelper::GetJetCSV(topHadW2Cand, btagger));
  }
  ...
  vars.FillVar(prefix + "TopHadCandidate_UnfilteredMass",
               topHadCand.unfilteredMass);
  ...
  vars.FillVar(prefix + "TopHadCandidate_MassRatioPassed",
               topHadCand.massRatioPassed);

  vars.FillVar(prefix + "TopHadCandidate_Ropt", topHadCand.Ropt);
  vars.FillVar(prefix + "TopHadCandidate_RoptCalc", topHadCand.RoptCalc);
  vars.FillVar(prefix + "TopHadCandidate_PtForRoptCalc",
               topHadCand.ptForRoptCalc);

  vars.FillVar(prefix + "TopHadCandidate_Tau1Unfiltered",
               topHadCand.tau1Unfiltered);
  vars.FillVar(prefix + "TopHadCandidate_Tau2Unfiltered",
               topHadCand.tau2Unfiltered);
  vars.FillVar(prefix + "TopHadCandidate_Tau3Unfiltered",
               topHadCand.tau3Unfiltered);
  vars.FillVar(prefix + "TopHadCandidate_Tau1Filtered",
               topHadCand.tau1Filtered);
  vars.FillVar(prefix + "TopHadCandidate_Tau2Filtered",
               topHadCand.tau2Filtered);
  vars.FillVar(prefix + "TopHadCandidate_Tau3Filtered",
               topHadCand.tau3Filtered);
  ...
  if (topHadCandVec.Pt() > 0 && lepCandVec.Pt() > 0) {
    vars.FillVar(prefix + "TopHadCandidate_Dr_Lepton",
                 BoostedUtils::DeltaR(lepCandVec, topHadCandVec));
  }
  ...
  if (topHadBCand.pt() > 0.001) {
    vars.FillVar(prefix + "TopHadCandidate_M_Lepton_B",
                 (lepCandVec + topHadBCand.p4()).M());
    vars.FillVar(prefix + "TopHadCandidate_M_TopLep",
                 fabs(mtoplep_tophad1 - 173) < fabs(mtoplep_tophad2 - 173)
                   ? mtoplep_tophad1
                   : mtoplep_tophad2);
  }
}
...
void
ttHVarProcessor::FillTopLepCandidateVars(VariableContainer& vars,
                                         BoostedttHEvent& ttHEvent)
{
  ...
  if (topLepBCand.pt() > 0.001) {
    ...
    vars.FillVar(prefix + "TopLepCandidate_B_CSV",
                 MiniAODHelper::GetJetCSV(topLepBCand, btagger));
  }
...
void
ttHVarProcessor::FillAk5JetsVars(VariableContainer& vars,
                                 BoostedttHEvent& ttHEvent)
{
  ...
  vars.FillVar(prefix + "N_BTagsL_Higgs_ak5Jets",
               ttHEvent.GetNHiggsCandBTagL());
  vars.FillVar(prefix + "N_BTagsM_Higgs_ak5Jets",
               ttHEvent.GetNHiggsCandBTagM());
  vars.FillVar(prefix + "N_BTagsT_Higgs_ak5Jets",
               ttHEvent.GetNHiggsCandBTagT());
  vars.FillVar(prefix + "AvgCSV_Higgs_ak5Jets",
               ttHEvent.GetAverageCSVHiggsCand());

  vars.FillVar(prefix + "N_Jets_TopHad_ak5Jets", ttHEvent.GetNTopHadak5Jets());
  vars.FillVar(prefix + "N_BTagsL_TopHad_ak5Jets",
               ttHEvent.GetNTopHadCandBTagL());
  vars.FillVar(prefix + "N_BTagsM_TopHad_ak5Jets",
               ttHEvent.GetNTopHadCandBTagM());
  vars.FillVar(prefix + "N_BTagsT_TopHad_ak5Jets",
               ttHEvent.GetNTopHadCandBTagT());
  vars.FillVar(prefix + "AvgCSV_TopHad_ak5Jets",
               ttHEvent.GetAverageCSVTopHadCand());

  vars.FillVar(prefix + "N_Jets_TopLep_ak5Jets", ttHEvent.GetNTopLepak5Jets());
  vars.FillVar(prefix + "N_BTagsL_TopLep_ak5Jets",
               ttHEvent.GetNTopLepCandBTagL());
  vars.FillVar(prefix + "N_BTagsM_TopLep_ak5Jets",
               ttHEvent.GetNTopLepCandBTagM());
  vars.FillVar(prefix + "N_BTagsT_TopLep_ak5Jets",
               ttHEvent.GetNTopLepCandBTagT());
  vars.FillVar(prefix + "AvgCSV_TopLep_ak5Jets",
               ttHEvent.GetAverageCSVTopLepCand());
...
void
ttHVarProcessor::FillCombinationVars(VariableContainer& vars,
                                     BoostedttHEvent& ttHEvent)
{
  ...
  if (higgsCandVec2.Pt() > 0 && topHadCandVec.Pt() > 0) {
    vars.FillVar(prefix + "HiggsCandidate_Dr_TopHadCandidate",
                 BoostedUtils::DeltaR(topHadCandVec, higgsCandVec2));
    vars.FillVar(prefix + "HiggsCandidate_Dphi_TopHadCandidate",
                 BoostedUtils::DeltaPhi(topHadCandVec, higgsCandVec2));
    vars.FillVar(prefix + "HiggsCandidate_Deta_TopHadCandidate",
                 BoostedUtils::DeltaEta(topHadCandVec, higgsCandVec2));
  }

  if (higgsCandVec2.Pt() > 0 && topLepCandVec.Pt() > 0) {
    vars.FillVar(prefix + "HiggsCandidate_Dr_TopLepCandidate",
                 BoostedUtils::DeltaR(topLepCandVec, higgsCandVec2));
    vars.FillVar(prefix + "HiggsCandidate_Dphi_TopLepCandidate",
                 BoostedUtils::DeltaPhi(topLepCandVec, higgsCandVec2));
    vars.FillVar(prefix + "HiggsCandidate_Deta_TopLepCandidate",
                 BoostedUtils::DeltaEta(topLepCandVec, higgsCandVec2));
  }

  if (topHadCandVec.Pt() > 0 && topLepCandVec.Pt() > 0) {
    vars.FillVar(prefix + "TopHadCandidate_Dr_TopLepCandidate",
                 BoostedUtils::DeltaR(topLepCandVec, topHadCandVec));
    vars.FillVar(prefix + "TopHadCandidate_Dphi_TopLepCandidate",
                 BoostedUtils::DeltaPhi(topLepCandVec, topHadCandVec));
    vars.FillVar(prefix + "TopHadCandidate_Deta_TopLepCandidate",
                 BoostedUtils::DeltaEta(topLepCandVec, topHadCandVec));
  }
...
void
ttHVarProcessor::FillMCVars(VariableContainer& vars,
                            BoostedttHEvent& ttHEvent,
                            const InputCollections& input)
{
  ...
  if (input.genTopEvt.IsFilled() && input.genTopEvt.TTxIsFilled()) {
    std::vector<reco::GenParticle> additional_bs =
      input.genTopEvt.GetAdditionalBHadrons();
  ...
  if (higgsCandVec2.Pt() > 0) {
    if (higgs_mc.Pt() > 0)
      vars.FillVar(prefix + "HiggsCandidate_Dr_Higgs",
                   BoostedUtils::DeltaR(higgs_mc, higgsCandVec2));

    if (tophad_mc.size() == 1) {
      vars.FillVar(prefix + "HiggsCandidate_Dr_B_TopHad",
                   BoostedUtils::DeltaR(bhad_mc[0], higgsCandVec2));
      vars.FillVar(prefix + "HiggsCandidate_Dr_TopHad",
                   BoostedUtils::DeltaR(tophad_mc[0], higgsCandVec2));
    } else if (tophad_mc.size() == 2) {
      vars.FillVar(prefix + "HiggsCandidate_Dr_B_TopHad",
                   fmin(BoostedUtils::DeltaR(bhad_mc[0], higgsCandVec2),
                        BoostedUtils::DeltaR(bhad_mc[1], higgsCandVec2)));
      vars.FillVar(prefix + "HiggsCandidate_Dr_TopHad",
                   fmin(BoostedUtils::DeltaR(tophad_mc[0], higgsCandVec2),
                        BoostedUtils::DeltaR(tophad_mc[1], higgsCandVec2)));
    }

    if (toplep_mc.size() == 1) {
      vars.FillVar(prefix + "HiggsCandidate_Dr_B_TopLep",
                   BoostedUtils::DeltaR(blep_mc[0], higgsCandVec2));
      vars.FillVar(prefix + "HiggsCandidate_Dr_TopLep",
                   BoostedUtils::DeltaR(toplep_mc[0], higgsCandVec2));
    } else if (toplep_mc.size() == 2) {
      vars.FillVar(prefix + "HiggsCandidate_Dr_B_TopLep",
                   fmin(BoostedUtils::DeltaR(blep_mc[0], higgsCandVec2),
                        BoostedUtils::DeltaR(blep_mc[1], higgsCandVec2)));
      vars.FillVar(prefix + "HiggsCandidate_Dr_TopLep",
                   fmin(BoostedUtils::DeltaR(toplep_mc[0], higgsCandVec2),
                        BoostedUtils::DeltaR(toplep_mc[1], higgsCandVec2)));
    }
  }

  if (topHadCandVec.Pt() > 0) {
    if (higgs_mc.Pt() > 0)
      vars.FillVar(prefix + "TopHadCandidate_Dr_Higgs",
                   BoostedUtils::DeltaR(higgs_mc, topHadCandVec));

    if (tophad_mc.size() == 1)
      vars.FillVar(prefix + "TopHadCandidate_Dr_TopHad",
                   BoostedUtils::DeltaR(tophad_mc[0], topHadCandVec));
    else if (tophad_mc.size() == 2)
      vars.FillVar(prefix + "TopHadCandidate_Dr_TopHad",
                   fmin(BoostedUtils::DeltaR(tophad_mc[0], topHadCandVec),
                        BoostedUtils::DeltaR(tophad_mc[1], topHadCandVec)));

    if (toplep_mc.size() == 1)
      vars.FillVar(prefix + "TopHadCandidate_Dr_TopLep",
                   BoostedUtils::DeltaR(toplep_mc[0], topHadCandVec));
    else if (toplep_mc.size() == 2)
      vars.FillVar(prefix + "TopHadCandidate_Dr_TopLep",
                   fmin(BoostedUtils::DeltaR(toplep_mc[0], topHadCandVec),
                        BoostedUtils::DeltaR(toplep_mc[1], topHadCandVec)));
  }

  if (topLepCandVec.Pt() > 0) {
    if (higgs_mc.Pt() > 0)
      vars.FillVar(prefix + "TopLepCandidate_Dr_Higgs",
                   BoostedUtils::DeltaR(higgs_mc, topLepCandVec));

    if (tophad_mc.size() == 1)
      vars.FillVar(prefix + "TopLepCandidate_Dr_TopHad",
                   BoostedUtils::DeltaR(tophad_mc[0], topLepCandVec));
    else if (tophad_mc.size() == 2)
      vars.FillVar(prefix + "TopLepCandidate_Dr_TopHad",
                   fmin(BoostedUtils::DeltaR(tophad_mc[0], topLepCandVec),
                        BoostedUtils::DeltaR(tophad_mc[1], topLepCandVec)));

    if (toplep_mc.size() == 1)
      vars.FillVar(prefix + "TopLepCandidate_Dr_TopLep",
                   BoostedUtils::DeltaR(toplep_mc[0], topLepCandVec));
    else if (toplep_mc.size() == 2)
      vars.FillVar(prefix + "TopLepCandidate_Dr_TopLep",
                   fmin(BoostedUtils::DeltaR(toplep_mc[0], topLepCandVec),
                        BoostedUtils::DeltaR(toplep_mc[1], topLepCandVec)));
  }
  ...
  if (tophad_mc.size() > 1 && topHadCandVec.Pt() > 0) {
    if (BoostedUtils::DeltaR(tophad_mc[1], topHadCandVec) <
        BoostedUtils::DeltaR(tophad_mc[0], topHadCandVec))
      topHadMCID = 1;
  }
  ...
  if (topHadMCID >= 0 && topHadBCand.pt() > 0)
    vars.FillVar(prefix + "Dr_Bhad",
                 BoostedUtils::DeltaR(bhad_mc[topHadMCID], topHadBCand.p4()));

  float drb11 = 9;
  float drb22 = 9;
  float drb21 = 9;
  float drb12 = 9;
  if (b1_mc.pt() > 0 && b2_mc.pt() > 0 && higgsB1Cand.pt() > 0 &&
      higgsB2Cand.pt() > 0) {
  ...
  if (toplep_mc.size() > 1 && topLepCandVec.Pt() > 0.001) {
    if (BoostedUtils::DeltaR(toplep_mc[1], topLepCandVec) <
        BoostedUtils::DeltaR(toplep_mc[0], topLepCandVec))
      topLepMCID = 1;
  }
  ...
  if (toplep_mc.size() > 0) {
    if (topLepBCand.pt() > 0)
      vars.FillVar(prefix + "Dr_Blep",
                   BoostedUtils::DeltaR(blep_mc[topLepMCID], topLepBCand.p4()));
    if (nuCandVec.Pt() > 0)
      vars.FillVar(prefix + "Dr_Nu",
                   BoostedUtils::DeltaR(nu_mc[topLepMCID], nuCandVec));
    if (lepCandVec.Pt() > 0)
      vars.FillVar(prefix + "Dr_Lep",
                   BoostedUtils::DeltaR(lep_mc[topLepMCID], lepCandVec));
  }
}

/*
void ttHVarProcessor::FillMEMVars(VariableContainer& vars, BoostedttHEvent&
ttHEvent, const InputCollections& input){
  ...
  for(auto itJet=input.selectedJets.begin(); itJet!=input.selectedJets.end();
itJet++){
  ...
  std::sort(unmatchedJets.begin(),
unmatchedJets.end(),BoostedUtils::FirstHasHigherCSV);
...
itJet++){
  ...
  result = mem.GetOutput(hypo, lepvecs, lepcharges, jetvecs, jetcsvs, jettype,
jetvecs, jetcsvs, metP4);
