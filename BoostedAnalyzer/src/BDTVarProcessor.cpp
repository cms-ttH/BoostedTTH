...
BDTVarProcessor::BDTVarProcessor()
    :  //     bdtohio2(BDTOhio_v2(BoostedUtils::GetAnalyzerPath()+"/data/bdtweights/ohio_weights_run2_v2/")),
      ...
      commonBDT5(new BDTClassifier(
          string(getenv("CMSSW_BASE")) +
          "/src/TTH/CommonClassifier/data/lj_BDT_DeepCSV_Summer18_v1/")),
...
BDTVarProcessor::BDTVarProcessor(BDTClassifier* bdt_)
    : commonBDT5(bdt_), recoLikelihoodVariables(new RecoLikelihoodVariables()) {
  needToDeleteBDTClassifier = false;
}

void BDTVarProcessor::Init(const InputCollections& input,
  ...
  map<string, float> bdtinputs_common5 =
      commonBDT5->GetVariablesOfLastEvaluation();
  for (auto it = bdtinputs_common5.begin(); it != bdtinputs_common5.end();
       it++) {
    vars.InitVar("BDT_common5_input_" + it->first);
  }

  //   map<TString,double> DNN_inputs =
  //   recoLikelihoodVariables->GetRecoLikelihoodVariables(); for(auto
  //   it=DNN_inputs.begin(); it!=DNN_inputs.end(); it++){
...
void BDTVarProcessor::Process(const InputCollections& input,
                              VariableContainer& vars) {
  if (!initialized) cerr << "tree processor not initialized" << endl;
  if (input.selectedMuons.size() + input.selectedElectrons.size() != 1) return;
  //   float
  //   bdtoutput2=bdtohio2.Evaluate(input.selectedMuons,input.selectedElectrons,
  //   input.selectedJets, input.selectedJetsLoose, input.correctedMET);
  ...
  //
  //   float
  //   bdtoutput3=bdt3.Evaluate(input.selectedMuons,input.selectedElectrons,
  //   input.selectedJets, input.selectedJetsLoose, input.correctedMET);
  ...
  vector<TLorentzVector> lepvecs = BoostedUtils::GetTLorentzVectors(
  ...
  vector<TLorentzVector> loose_jetvecs = BoostedUtils::GetTLorentzVectors(
  ...
  float bdtoutput_common5 =
      commonBDT5->GetBDTOutput(lepvecs, jetvecs, jetcsvs, metP4);
  ...
  //   recoLikelihoodVariables->CalculateRecoLikelihoodVariables(lepvecs,
  //   jetvecs, jetcsvs, metP4); map<TString,double> DNN_inputs =
  //   recoLikelihoodVariables->GetRecoLikelihoodVariables(); for(auto
  //   it=DNN_inputs.begin(); it!=DNN_inputs.end(); it++){
