...
void
BDTVarProcessor::Init(const InputCollections& input, VariableContainer& vars)
{
  ...
  //   recoLikelihoodVariables->GetRecoLikelihoodVariables();
...
void
BDTVarProcessor::Process(const InputCollections& input, VariableContainer& vars)
{
  if (!initialized)
    cerr << "tree processor not initialized" << endl;
  if (input.selectedMuons.size() + input.selectedElectrons.size() != 1)
    return;
  //   float
