...
BDTVarProcessor::~BDTVarProcessor()
{
    if (needToDeleteBDTClassifier) { delete commonBDT5; }
    delete recoLikelihoodVariables;
}

BDTVarProcessor::BDTVarProcessor(BDTClassifier* bdt_) : commonBDT5(bdt_), recoLikelihoodVariables(new RecoLikelihoodVariables())
{
...
void BDTVarProcessor::Init(const InputCollections& input, VariableContainer& vars)
{
    ...
    for (auto it = bdtinputs_common5.begin(); it != bdtinputs_common5.end(); it++) { vars.InitVar("BDT_common5_input_" + it->first); }
...
void BDTVarProcessor::Process(const InputCollections& input, VariableContainer& vars)
{
    ...
    for (auto j = input.selectedJets.begin(); j != input.selectedJets.end(); j++) { jetcsvs.push_back(CSVHelper::GetJetCSV(*j, "DeepJet")); }
    for (auto j = input.selectedJetsLoose.begin(); j != input.selectedJetsLoose.end(); j++) { loose_jetcsvs.push_back(CSVHelper::GetJetCSV(*j, "DeepJet")); }
    ...
    if (commonBDT5->GetCategoryOfLastEvaluation() != "none") {
        map< string, float > bdtinputs_common5 = commonBDT5->GetVariablesOfLastEvaluation();
        for (auto it = bdtinputs_common5.begin(); it != bdtinputs_common5.end(); it++) { vars.FillVar("BDT_common5_input_" + it->first, it->second); }
    }
