...
void AK8JetProcessor::Process(const InputCollections& input, VariableContainer& vars)
{
    ...
    for (size_t i = 0; i < input.selectedJetsAK8.size(); i++) {
        ...
        if (minDr_TopLep < 999) { vars.FillVars("AK8Jet_Dr_GenTopLep", i, minDr_TopLep); }
    }
    ...
    for (size_t i = 0; i < input.selectedJetsAK8.size(); i++) {
        if (higgs.pt() > 0.) { vars.FillVars("AK8Jet_Dr_GenHiggs", i, BoostedUtils::DeltaR(higgs.p4(), input.selectedJetsAK8[i].p4())); }
