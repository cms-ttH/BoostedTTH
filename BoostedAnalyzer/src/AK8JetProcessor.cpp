...
void AK8JetProcessor::Init(const InputCollections& input,
...
void AK8JetProcessor::Process(const InputCollections& input,
                              VariableContainer& vars) {
  ...
  for (std::vector<pat::Jet>::const_iterator itJet =
           input.selectedJetsAK8.begin();
       itJet != input.selectedJetsAK8.end(); ++itJet) {
    ...
    vars.FillVars("AK8Jet_DoubleCSV", iJet,
                  CSVHelper::GetJetCSV(*itJet, btagger));
    vars.FillVars("AK8Jet_CSV_DNN", iJet,
                  CSVHelper::GetJetCSV_DNN(*itJet, btagger));
    ...
    */
    ...
    if (sdSubjetsPuppi.size() > 0) {
      vars.FillVars("AK8Jet_Puppi_Softdrop_Mass", iJet,
                    itJet->userFloat("ak8PFJetsPuppiSoftDropMass"));
      vars.FillVars(
          "AK8Jet_CHS_Pruned_Mass", iJet,
          itJet->userFloat("ak8PFJetsCHSValueMap:ak8PFJetsCHSPrunedMass"));
      for (auto& it : sdSubjetsPuppi) {
        ...
        if (i == 1) {
          ...
          vars.FillVars("AK8Subjet1_DeepJet", iJet,
                        CSVHelper::GetJetCSV(*it, btagger));
        }
        if (i == 2) {
          ...
          vars.FillVars("AK8Subjet2_DeepJet", iJet,
                        CSVHelper::GetJetCSV(*it, btagger));
        }
      }
    }
  ...
  for (size_t i = 0; i < input.selectedJetsAK8.size(); i++) {
    ...
    for (size_t j = 0; j < toplep.size(); j++) {
      float Dr_temp =
          BoostedUtils::DeltaR(toplep[j].p4(), input.selectedJetsAK8[i].p4());
  ...
  for (size_t i = 0; i < input.selectedJetsAK8.size(); i++) {
    ...
    for (size_t j = 0; j < tophad.size(); j++) {
      float Dr_temp =
          BoostedUtils::DeltaR(tophad[j].p4(), input.selectedJetsAK8[i].p4());
      if (Dr_temp < minDr_TopHad) {
        minDr_TopHad = Dr_temp;
        minDr_WHad =
            BoostedUtils::DeltaR(whad[j].p4(), input.selectedJetsAK8[i].p4());
        minDr_TopB =
            BoostedUtils::DeltaR(bhad[j].p4(), input.selectedJetsAK8[i].p4());
        minDr_TopQ1 =
            BoostedUtils::DeltaR(q1[j].p4(), input.selectedJetsAK8[i].p4());
        minDr_TopQ2 =
            BoostedUtils::DeltaR(q2[j].p4(), input.selectedJetsAK8[i].p4());
      }
    }

    if (minDr_TopHad < 999)
      vars.FillVars("AK8Jet_Dr_GenTopHad", i, minDr_TopHad);
  ...
  for (size_t i = 0; i < input.selectedJetsAK8.size(); i++) {
    if (higgs.pt() > 0.) {
      vars.FillVars(
          "AK8Jet_Dr_GenHiggs", i,
          BoostedUtils::DeltaR(higgs.p4(), input.selectedJetsAK8[i].p4()));
    }
    if (b1.pt() > 0.)
      vars.FillVars(
          "AK8Jet_Dr_GenB1", i,
          BoostedUtils::DeltaR(b1.p4(), input.selectedJetsAK8[i].p4()));
    if (b2.pt() > 0.)
      vars.FillVars(
          "AK8Jet_Dr_GenB2", i,
          BoostedUtils::DeltaR(b2.p4(), input.selectedJetsAK8[i].p4()));
  }
}
