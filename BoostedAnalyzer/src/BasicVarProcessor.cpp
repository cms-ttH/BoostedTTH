...
void BasicVarProcessor::Init(const InputCollections& input,
...
void BasicVarProcessor::Process(const InputCollections& input,
                                VariableContainer& vars) {
  ...
  for (std::vector<pat::Jet>::const_iterator itJet = input.selectedJets.begin();
    ...
    vars.FillVars("Jet_PileUpID", iJet,
                  itJet->userFloat("pileupJetId:fullDiscriminant"));
  ...
  for (std::vector<math::XYZTLorentzVector>::iterator itJetVec1 =
    ...
    for (std::vector<math::XYZTLorentzVector>::iterator itJetVec2 =
      ...
      for (std::vector<math::XYZTLorentzVector>::iterator itJetVec3 =
               itJetVec2 + 1;
           itJetVec3 != jetvecs.end(); ++itJetVec3) {
  ...
  for (std::vector<pat::Jet>::iterator itJetUntagged1 =
       ...
       itJetUntagged1 != selectedUntaggedJets.end(); ++itJetUntagged1) {
    for (std::vector<pat::Jet>::iterator itJetUntagged2 = itJetUntagged1 + 1;
         itJetUntagged2 != selectedUntaggedJets.end(); ++itJetUntagged2) {
      for (std::vector<pat::Jet>::iterator itJetTagged =
               selectedTaggedJets.begin();
           itJetTagged != selectedTaggedJets.end(); ++itJetTagged) {
  ...
  if (input.selectedElectrons.size() > 0 || input.selectedMuons.size() > 0) {
    mtw = sqrt(
        2 *
        (primLepVec.Pt() * input.correctedMET.corP4(pat::MET::Type1XY).pt() -
         primLepVec.Px() * input.correctedMET.corP4(pat::MET::Type1XY).px() -
         primLepVec.Py() * input.correctedMET.corP4(pat::MET::Type1XY).py()));
  }
