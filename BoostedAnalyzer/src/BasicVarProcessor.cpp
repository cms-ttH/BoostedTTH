...
void
BasicVarProcessor::Init(const InputCollections& input, VariableContainer& vars)
{
...
void
BasicVarProcessor::Process(const InputCollections& input,
                           VariableContainer& vars)
{
  ...
  for (std::vector<pat::Jet>::const_iterator itJet = input.selectedJets.begin();
    ...
    vars.FillVars(
      "Jet_PileUpID", iJet, itJet->userFloat("pileupJetId:fullDiscriminant"));
  ...
  for (std::vector<pat::Jet>::const_iterator itJet =
    ...
    vars.FillVars("LooseJet_PileUpID",
                  iJet,
  ...
  if (input.selectedElectrons.size() > 0 || input.selectedMuons.size() > 0) {
    mtw = sqrt(
      2 * (primLepVec.Pt() * input.correctedMET.corP4(pat::MET::Type1XY).pt() -
