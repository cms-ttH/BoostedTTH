...
void StdTopVarProcessor::Init(const InputCollections& input,
...
void StdTopVarProcessor::Process(const InputCollections& input,
                                 VariableContainer& vars) {
  ...
  for (std::vector<pat::Jet>::const_iterator it1Jet =
    ...
    for (std::vector<pat::Jet>::const_iterator it2Jet =
      ...
      if (abs(80.4 - (it1Jet->p4() + it2Jet->p4()).M()) <
  ...
  for (std::vector<pat::Jet>::const_iterator itJet = selectedTaggedJets.begin();
    ...
    if (HadTop.M() == 0 ||
      ...
      cout << "HadTop_W_Pt: " << HadW.Pt() << ", HadTop_B_Pt: " << itJet->pt()
           << ", Top mass: " << (HadW + itJet->p4()).M() << endl;
    }
  }
  ...
  //   vars.FillVar( "HadTop_B_CSV",CSVHelper::GetJetCSV(HadB,btagger) );
  ...
  if (abs(80.4 - (nu1 + PrimLep).M()) < abs(80.4 - (nu2 + PrimLep).M()))
    LepW = nu1 + PrimLep;
  else
    LepW = nu2 + PrimLep;
  ...
  //   vars.FillVar( "LepTop_B_CSV",CSVHelper::GetJetCSV(LepB,btagger) );
