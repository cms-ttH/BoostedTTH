...
void
GenWeights::GetGenWeights(map<string, float>& weights,
...
bool
GenWeights::GetLHAPDFWeight(map<string, float>& weights,
                            const GenEventInfoProduct& genInfos)
{
  ...
  for (size_t p = 0; p < initializedPDFSets.size(); p++) {
    ...
    for (size_t j = 0; j < PDFs.size(); ++j) {
      double xpdf1 = PDFs[j]->xfxQ(
        pdfInfos->id.first, pdfInfos->x.first, pdfInfos->scalePDF);
      double xpdf2 = PDFs[j]->xfxQ(
...
bool
GenWeights::initLHAPDF(string name)
{
...
bool
GenWeights::initLHAPDF(vector<string> names)
{
  // initialize the LHAPDFs
  cout << "Initializing additional PDFs for reweighting:" << endl;
  for (auto name : names) {
...
void
GenWeights::GetNamesFromLHE(const LHERunInfoProduct& myLHERunInfoProduct)
{
