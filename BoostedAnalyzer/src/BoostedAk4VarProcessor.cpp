...
void BoostedAk4VarProcessor::Process(const InputCollections& input,VariableContainer& vars){
  ...
  for(int unsigned i = 0; i< input.selectedAk4Cluster.size(); ++i){
    ...
    for(std::vector<pat::Jet>::const_iterator itAk4Jet = input.selectedAk4Cluster[i].ak4jets.begin(); itAk4Jet != input.selectedAk4Cluster[i].ak4jets.end() && itAk4Jet-input.selectedAk4Cluster[i].ak4jets.begin()<5; ++itAk4Jet){
