...
void BoostedAk4VarProcessor::Init(const InputCollections& input, VariableContainer& vars)
{
    ...
    vars.InitVars("BoostedAk4Cluster_Dr_Ak4Cluster_BoostedJet1", -9, "N_BoostedAk4Cluster");
    vars.InitVars("BoostedAk4Cluster_Dr_Ak4Cluster_BoostedJet2", -9, "N_BoostedAk4Cluster");
    vars.InitVars("BoostedAk4Cluster_Dr_Ak4Cluster_BoostedJet3", -9, "N_BoostedAk4Cluster");
...
void BoostedAk4VarProcessor::Process(const InputCollections& input, VariableContainer& vars)
{
    ...
    for (int unsigned i = 0; i < input.selectedAk4Cluster.size(); ++i) {
        ...
        vars.FillVars("BoostedAk4Cluster_N_Ak4Jets", i, input.selectedAk4Cluster[i].ak4jets.size());

        for (std::vector< pat::Jet >::const_iterator itAk4Jet = input.selectedAk4Cluster[i].ak4jets.begin();
             itAk4Jet != input.selectedAk4Cluster[i].ak4jets.end() && itAk4Jet - input.selectedAk4Cluster[i].ak4jets.begin() < 5; ++itAk4Jet) {
            string iAk4JetNumber = std::to_string(itAk4Jet - input.selectedAk4Cluster[i].ak4jets.begin() + 1);
            vars.FillVars("BoostedAk4Cluster_Ak4Jet" + iAk4JetNumber + "_E", i, itAk4Jet->energy());
            vars.FillVars("BoostedAk4Cluster_Ak4Jet" + iAk4JetNumber + "_M", i, itAk4Jet->mass());
            vars.FillVars("BoostedAk4Cluster_Ak4Jet" + iAk4JetNumber + "_Pt", i, itAk4Jet->pt());
            vars.FillVars("BoostedAk4Cluster_Ak4Jet" + iAk4JetNumber + "_Phi", i, itAk4Jet->phi());
            vars.FillVars("BoostedAk4Cluster_Ak4Jet" + iAk4JetNumber + "_Eta", i, itAk4Jet->eta());
        }

        if (input.selectedBoostedJets.size() > 0)
            vars.FillVars("BoostedAk4Cluster_Dr_Ak4Cluster_BoostedJet1", i,
                          BoostedUtils::DeltaR(input.selectedAk4Cluster[i].fatjet, input.selectedBoostedJets[0].fatjet.p4()));
        if (input.selectedBoostedJets.size() > 1)
            vars.FillVars("BoostedAk4Cluster_Dr_Ak4Cluster_BoostedJet2", i,
                          BoostedUtils::DeltaR(input.selectedAk4Cluster[i].fatjet, input.selectedBoostedJets[1].fatjet.p4()));
        if (input.selectedBoostedJets.size() > 2)
            vars.FillVars("BoostedAk4Cluster_Dr_Ak4Cluster_BoostedJet3", i,
                          BoostedUtils::DeltaR(input.selectedAk4Cluster[i].fatjet, input.selectedBoostedJets[2].fatjet.p4()));
    }
}
