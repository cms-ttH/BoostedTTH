...
void SlimmedNtuples::Init(const InputCollections& input,
                          VariableContainer& vars) {
  cout << "this method should not be called -> something is wrong" << endl;
}
void SlimmedNtuples::Process(const InputCollections& input,
                             VariableContainer& vars) {
  cout << "this method should not be called -> something is wrong" << endl;
}
...
void SlimmedNtuples::Init(const std::vector<InputCollections>& input,
                          VariableContainer& vars) {
...
void SlimmedNtuples::Process(const std::vector<InputCollections>& input,
                             VariableContainer& vars) {
  ...
  vars.FillIntVar("nleps", int(input_nom.selectedElectrons.size() +
                               input_nom.selectedMuons.size()));

  // fill lepton stuff in the tree
  for (std::vector<pat::Electron>::const_iterator itLep =
           input_nom.selectedElectrons.begin();
       itLep != input_nom.selectedElectrons.end(); ++itLep) {
  ...
  for (std::vector<pat::Muon>::const_iterator itLep =
       ...
       itLep != input_nom.selectedMuons.end(); ++itLep) {
