...
void
SlimmedNtuples::Init(const InputCollections& input, VariableContainer& vars)
{
  cout << "this method should not be called -> something is wrong" << endl;
}
void
SlimmedNtuples::Process(const InputCollections& input, VariableContainer& vars)
{
  cout << "this method should not be called -> something is wrong" << endl;
}
...
template<typename T>
T
SlimmedNtuples::GetSortedBySeed(const T& collection)
{
  T result = collection;
  std::sort(result.begin(),
            result.end(),
            [](typename T::value_type a, typename T::value_type b) {
              return ptr(a)->userInt("deterministicSeed") >
                     ptr(b)->userInt("deterministicSeed");
            });
  return result;
}

void
SlimmedNtuples::Init(const std::vector<InputCollections>& input,
...
void
SlimmedNtuples::Process(const std::vector<InputCollections>& input,
                        VariableContainer& vars)
{
  if (!initialized)
    cerr << "slimmed ntuples not initialized" << endl;
  ...
  vars.FillIntVar(
    "nleps",
    int(input_nom.selectedElectrons.size() + input_nom.selectedMuons.size()));

  // fill lepton stuff in the tree
  for (std::vector<pat::Electron>::const_iterator itLep =
         input_nom.selectedElectrons.begin();
       itLep != input_nom.selectedElectrons.end();
       ++itLep) {
  ...
  for (std::vector<pat::Muon>::const_iterator itLep =
         input_nom.selectedMuons.begin();
       itLep != input_nom.selectedMuons.end();
       ++itLep) {
  ...
  for (std::vector<pat::Jet>::const_iterator itJet = jets.begin();
    ...
    double jes_nom = 1.;
    double jer_nom = 1.;
    if (jet.hasUserFloat("HelperJES"))
      jes_nom = jet.userFloat("HelperJES");
    if (jet.hasUserFloat("HelperJER"))
      jer_nom = jet.userFloat("HelperJER");
  ...
  for (uint i_sys = 1; i_sys < uint(input.size()); i_sys++) {
    ...
    for (std::vector<pat::Jet>::const_iterator itJet = jets.begin();
      ...
      jet.scaleEnergy(
        1. / (jes_nom * jer_nom *
              jet.userFloat("Helper" + Systematics::toString(systematic))));
      if (jet.pt() < 20.)
        continue;
      jet.scaleEnergy(
        jes_nom * jer_nom *
