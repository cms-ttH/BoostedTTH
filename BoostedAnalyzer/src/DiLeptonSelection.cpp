...
DiLeptonSelection::DiLeptonSelection(std::vector< std::string > elelTriggers_, std::vector< std::string > mumuTriggers_,
                                     std::vector< std::string > elmuTriggers_, std::vector< std::string > elTriggers_, std::vector< std::string > muTriggers_,
                                     std::string channel_, int step_, std::string dataset_) :
...
DiLeptonSelection::DiLeptonSelection(std::string elelTrigger, std::string mumuTrigger, std::string elmuTrigger, std::string elTrigger, std::string muTrigger,
                                     std::string channel_, int step_, std::string dataset_) :
    DiLeptonSelection(std::vector< std::string >(1, elelTrigger), std::vector< std::string >(1, mumuTrigger), std::vector< std::string >(1, elmuTrigger),
                      std::vector< std::string >(1, elTrigger), std::vector< std::string >(1, muTrigger), channel_, step_, dataset_)
{
}

DiLeptonSelection::DiLeptonSelection(const edm::ParameterSet& iConfig, int step_) :
    DiLeptonSelection(iConfig.getParameter< std::vector< std::string > >("elelTriggers"), iConfig.getParameter< std::vector< std::string > >("mumuTriggers"),
                      iConfig.getParameter< std::vector< std::string > >("elmuTriggers"),
                      iConfig.getParameter< std::vector< std::string > >("electronTriggers"),
                      iConfig.getParameter< std::vector< std::string > >("muonTriggers"), iConfig.getParameter< std::string >("dlchannel"), step_,
...
bool DiLeptonSelection::IsSelected(const InputCollections& input, Cutflow& cutflow)
{
    ...
    if (dataset == "NA") {
        ...
        elmu_step2 = (elmuTriggered || elTriggered || muTriggered) && nleadleptons >= 1 &&
                     ((nelectrons_p == 1 && nmuons_n == 1 && nelectrons_n == 0 && nmuons_p == 0) ||
                      (nelectrons_n == 1 && nmuons_p == 1 && nelectrons_p == 0 && nmuons_n == 0));
        mumu_step2 = (mumuTriggered || muTriggered) && nleadmuons >= 1 && (nmuons_p == 1 && nmuons_n == 1 && nelectrons_p == 0 && nelectrons_n == 0);
    }
    else {
        if (dataset == "elel")
            elel_step2 = elelTriggered && nleadelectrons >= 1 && (nelectrons_p == 1 && nelectrons_n == 1 && nmuons_p == 0 && nmuons_n == 0);
        else if (dataset == "mumu")
            mumu_step2 = mumuTriggered && nleadmuons >= 1 && (nmuons_p == 1 && nmuons_n == 1 && nelectrons_p == 0 && nelectrons_n == 0);
            ...
            elel_step2 = (!elelTriggered && elTriggered) && nleadelectrons >= 1 && (nelectrons_p == 1 && nelectrons_n == 1 && nmuons_p == 0 && nmuons_n == 0);
            elmu_step2 = (!elmuTriggered && !muTriggered && elTriggered) && nleadleptons >= 1 &&
                         ((nelectrons_p == 1 && nmuons_n == 1 && nelectrons_n == 0 && nmuons_p == 0) ||
                          (nelectrons_n == 1 && nmuons_p == 1 && nelectrons_p == 0 && nmuons_n == 0));
        }
        else if (dataset == "mu") {
    ...
    */
    ...
    if (channel == "elel") {
        if (step < 0 || step == 1) {
            if (!elelTriggered && !elTriggered) { return false; }
            else {
        ...
        if (step < 0 || step == 1) {
            if (!mumuTriggered && !muTriggered) { return false; }
            else {
                cutflow.EventSurvivedStep("Dilepton trigger", input.weights.at("Weight"));
            }
        }
        ...
        if (step < 0 || step == 1) {
            if (!elmuTriggered && !elTriggered && !muTriggered) { return false; }
            else {
                cutflow.EventSurvivedStep("Dilepton trigger", input.weights.at("Weight"));
            }
        }
        ...
        if (step < 0 || step == 1) {
            if (!elmuTriggered && !elelTriggered && !mumuTriggered && !elTriggered && !muTriggered) { return false; }
            else {
                cutflow.EventSurvivedStep("Dilepton trigger", input.weights.at("Weight"));
            }
        }
