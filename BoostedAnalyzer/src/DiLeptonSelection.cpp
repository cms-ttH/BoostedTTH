...
DiLeptonSelection::DiLeptonSelection(std::string elelTrigger,
                                     std::string mumuTrigger,
                                     std::string elmuTrigger,
  ...
  : DiLeptonSelection(std::vector<std::string>(1, elelTrigger),
                      ...
                      std::vector<std::string>(1, muTrigger),
                      channel_,
                      step_,
                      dataset_)
{
...
void
DiLeptonSelection::InitCutflow(Cutflow& cutflow)
{
  if (step < 0 || step == 1)
...
bool
DiLeptonSelection::IsSelected(const InputCollections& input, Cutflow& cutflow)
{
  if (!initialized)
    cerr << "DiLeptonSelection not initialized" << endl;

  int nleadelectrons_p = 0;
  int nleadelectrons_n = 0;

  for (auto e = input.selectedElectronsDL.begin();
       e != input.selectedElectronsDL.end();
       e++) {
    if (e->charge() > 0) {
      nleadelectrons_p++;
    } else {
  ...
  for (auto e = input.selectedElectronsLoose.begin();
       e != input.selectedElectronsLoose.end();
       e++) {

    bool leadID = false;

    for (auto le = input.selectedElectronsDL.begin();
         le != input.selectedElectronsDL.end();
         le++) {
  ...
  for (auto mu = input.selectedMuonsLoose.begin();
       mu != input.selectedMuonsLoose.end();
       mu++) {

    bool leadID = false;

    for (auto lmu = input.selectedMuonsDL.begin();
         lmu != input.selectedMuonsDL.end();
         lmu++) {
  ...
  if (dataset == "NA") {
    ...
    elmu_step2 = (elmuTriggered || elTriggered || muTriggered) &&
                 nleadleptons >= 1 && ((nelectrons_p == 1 && nmuons_n == 1 &&
  ...
  mumuTriggered << "   elmuTriggered: " << elmuTriggered << std::endl;
  std::cout << "nleadelectrons: " << nleadelectrons << "   nelectrons_p: " <<
  nelectrons_p << "   nelectrons_n: " << nelectrons_n << std::endl;
  std::cout << "nleadmuons: " << nleadmuons << "   nmuons_p: " << nmuons_p << "
  nmuons_n: " << nmuons_n << std::endl;
  */
