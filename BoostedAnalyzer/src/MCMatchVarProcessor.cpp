...
void MCMatchVarProcessor::Init(const InputCollections& input,
  ...
  vars.InitVars("GenW_NotFromTop_DecProd_Pt", -9.,
                "N_GenW_NotFromTop_DecProds");
  vars.InitVars("GenW_NotFromTop_DecProd_Eta", -9.,
                "N_GenW_NotFromTop_DecProds");
  vars.InitVars("GenW_NotFromTop_DecProd_Phi", -9.,
                "N_GenW_NotFromTop_DecProds");
  vars.InitVars("GenW_NotFromTop_DecProd_E", -9., "N_GenW_NotFromTop_DecProds");
  vars.InitVars("GenW_NotFromTop_DecProd_PDGID", -999,
                "N_GenW_NotFromTop_DecProds");
...
void MCMatchVarProcessor::Process(const InputCollections& input,
                                  VariableContainer& vars) {
  ...
  if (input.genTopEvt.IsFilled()) {
    ...
    w_not_from_top_decay_products =
        input.genTopEvt.GetWNotFromTopDecayProducts();
    // for THQ
  ...
  if (input.genTopEvt.IsFilled() && input.genTopEvt.TTxIsFilled() &&
    ...
    std::vector<reco::GenParticle> bhad_hadron =
        input.genTopEvt.GetAllTopHadBHadrons();
    std::vector<reco::GenParticle> blep_hadron =
        input.genTopEvt.GetAllTopLepBHadrons();
  ...
  vars.FillVar("N_GenW_NotFromTop_DecProds",
               w_not_from_top_decay_products.size());
  for (size_t i = 0; i < w_not_from_top_decay_products.size(); i++) {
    vars.FillVars("GenW_NotFromTop_DecProd_Pt", i,
                  w_not_from_top_decay_products.at(i).pt());
    vars.FillVars("GenW_NotFromTop_DecProd_Eta", i,
                  w_not_from_top_decay_products.at(i).eta());
    vars.FillVars("GenW_NotFromTop_DecProd_Phi", i,
                  w_not_from_top_decay_products.at(i).phi());
    vars.FillVars("GenW_NotFromTop_DecProd_E", i,
                  w_not_from_top_decay_products.at(i).energy());
    vars.FillVars("GenW_NotFromTop_DecProd_PDGID", i,
                  w_not_from_top_decay_products.at(i).pdgId());
  }
