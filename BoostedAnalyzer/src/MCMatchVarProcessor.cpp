...
void MCMatchVarProcessor::Process(const InputCollections& input, VariableContainer& vars)
{
    ...
    for (size_t i = 0; i < toplep.size(); i++) {
        ...
        if (minDrTopLep < .25) { vars.FillVars("GenTopLep_B_Idx", i, idxblep); }
    }
    ...
    for (size_t i = 0; i < tophad.size(); i++) {
        ...
        vars.FillVars("GenTopHad_B_Q1_Q2_maxDR", i,
                      std::max(std::max(BoostedUtils::DeltaR(q1[i].p4(), q2[i].p4()), BoostedUtils::DeltaR(bhad[i].p4(), q1[i].p4())),
                               BoostedUtils::DeltaR(bhad[i].p4(), q2[i].p4())));
    }
    ...
    //         }
    //
    //         if (minDrB1 < .25) { vars.FillVar("GenHiggs_B1_Idx", idxb1); }
    //         if (minDrB2 < .25) { vars.FillVar("GenHiggs_B2_Idx", idxb2); }
    //     }
