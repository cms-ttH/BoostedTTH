...
TreeWriter::~TreeWriter()
{
    ...
    if (outFile != 0) { outFile->cd(); }
    if (tree != 0) { tree->Write(); }
    if (outFile != 0) {
        outFile->Write();
        outFile->Close();
    }
    if (tree != 0) { cout << "Tree Written to " << outFile->GetPath() << endl; }
}
...
const std::vector< std::unique_ptr< TreeProcessor > >& TreeWriter::GetTreeProcessors() const { return processors; }

std::vector< std::string > TreeWriter::GetTreeProcessorNames() const { return processorNames; }

bool TreeWriter::Process(const InputCollections& input, const bool& verbose)
{
    if (!initialized) {
        for (uint i = 0; i < processors.size(); i++) { processors[i]->Init(input, vars); }
