...
TreeWriter::~TreeWriter()
{
  for (uint i = 0; i < stopwatches.size(); i++) {
    cout << "time spent in " << processorNames[i]
         << " -- real time: " << stopwatches[i].RealTime()
         << ", cpu time: " << stopwatches[i].CpuTime() << endl;
  }
  if (outFile != 0) {
    outFile->cd();
  }
...
void
TreeWriter::Init(std::string fileName)
{
...
void
TreeWriter::AddTreeProcessor(TreeProcessor* processor, string name)
{
...
const std::vector<std::unique_ptr<TreeProcessor>>&
TreeWriter::GetTreeProcessors() const
{
  return processors;
}

std::vector<std::string>
TreeWriter::GetTreeProcessorNames() const
{
  return processorNames;
}

bool
TreeWriter::Process(const InputCollections& input, const bool& verbose)
{
  ...
  for (uint i = 0; i < processors.size(); i++) {
    if (verbose)
      std::cout << "Start processing " << processorNames.at(i) << std::endl;
    stopwatches[i].Start(firstEvent);
    processors[i]->Process(input, vars);
    stopwatches[i].Stop();
    if (verbose)
      std::cout << "Done processing " << processorNames.at(i) << std::endl;
  }
...
bool
TreeWriter::Process(const std::vector<InputCollections>& input,
                    const bool& verbose)
{
