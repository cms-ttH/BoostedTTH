...
BoostedttHEvent::BoostedttHEvent()
    : input(0), verbose(false), btagger("DeepJet") {
  ResetEvent();
}
...
void BoostedttHEvent::ak5JetsIdentifyHiggsCand(
    const float jetMatchingThreshold) {
  HiggsCandak5Jet.clear();
  nHiggsak5Jets = 0;
  nHiggsCandBTagL = 0;
  nHiggsCandBTagM = 0;
...
void BoostedttHEvent::ak5JetsIdentifyTopHadCand(
    const float jetMatchingThreshold) {
  TopHadCandak5Jet.clear();
  nTopHadak5Jets = 0;
  nTopHadCandBTagL = 0;
...
void BoostedttHEvent::ak5JetsIdentifyTopLepCand(
    const float jetMatchingThreshold) {
  TopLepCandak5Jet.clear();
  nTopLepak5Jets = 0;
  nTopLepCandBTagL = 0;
...
void BoostedttHEvent::ak5JetsClean(bool cleanHiggsCand, bool cleanTopHadCand,
...
void BoostedttHEvent::ak4ClusterHiggsCandBoostedRec(
    const bool cleanTopHadCand, const bool cleanTopLepCand,
  ...
  for (std::vector<boosted::Ak4Cluster>::const_iterator itAk4Clu =
       ...
       itAk4Clu != input->selectedAk4Cluster.end(); ++itAk4Clu) {
...
void BoostedttHEvent::ak4JetsHiggsCandBoostedRec(
    const bool cleanTopHadCand, const bool cleanTopLepCand,
    const float subjetCleaningThreshold) {
  higgsCand = boosted::BoostedJet();
  higgsB1Cand = pat::Jet();
  ...
  for (std::vector<boosted::BoostedJet>::const_iterator itBooJet =
       ...
       itBooJet != input->selectedBoostedJets.end(); ++itBooJet) {
    ...
    for (std::vector<pat::Jet>::const_iterator itAk4Jet = selectedJets.begin();
         itAk4Jet != selectedJets.end(); ++itAk4Jet) {
...
void BoostedttHEvent::HiggsCandBoostedRec(const boosted::SubjetType subjettype,
  ...
  for (std::vector<boosted::BoostedJet>::const_iterator itJet =
           input->selectedBoostedJets.begin();
       itJet != input->selectedBoostedJets.end(); ++itJet) {
    ...
    if (cleanTopLepCand && foundTopLepCand) {
      ...
      for (std::vector<pat::Jet>::const_iterator itFilt =
           ...
           itFilt != itJet->filterjets.end(); ++itFilt) {
        if (topLepBCand.pt() > 0. &&
...
void BoostedttHEvent::HiggsCandRec() {  // Non Boosted Higgs Reconstruction by
                                        // Delta R and Loose Btags
...
void BoostedttHEvent::TopHadCandBoostedRec(
    TopTagger toptagger, const bool cleanTopLepCand, const bool cleanHiggsCand,
    const float subjetCleaningThreshold) {
  topHadCand = boosted::BoostedJet();
  topHadBCand = pat::Jet();
  topHadW1Cand = pat::Jet();
...
void BoostedttHEvent::BoostedTopHiggsEventRec(
    TopTagger toptagger, const boosted::SubjetType subjettype,
    HiggsTagger higgstagger) {
  ResetEvent();
...
void BoostedttHEvent::BoostedHiggsEventRec(const boosted::SubjetType subjettype,
                                           HiggsTagger higgstagger) {
  ResetEvent();
...
void BoostedttHEvent::BoostedTopAk4HiggsEventRec(
    TopTagger toptagger, const boosted::SubjetType subjettype,
    HiggsTagger higgstagger) {
  ResetEvent();
...
void BoostedttHEvent::BoostedTopAk4HiggsFromAk4CEventRec(
    TopTagger toptagger, const boosted::SubjetType subjettype,
    HiggsTagger higgstagger) {
  ResetEvent();
...
const InputCollections& BoostedttHEvent::GetInput() { return *input; }

math::XYZTLorentzVector BoostedttHEvent::GetLeptonVec() { return lepVecCand; }

math::XYZTLorentzVector BoostedttHEvent::GetNeutrinoVec() { return nuVecCand; }
...
std::vector<pat::Jet> BoostedttHEvent::Getak5JetsAll() { return selectedJets; }

int BoostedttHEvent::GetNJets() { return nJets; }

int BoostedttHEvent::GetNBTagL() { return nBTagL; }

int BoostedttHEvent::GetNBTagM() { return nBTagM; }

int BoostedttHEvent::GetNBTagT() { return nBTagT; }
...
int BoostedttHEvent::GetNHiggsak5Jets() { return nHiggsak5Jets; }

int BoostedttHEvent::GetNHiggsCandBTagL() { return nHiggsCandBTagL; }

int BoostedttHEvent::GetNHiggsCandBTagM() { return nHiggsCandBTagM; }

int BoostedttHEvent::GetNHiggsCandBTagT() { return nHiggsCandBTagT; }
...
int BoostedttHEvent::GetNTopHadak5Jets() { return nTopHadak5Jets; }

int BoostedttHEvent::GetNTopHadCandBTagL() { return nTopHadCandBTagL; }

int BoostedttHEvent::GetNTopHadCandBTagM() { return nTopHadCandBTagM; }

int BoostedttHEvent::GetNTopHadCandBTagT() { return nTopHadCandBTagT; }
...
int BoostedttHEvent::GetNTopLepak5Jets() { return nTopLepak5Jets; }

int BoostedttHEvent::GetNTopLepCandBTagL() { return nTopLepCandBTagL; }

int BoostedttHEvent::GetNTopLepCandBTagM() { return nTopLepCandBTagM; }

int BoostedttHEvent::GetNTopLepCandBTagT() { return nTopLepCandBTagT; }
...
int BoostedttHEvent::GetNCleanedak5Jets() { return nCleanedak5Jets; }

int BoostedttHEvent::GetNCleanedBTagL() { return nCleanedBTagL; }

int BoostedttHEvent::GetNCleanedBTagM() { return nCleanedBTagM; }

int BoostedttHEvent::GetNCleanedBTagT() { return nCleanedBTagT; }
...
bool BoostedttHEvent::GetFoundHiggsCand() { return foundHiggsCand; }

int BoostedttHEvent::GetHiggsCandIndex() { return higgsCandIndex; }
...
bool BoostedttHEvent::GetUseAk4Cluster() { return useAk4Cluster; }

bool BoostedttHEvent::GetFoundTopHadCand() { return foundTopHadCand; }

int BoostedttHEvent::GetTopHadCandIndex() { return topHadCandIndex; }
...
bool BoostedttHEvent::GetFoundTopLepCand() { return foundTopLepCand; }
...
float BoostedttHEvent::GetTTHBB_ME() {
  ...
  TLorentzVector top = BoostedUtils::GetTLorentzVector(
      topHadBCand.p4() + topHadW1Cand.p4() + topHadW2Cand.p4());
  TLorentzVector topbar = BoostedUtils::GetTLorentzVector(
      lepVecCand + nuVecCand + topLepBCand.p4());
