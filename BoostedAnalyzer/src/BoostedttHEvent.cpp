...
BoostedttHEvent::BoostedttHEvent() : input(0), verbose(false), btagger("DeepJet") { ResetEvent(); }

BoostedttHEvent::~BoostedttHEvent() {}

void BoostedttHEvent::SetInput(const InputCollections* input_) { input = input_; }
...
void BoostedttHEvent::LeptonRec() { lepVecCand = BoostedUtils::GetPrimLepVec(input->selectedElectronsLoose, input->selectedMuonsLoose); }
...
void BoostedttHEvent::ak4ClusterHiggsCandBoostedRec(const bool cleanTopHadCand, const bool cleanTopLepCand, const float subjetCleaningThreshold)
{
    ...
    for (std::vector< boosted::Ak4Cluster >::const_iterator itAk4Clu = input->selectedAk4Cluster.begin(); itAk4Clu != input->selectedAk4Cluster.end();
         ++itAk4Clu) {
    ...
    if (verbose) {
        if (foundHiggsCand)
            std::cout << "Ak4: found higgs candidate, max tag " << higgsCandTag << std::endl;
        else
...
void BoostedttHEvent::ak4JetsHiggsCandBoostedRec(const bool cleanTopHadCand, const bool cleanTopLepCand, const float subjetCleaningThreshold)
{
    ...
    // loop over fatjets
    for (std::vector< boosted::BoostedJet >::const_iterator itBooJet = input->selectedBoostedJets.begin(); itBooJet != input->selectedBoostedJets.end();
         ++itBooJet) {
...
void BoostedttHEvent::HiggsCandBoostedRec(const boosted::SubjetType subjettype, HiggsTagger higgstagger, const bool cleanTopHadCand, const bool cleanTopLepCand,
                                          const float subjetCleaningThreshold)
{
    ...
    for (std::vector< boosted::BoostedJet >::const_iterator itJet = input->selectedBoostedJets.begin(); itJet != input->selectedBoostedJets.end(); ++itJet) {
        ...
        switch (subjettype) {
            ...
            default: cleanedSubjets = itJet->filterjets;
        }
    ...
    if (verbose) {
        if (foundHiggsCand)
            std::cout << "found higgs candidate, max tag " << higgsCandTag << std::endl;
        else
            std::cout << "did not find higgs candidate" << std::endl;
    }
}

void BoostedttHEvent::HiggsCandRec()
{  // Non Boosted Higgs Reconstruction by
        ...
        for (size_t iTopHadQ1 = 0; iTopHadQ1 < iTopHadQ2; iTopHadQ1++) {
            for (size_t iTopHadB = 0; iTopHadB < cleanedak5Jets.size(); iTopHadB++) {
                for (size_t iTopLepB = 0; iTopLepB < cleanedak5Jets.size(); iTopLepB++) {
                    ...
                    if (nCleanedBTagM > 1 &&
                        !(BoostedUtils::PassesCSV(cleanedak5Jets[iTopHadB], 'M') && BoostedUtils::PassesCSV(cleanedak5Jets[iTopLepB], 'M')))
                        continue;
                    if (nCleanedBTagM == 1 &&
                        !(BoostedUtils::PassesCSV(cleanedak5Jets[iTopHadB], 'M') || BoostedUtils::PassesCSV(cleanedak5Jets[iTopLepB], 'M')))
                        continue;
...
math::XYZTLorentzVector BoostedttHEvent::GetNeutrino1Vec() { return nu1VecCand; }

math::XYZTLorentzVector BoostedttHEvent::GetNeutrino2Vec() { return nu2VecCand; }
...
std::vector< pat::Jet > BoostedttHEvent::Getak5JetsCleaned() { return cleanedak5Jets; }
...
float BoostedttHEvent::GetAverageCSVClean()
{
    ...
    for (size_t iJet = 0; iJet < cleanedak5Jets.size(); iJet++) avgCSV += fmax(CSVHelper::GetJetCSV(cleanedak5Jets[iJet], btagger), 0.);
