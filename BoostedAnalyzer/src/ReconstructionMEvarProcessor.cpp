...
void ReconstructionMEvarProcessor::Init(const InputCollections& input,
...
void ReconstructionMEvarProcessor::Process(const InputCollections& input,
                                           VariableContainer& vars) {
  ...
  if (input.genTopEvt.IsFilled() && input.genTopEvt.IsSemiLepton() &&
    ...
    if (qs_true.size() == 2 && additional_b_genjets.size() >= 2) {
      ...
      TLorentzVector nu_true =
          BoostedUtils::GetTLorentzVector(input.genTopEvt.GetNeutrinoVec());
      mcmatcher.Setup(
          bhad_true, qs_true[0], qs_true[1], blep_true, lep_true, nu_true,
  ...
  vars.FillVar(
      ...
      sum_tth_me_likelihood / (sum_tth_me_likelihood + sum_ttbb_me_likelihood));
