...
void ReconstructionMEvarProcessor::Process(const InputCollections& input, VariableContainer& vars)
{
    ...
    if (input.genTopEvt.IsFilled() && input.genTopEvt.IsSemiLepton() && input.sampleType == ttbb) {
        ...
        if (qs_true.size() == 2 && additional_b_genjets.size() >= 2) {
            ...
            TLorentzVector lep_true  = BoostedUtils::GetTLorentzVector(input.genTopEvt.GetLeptonVec());
            TLorentzVector nu_true   = BoostedUtils::GetTLorentzVector(input.genTopEvt.GetNeutrinoVec());
            mcmatcher.Setup(bhad_true, qs_true[0], qs_true[1], blep_true, lep_true, nu_true, BoostedUtils::GetTLorentzVector(additional_b_genjets[0].p4()),
    ...
    for (auto tagname = alltags.begin(); tagname != alltags.end(); tagname++) { best_int[*tagname] = 0; }
    ...
    for (auto t = best_tag.begin(); t != best_tag.end(); t++) { vars.FillVar("Gen_highest_" + t->first, t->second); }
    float toptag = -9999;
    for (uint i = 0; i < nints; i++) {
        float tag = quality.TopAndWHadLikelihood(*ints[i]);
        if (tag > toptag) { toptag = tag; }
    }
