...
boosted::Ak4ClusterCollection
Ak4Cluster::GetAk4Cluster(const pat::JetCollection& inputAk4Jets,
                          const int algo = 0)
{
  ...
  while (outputAk4Cluster.size() > 1) {
    ...
    if (mindij < mindiB) {
      ...
      for (unsigned int iA = 0;
           iA < outputAk4Cluster[cluster_jet2_id].ak4jets.size();
           ++iA) {
  ...
  std::sort(outputAk4Cluster.begin(),
            outputAk4Cluster.end(),
...
boosted::Ak4ClusterCollection
Ak4Cluster::GetSelectedAk4Cluster(
  ...
  for (unsigned int iA = 0; iA < allAk4Clusters.size(); ++iA) {
    ...
    if (mode == "C") {
      ...
      if (tempCluster.ak4jets.size() < 2) {
        ...
        std::sort(sortedPtAk4Jets.begin(),
                  sortedPtAk4Jets.end(),
                  BoostedUtils::FirstJetIsHarder);
...
bool
Ak4Cluster::FirstFatjetInAk4ClusterIsHarder(boosted::Ak4Cluster clu1,
