    ...
    while (outputAk4Cluster.size() > 1) {
        ...
        for (unsigned int iA = 0; iA < outputAk4Cluster.size(); ++iA) {
            for (unsigned int iB = 0; iB < outputAk4Cluster.size(); ++iB) {
                ...
                // get the dij
                dij[iA][iB] = min(pow(outputAk4Cluster[iA].fatjet.Pt(), 2 * algo), pow(outputAk4Cluster[iA].fatjet.Pt(), 2 * algo)) *
                              BoostedUtils::DeltaR(outputAk4Cluster[iA].fatjet, outputAk4Cluster[iB].fatjet) / (clusterRadius * clusterRadius);
    ...
    for (unsigned int iA = 0; iA < allAk4Clusters.size(); ++iA) {
        ...
        if (mode == "C") {
            ...
            if (tempCluster.fatjet.Pt() > iMinClusterPt) {
                if (tempCluster.ak4jets.size() < 2) { tempCluster.isGoodHiggsCluster = false; }
                else
            ...
            if (tempCluster.ak4jets.size() < 2) { tempCluster.isGoodHiggsCluster = false; }
                ...
                for (std::vector< pat::Jet >::const_iterator itFilt2 = sortedPtAk4Jets.begin(), edFilt2 = sortedPtAk4Jets.end(); itFilt2 != edFilt2;
                     ++itFilt2) {
                ...
                if (mode == "A") {
                    ...
                    if ((sortedAk4Jets[0].p4() + sortedAk4Jets[1].p4()).pt() > iMinClusterPt)
                        tempCluster.isGoodHiggsCluster = true;
                    else
                        tempCluster.isGoodHiggsCluster = false;
                }
                else if (mode == "B") {
                    if ((threeHardestAk4Jets[0].p4() + threeHardestAk4Jets[1].p4() + threeHardestAk4Jets[2].p4()).pt() > iMinClusterPt)
                        tempCluster.isGoodHiggsCluster = true;
                    else
                        tempCluster.isGoodHiggsCluster = false;
                }
...
bool Ak4Cluster::FirstFatjetInAk4ClusterIsHarder(boosted::Ak4Cluster clu1, boosted::Ak4Cluster clu2) { return clu1.fatjet.pt() > clu2.fatjet.pt(); }