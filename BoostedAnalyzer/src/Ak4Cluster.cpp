...
boosted::Ak4ClusterCollection Ak4Cluster::GetAk4Cluster(
    const pat::JetCollection& inputAk4Jets, const int algo = 0) {
  ...
  for (std::vector<pat::Jet>::const_iterator itJet = inputAk4Jets.begin();
       itJet != inputAk4Jets.end(); ++itJet) {
  ...
  while (outputAk4Cluster.size() > 1) {
    ...
    for (unsigned int iA = 0; iA < outputAk4Cluster.size(); ++iA) {
      for (unsigned int iB = 0; iB < outputAk4Cluster.size(); ++iB) {
        ...
        // get the dij
        dij[iA][iB] = min(pow(outputAk4Cluster[iA].fatjet.Pt(), 2 * algo),
                          pow(outputAk4Cluster[iA].fatjet.Pt(), 2 * algo)) *
                      BoostedUtils::DeltaR(outputAk4Cluster[iA].fatjet,
                                           outputAk4Cluster[iB].fatjet) /
    ...
    if (mindij < mindiB) {
      ...
      outputAk4Cluster[cluster_jet1_id].fatjet +=
          outputAk4Cluster[cluster_jet2_id].fatjet;

      for (unsigned int iA = 0;
           iA < outputAk4Cluster[cluster_jet2_id].ak4jets.size(); ++iA) {
        outputAk4Cluster[cluster_jet1_id].ak4jets.push_back(
            outputAk4Cluster[cluster_jet2_id].ak4jets[iA]);
      }
  ...
  for (unsigned int iAk4Cluster = 0; iAk4Cluster < outputAk4Cluster.size();
       ++iAk4Cluster) {
    std::sort(outputAk4Cluster[iAk4Cluster].ak4jets.begin(),
              outputAk4Cluster[iAk4Cluster].ak4jets.end(),
              BoostedUtils::FirstJetIsHarder);
  }

  // sort ak4cluster by pT
  std::sort(outputAk4Cluster.begin(), outputAk4Cluster.end(),
            Ak4Cluster::FirstFatjetInAk4ClusterIsHarder);

  return outputAk4Cluster;
}

boosted::Ak4ClusterCollection Ak4Cluster::GetSelectedAk4Cluster(
    const boosted::Ak4ClusterCollection& allAk4Clusters,
    const double iMinClusterPt, const string mode = "C") {
    ...
    if (mode == "C") {
      ...
      if (tempCluster.fatjet.Pt() > iMinClusterPt) {
        if (tempCluster.ak4jets.size() < 2) {
          ...
          tempCluster.isGoodHiggsCluster = true;

        if (tempCluster.ak4jets.size() < 3)
          tempCluster.isGoodTopCluster = false;
        else
          tempCluster.isGoodTopCluster = true;
      }
    } else if (mode == "A" || mode == "B") {
      // mode A: pT cut on two filterjets with best CSV out of the three hardest
      // subjets mode B: pT cut on three hardest filterjets
      if (tempCluster.ak4jets.size() < 3)
        tempCluster.isGoodTopCluster = false;
      else {
        std::vector<pat::Jet> sortedAk4TopJets = tempCluster.ak4jets;
        BoostedUtils::TopSubjetCSVDef(sortedAk4TopJets);
        if ((sortedAk4TopJets[0].p4() + sortedAk4TopJets[1].p4() +
             sortedAk4TopJets[2].p4())
                .pt() > iMinClusterPt) {
          tempCluster.isGoodTopCluster = true;
        } else
          tempCluster.isGoodTopCluster = false;
      ...
      if (tempCluster.ak4jets.size() < 2) {
        ...
        std::sort(sortedPtAk4Jets.begin(), sortedPtAk4Jets.end(),
                  BoostedUtils::FirstJetIsHarder);
        std::vector<pat::Jet> threeHardestAk4Jets;
        for (std::vector<pat::Jet>::const_iterator
                 itFilt2 = sortedPtAk4Jets.begin(),
                 edFilt2 = sortedPtAk4Jets.end();
             itFilt2 != edFilt2; ++itFilt2) {
        ...
        if (mode == "A") {
          ...
          std::sort(sortedAk4Jets.begin(), sortedAk4Jets.end(),
                    BoostedUtils::FirstHasHigherCSV);
          if ((sortedAk4Jets[0].p4() + sortedAk4Jets[1].p4()).pt() >
              iMinClusterPt)
            tempCluster.isGoodHiggsCluster = true;
          else
            tempCluster.isGoodHiggsCluster = false;
        } else if (mode == "B") {
          if ((threeHardestAk4Jets[0].p4() + threeHardestAk4Jets[1].p4() +
               threeHardestAk4Jets[2].p4())
                  .pt() > iMinClusterPt)
            tempCluster.isGoodHiggsCluster = true;
          else
            tempCluster.isGoodHiggsCluster = false;
      ...
      cout << "Error in GetSelectedAk4Cluster: No valid selection mode.";

    if (tempCluster.isGoodHiggsCluster || tempCluster.isGoodTopCluster)
...
bool Ak4Cluster::FirstFatjetInAk4ClusterIsHarder(boosted::Ak4Cluster clu1,
