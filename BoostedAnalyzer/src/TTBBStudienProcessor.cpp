...
void
TTBBStudienProcessor::Init(const InputCollections& input,
...
void
TTBBStudienProcessor::Process(const InputCollections& input,
                              VariableContainer& vars)
{
  if (!initialized)
...
void
TTBBStudienProcessor::initTopDR(const InputCollections& input,
                                VariableContainer& vars)
{
...
void
TTBBStudienProcessor::processTopDR(const InputCollections& input,
                                   VariableContainer& vars)
{
  ...
  for (std::vector<pat::Jet>::const_iterator it1Jet =
       ...
       it1Jet != selectedUntaggedJets.end();
       ++it1Jet) {
    for (std::vector<pat::Jet>::const_iterator it2Jet =
           selectedUntaggedJets.begin();
         it2Jet != selectedUntaggedJets.end();
         ++it2Jet) {
  ...
  additionalTaggedJets.erase(std::remove_if(additionalTaggedJets.begin(),
                                            additionalTaggedJets.end(),
                                            [&savedBJet](pat::Jet x) {
                                              return (x.p4() == savedBJet.p4());
                                            }),
                             additionalTaggedJets.end());
  ...
  for (std::vector<pat::Jet>::const_iterator itJet =
       ...
       itJet != additionalUntaggedJets.end();
       ++itJet) {
    int iJet = itJet - additionalUntaggedJets.begin();
    vars.FillVars("HadTop_additionalUntaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(HadTop, itJet->p4()));
    vars.FillVars("HadTop_B_additionalUntaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(HadB, itJet->p4()));
    vars.FillVars("HadTop_W_additionalUntaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(HadW, itJet->p4()));
    vars.FillVars("HadTop_W1_additionalUntaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(HadW1, itJet->p4()));
    vars.FillVars("HadTop_W2_additionalUntaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(HadW2, itJet->p4()));

    vars.FillVars("LepTop_additionalUntaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(LepTop, itJet->p4()));
    vars.FillVars("LepTop_B_additionalUntaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(LepB, itJet->p4()));
    vars.FillVars("LepTop_W_additionalUntaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(LepW, itJet->p4()));
    vars.FillVars("LepTop_W1_additionalUntaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(LepW1, itJet->p4()));
    vars.FillVars("LepTop_W2_additionalUntaggedJets",
                  iJet,
  ...
  for (std::vector<pat::Jet>::const_iterator itJet =
       ...
       itJet != additionalTaggedJets.end();
       ++itJet) {
    int iJet = itJet - additionalTaggedJets.begin();
    vars.FillVars("HadTop_additionalTaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(HadTop, itJet->p4()));
    vars.FillVars("HadTop_B_additionalTaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(HadB, itJet->p4()));
    vars.FillVars("HadTop_W_additionalTaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(HadW, itJet->p4()));
    vars.FillVars("HadTop_W1_additionalTaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(HadW1, itJet->p4()));
    vars.FillVars("HadTop_W2_additionalTaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(HadW2, itJet->p4()));

    vars.FillVars("LepTop_additionalTaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(LepTop, itJet->p4()));
    vars.FillVars("LepTop_B_additionalTaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(LepB, itJet->p4()));
    vars.FillVars("LepTop_W_additionalTaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(LepW, itJet->p4()));
    vars.FillVars("LepTop_W1_additionalTaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(LepW1, itJet->p4()));
    vars.FillVars("LepTop_W2_additionalTaggedJets",
                  iJet,
                  BoostedUtils::DeltaR(LepW2, itJet->p4()));
  }
}
...
void
TTBBStudienProcessor::initEventType(const InputCollections& input,
                                    VariableContainer& vars)
{
...
void
TTBBStudienProcessor::processEventType(const InputCollections& input,
                                       VariableContainer& vars)
{
  int iBB = 0;
  int iCC = 0;

  if (input.sampleType == SampleType::ttbb)
    iBB = 3;
  if (input.sampleType == SampleType::ttb)
    iBB = 1;
  if (input.sampleType == SampleType::tt2b)
    iBB = 2;
  if (input.sampleType == SampleType::ttcc)
    iCC = 1;
...
void
TTBBStudienProcessor::initAmountTags(const InputCollections& input,
                                     VariableContainer& vars)
{
...
void
TTBBStudienProcessor::processAmountTags(const InputCollections& input,
                                        VariableContainer& vars)
{
