...
void TTBBStudienProcessor::processTopDR(const InputCollections& input, VariableContainer& vars)
{
    ...
    for (std::vector< pat::Jet >::const_iterator itJet = input.selectedJets.begin(); itJet != input.selectedJets.end(); ++itJet) {
        if (CSVHelper::PassesCSV(*itJet, "DeepJet", CSVHelper::CSVwp::Medium, input.era))
    ...
    additionalUntaggedJets.erase(std::remove_if(additionalUntaggedJets.begin(), additionalUntaggedJets.end(),
    ...
    additionalTaggedJets.erase(
        std::remove_if(additionalTaggedJets.begin(), additionalTaggedJets.end(), [&savedBJet](pat::Jet x) { return (x.p4() == savedBJet.p4()); }),
    ...
    TVector2                MET;
    MET.Set(input.correctedMET.corP4(pat::MET::Type1).px(), input.correctedMET.corP4(pat::MET::Type1).py());
...
void TTBBStudienProcessor::processEventType(const InputCollections& input, VariableContainer& vars)
{
    ...
    if (input.genTopEvt.IsFilled()) { vars.FillVar("TTBB_GenEvt_TTxId_FromProducer", input.genTopEvt.GetTTxIdFromProducer()); }
}
