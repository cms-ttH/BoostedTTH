...
void TTBBStudienProcessor::Init(const InputCollections& input,
...
void TTBBStudienProcessor::Process(const InputCollections& input,
                                   VariableContainer& vars) {
  if (!initialized)
...
void TTBBStudienProcessor::initTopDR(const InputCollections& input,
                                     VariableContainer& vars) {
...
void TTBBStudienProcessor::processTopDR(const InputCollections& input,
                                        VariableContainer& vars) {
  ...
  for (std::vector<pat::Jet>::const_iterator itJet = input.selectedJets.begin();
    ...
    if (CSVHelper::PassesCSV(*itJet, "DeepJet", CSVHelper::CSVwp::Medium,
                             input.era))
  ...
  for (std::vector<pat::Jet>::const_iterator it1Jet =
           selectedUntaggedJets.begin();
       it1Jet != selectedUntaggedJets.end(); ++it1Jet) {
    for (std::vector<pat::Jet>::const_iterator it2Jet =
             selectedUntaggedJets.begin();
         it2Jet != selectedUntaggedJets.end(); ++it2Jet) {
      if (HadW1.M() == 0 || HadW2.M() ||
          abs(80.4 - (it1Jet->p4() + it2Jet->p4()).M()) <
              abs(80.4 - (HadW1 + HadW2).M())) {
        HadW1 = it1Jet->p4();
        HadW2 = it2Jet->p4();
        HadW = HadW1 + HadW2;
        savedW1Jet = *it1Jet;
  ...
  additionalUntaggedJets.erase(
      std::remove_if(
          additionalUntaggedJets.begin(), additionalUntaggedJets.end(),
          [&savedW1Jet, &savedW2Jet](pat::Jet x) {
            return (x.p4() == savedW1Jet.p4() || x.p4() == savedW2Jet.p4());
          }),
  ...
  for (std::vector<pat::Jet>::const_iterator itJet = selectedTaggedJets.begin();
       itJet != selectedTaggedJets.end(); ++itJet) {
    if (HadTop.M() == 0 ||
        abs(172.4 - (HadW + itJet->p4()).M()) < abs(172.4 - HadTop.M())) {
      HadB = itJet->p4();
      HadTop = HadB + HadW;
      savedBJet = *itJet;
    }
  ...
  // Create vector for additional tagged jets which are not used as b for the
  // top
  ...
  additionalTaggedJets.erase(
      std::remove_if(
          additionalTaggedJets.begin(), additionalTaggedJets.end(),
  ...
  TVector2 MET;
  MET.Set(input.correctedMET.corP4(pat::MET::Type1XY).px(),
          input.correctedMET.corP4(pat::MET::Type1XY).py());
  math::XYZTLorentzVector PrimLep =
  ...
  LepW1 = nu1 + PrimLep;
  LepW2 = nu2 + PrimLep;
  if (abs(80.4 - LepW1.M()) < abs(80.4 - LepW2.M()))
    LepW = LepW1;
  else
    LepW = LepW2;
  ...
  for (std::vector<pat::Jet>::const_iterator itJet = selectedTaggedJets.begin();
       itJet != selectedTaggedJets.end(); ++itJet) {
    if (LepTop.M() == 0 ||
        abs(172.4 - (LepW + itJet->p4()).M()) < abs(172.4 - LepTop.M())) {
      LepB = itJet->p4();
      LepTop = LepW + LepB;
    }
  }
  ...
  for (std::vector<pat::Jet>::const_iterator itJet =
           additionalUntaggedJets.begin();
       itJet != additionalUntaggedJets.end(); ++itJet) {
    int iJet = itJet - additionalUntaggedJets.begin();
    vars.FillVars("HadTop_additionalUntaggedJets", iJet,
                  BoostedUtils::DeltaR(HadTop, itJet->p4()));
    vars.FillVars("HadTop_B_additionalUntaggedJets", iJet,
                  BoostedUtils::DeltaR(HadB, itJet->p4()));
    vars.FillVars("HadTop_W_additionalUntaggedJets", iJet,
                  BoostedUtils::DeltaR(HadW, itJet->p4()));
    vars.FillVars("HadTop_W1_additionalUntaggedJets", iJet,
                  BoostedUtils::DeltaR(HadW1, itJet->p4()));
    vars.FillVars("HadTop_W2_additionalUntaggedJets", iJet,
                  BoostedUtils::DeltaR(HadW2, itJet->p4()));

    vars.FillVars("LepTop_additionalUntaggedJets", iJet,
                  BoostedUtils::DeltaR(LepTop, itJet->p4()));
    vars.FillVars("LepTop_B_additionalUntaggedJets", iJet,
                  BoostedUtils::DeltaR(LepB, itJet->p4()));
    vars.FillVars("LepTop_W_additionalUntaggedJets", iJet,
                  BoostedUtils::DeltaR(LepW, itJet->p4()));
    vars.FillVars("LepTop_W1_additionalUntaggedJets", iJet,
                  BoostedUtils::DeltaR(LepW1, itJet->p4()));
    vars.FillVars("LepTop_W2_additionalUntaggedJets", iJet,
                  BoostedUtils::DeltaR(LepW2, itJet->p4()));
  }

  for (std::vector<pat::Jet>::const_iterator itJet =
           additionalTaggedJets.begin();
       itJet != additionalTaggedJets.end(); ++itJet) {
    int iJet = itJet - additionalTaggedJets.begin();
    vars.FillVars("HadTop_additionalTaggedJets", iJet,
                  BoostedUtils::DeltaR(HadTop, itJet->p4()));
    vars.FillVars("HadTop_B_additionalTaggedJets", iJet,
                  BoostedUtils::DeltaR(HadB, itJet->p4()));
    vars.FillVars("HadTop_W_additionalTaggedJets", iJet,
                  BoostedUtils::DeltaR(HadW, itJet->p4()));
    vars.FillVars("HadTop_W1_additionalTaggedJets", iJet,
                  BoostedUtils::DeltaR(HadW1, itJet->p4()));
    vars.FillVars("HadTop_W2_additionalTaggedJets", iJet,
                  BoostedUtils::DeltaR(HadW2, itJet->p4()));

    vars.FillVars("LepTop_additionalTaggedJets", iJet,
                  BoostedUtils::DeltaR(LepTop, itJet->p4()));
    vars.FillVars("LepTop_B_additionalTaggedJets", iJet,
                  BoostedUtils::DeltaR(LepB, itJet->p4()));
    vars.FillVars("LepTop_W_additionalTaggedJets", iJet,
                  BoostedUtils::DeltaR(LepW, itJet->p4()));
    vars.FillVars("LepTop_W1_additionalTaggedJets", iJet,
                  BoostedUtils::DeltaR(LepW1, itJet->p4()));
    vars.FillVars("LepTop_W2_additionalTaggedJets", iJet,
                  BoostedUtils::DeltaR(LepW2, itJet->p4()));
  }
}

void TTBBStudienProcessor::initEventType(const InputCollections& input,
                                         VariableContainer& vars) {
...
void TTBBStudienProcessor::processEventType(const InputCollections& input,
                                            VariableContainer& vars) {
  int iBB = 0;
  int iCC = 0;
  ...
  if (input.genTopEvt.IsFilled()) {
    vars.FillVar("TTBB_GenEvt_TTxId_FromProducer",
                 input.genTopEvt.GetTTxIdFromProducer());
  }
}

void TTBBStudienProcessor::initAmountTags(const InputCollections& input,
                                          VariableContainer& vars) {
...
void TTBBStudienProcessor::processAmountTags(const InputCollections& input,
                                             VariableContainer& vars) {
