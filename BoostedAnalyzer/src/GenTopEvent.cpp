...
GenTopEventProducer::GenTopEventProducer(edm::ConsumesCollector&& iC)
    :
      ...
      genBHadPlusMothersIndicesToken{
          iC.consumes<std::vector<std::vector<int> > >(edm::InputTag(
      ...
      genTtbarIdToken{iC.consumes<int>(
          edm::InputTag("categorizeGenTtbar", "genTtbarId", ""))},
...
GenTopEvent GenTopEventProducer::Produce(const edm::Event& iEvent,
  ...
  if (doGenHadronMatch) {
    genTopEvt.FillTTxDetails(
        *h_customgenjets, *genBHadIndex, *genBHadJetIndex, *genBHadFlavour,
        *genBHadFromTopWeakDecay, *genBHadPlusMothers, *genCHadIndex,
        *genCHadJetIndex, *genCHadFlavour, *genCHadFromTopWeakDecay,
        *genCHadPlusMothers, *genCHadBHadronId);
  }
  return genTopEvt;
}
...
void GenTopEvent::FillTTxDetails(
  ...
  for (uint i = 0; i < customGenJets.size(); i++) {
    ...
    if (nb_per_genjet[i] > 0) {
      ...
      additional_c_genjet_hadron.push_back(genjet_leading_chadron[i] != 0
                                               ...
                                               : reco::GenParticle());
      additional_c_genjet_hadron2.push_back(
          genjet_subleading_chadron[i] != 0 ? *(genjet_subleading_chadron[i])
                                            : reco::GenParticle());
    }
...
std::vector<reco::GenParticle> GenTopEvent::GetAdditionalBGenJetsHadron()
    const {
  if (!ttxIsFilled)
    std::cerr << "Trying to access GenTopEvent ttX info but it is not filled"
              << std::endl;
...
std::vector<reco::GenParticle> GenTopEvent::GetAdditionalCGenJetsHadron()
    const {
  if (!ttxIsFilled)
    std::cerr << "Trying to access GenTopEvent ttX info but it is not filled"
              << std::endl;
...
std::vector<reco::GenParticle> GenTopEvent::GetAdditionalBGenJetsHadron2()
    const {
  if (!ttxIsFilled)
    std::cerr << "Trying to access GenTopEvent ttX info but it is not filled"
              << std::endl;
...
std::vector<reco::GenParticle> GenTopEvent::GetAdditionalCGenJetsHadron2()
    const {
  if (!ttxIsFilled)
    std::cerr << "Trying to access GenTopEvent ttX info but it is not filled"
              << std::endl;
...
void GenTopEvent::FillTTdecay(
    const std::vector<reco::GenParticle>& prunedGenParticles, int ttXid_) {
  foundT = false;
  foundTbar = false;
  foundH = false;
  ttXid = ttXid_;
  ...
  for (const auto& p : prunedGenParticles) {
      ...
      // for THW
        ...
        for (uint i = 0; i < p.numberOfDaughters(); i++) {
          if (abs(p.daughter(i)->pdgId()) <= 16) {
            w_not_from_top_decay_products.push_back(
    ...
    // for THQ
    else if (abs(p.pdgId()) < 5 && p.isLastCopy() &&
  ...
  //       }
  //   }
  if ((top.energy() < 1 && topbar.energy() < 1) ||
      (wplus.energy() < 1 && wminus.energy() < 1) ||
...
std::vector<reco::GenParticle> GenTopEvent::GetWNotFromTopDecayProducts()
    const {
  assert(isFilled);
  if (!isFilled)
    std::cerr << "Trying to access GenTopEvent but it is not filled"
...
std::vector<reco::GenParticle> GenTopEvent::GetAllLeptons() const {
  ...
  for (auto p = wplus_decay_products.begin();
       isFilled && p != wplus_decay_products.end(); p++) {
    if (abs(p->pdgId()) == 11 || abs(p->pdgId()) == 13 || abs(p->pdgId()) == 15)
      leptons.push_back(*p);
  }
  assert(isFilled);
  for (auto p = wminus_decay_products.begin();
       isFilled && p != wminus_decay_products.end(); p++) {
    if (abs(p->pdgId()) == 11 || abs(p->pdgId()) == 13 || abs(p->pdgId()) == 15)
      leptons.push_back(*p);
  }
  for (int i = 0;
...
std::vector<math::XYZTLorentzVector> GenTopEvent::GetAllTopHadDecayQuarkVecs()
    const {
  return GetLVs(GetAllTopHadDecayQuarks());
}
std::vector<math::XYZTLorentzVector> GenTopEvent::GetAllTopLepDecayQuarkVecs()
    const {
...
std::vector<math::XYZTLorentzVector> GenTopEvent::GetHiggsDecayProductVecs()
    const {
...
std::vector<math::XYZTLorentzVector> GenTopEvent::GetWplusDecayProductsVecs()
    const {
  return GetLVs(GetWplusDecayProducts());
}
std::vector<math::XYZTLorentzVector> GenTopEvent::GetWminusDecayProductsVecs()
    const {
...
std::vector<math::XYZTLorentzVector> GenTopEvent::GetLVs(
    const std::vector<reco::GenParticle>& ps) const {
