...
GenTopEventProducer::GenTopEventProducer(edm::ConsumesCollector&& iC, bool doGenHadronMatch)
{
    if (doGenHadronMatch) {
...
GenTopEvent GenTopEventProducer::Produce(const edm::Event& iEvent, bool doGenHadronMatch, bool returnDummy)
{
    ...
    if (doGenHadronMatch) {
        ...
        genTopEvt.FillTTxDetails(*h_customgenjets, *genBHadIndex, *genBHadJetIndex, *genBHadFlavour, *genBHadFromTopWeakDecay, *genBHadPlusMothers,
                                 *genCHadIndex, *genCHadJetIndex, *genCHadFlavour, *genCHadFromTopWeakDecay, *genCHadPlusMothers, *genCHadBHadronId);
    }
    else {
        genTopEvt.FillTTdecay(*prunedGenParticles, -1);
    }
    return genTopEvt;
}
...
bool GenTopEvent::IsFilled() const { return isFilled; }
bool GenTopEvent::TTxIsFilled() const { return ttxIsFilled; }

void GenTopEvent::FillTTxDetails(const std::vector< reco::GenJet >& customGenJets, const std::vector< int >& genBHadIndex,
                                 const std::vector< int >& genBHadJetIndex, const std::vector< int >& genBHadFlavour,
                                 const std::vector< int >& genBHadFromTopWeakDecay, const std::vector< reco::GenParticle >& genBHadPlusMothers,
                                 const std::vector< int >& genCHadIndex, const std::vector< int >& genCHadJetIndex, const std::vector< int >& genCHadFlavour,
                                 const std::vector< int >& genCHadFromTopWeakDecay, const std::vector< reco::GenParticle >& genCHadPlusMothers,
                                 const std::vector< int >& genCHadBHadronId)
{
    ...
    if (!isFilled) { std::cerr << "fill top info before filling addtional info" << std::endl; }
    ...
    for (uint i = 0; i < genBHadIndex.size(); i++) {
        const reco::GenParticle* bhadron =
            genBHadIndex[i] >= 0 && genBHadIndex[i] < int(genBHadPlusMothers.size()) ? &(genBHadPlusMothers[genBHadIndex[i]]) : 0;
        ...
        if (bhadron != 0 && motherflav == 25) { higgs_b_hadron = *bhadron; }
        ...
        if (genjetidx >= 0 && genjetidx < int(customGenJets.size())) {
            ...
            if (aftertop == 1) { nb_aftertop_per_genjet[genjetidx]++; }

            // find leading hadron of genjets
            if (genjet_leading_bhadron[genjetidx] == 0) { genjet_leading_bhadron[genjetidx] = bhadron; }
            ...
            if (from_tth && (genjet_leading_bhadron_from_tth[genjetidx] == 0 ||
                             (genjet_leading_bhadron_from_tth[genjetidx] != 0 && genjet_leading_bhadron_from_tth[genjetidx]->pt() < bhadron->pt()))) {
    ...
    for (uint i = 0; i < genCHadIndex.size(); i++) {
        const reco::GenParticle* chadron =
            genCHadIndex[i] >= 0 && genCHadIndex[i] < int(genCHadPlusMothers.size()) ? &(genCHadPlusMothers[genCHadIndex[i]]) : 0;
        int genjetidx  = genCHadJetIndex[i];
        int aftertop   = genCHadFromTopWeakDecay[i];
        int motherflav = genCHadFlavour[i];
        if (abs(motherflav) == 24) { w_c_hadron = *chadron; }
        // consider only hadrons not from B decays
            ...
            if (genjetidx >= 0 && genjetidx < int(customGenJets.size())) {
                ...
                if (aftertop == 1) { additionalnc_aftertop_per_genjet[genjetidx]++; }
                // find leading hadron of genjets
                if (genjet_leading_chadron[genjetidx] == 0) { genjet_leading_chadron[genjetidx] = chadron; }
    ...
    for (uint i = 0; i < customGenJets.size(); i++) {
        ...
        if (customGenJets[i].pt() < ttxptcut || fabs(customGenJets[i].eta()) > ttxetacut) { continue; }
        // skip light jets
        if (nb_per_genjet[i] > 0) {
            // associate genjet with tth decay products
            if (mother_of_genjet_b[i] == 25) { higgs_b_genjet = customGenJets[i]; }
        ...
        // no b and no c hadron in jet
            ...
            if (match) { w_genjets.push_back(customGenJets[i]); }
            else {
...
std::vector< reco::GenParticle > GenTopEvent::GetAdditionalBHadrons() const { return additional_b_hadrons; }
std::vector< int >               GenTopEvent::GetAdditionalBHadronMothers() const { return additional_b_hadron_mother; }
std::vector< int >               GenTopEvent::GetAdditionalBHadronAfterTopType() const { return additional_b_hadron_aftertop; }
std::vector< reco::GenParticle > GenTopEvent::GetAdditionalCHadrons() const { return additional_c_hadrons; }
std::vector< int >               GenTopEvent::GetAdditionalCHadronMothers() const { return additional_c_hadron_mother; }
std::vector< int >               GenTopEvent::GetAdditionalCHadronAfterTopType() const { return additional_c_hadron_aftertop; }
...
void GenTopEvent::FillTTdecay(const std::vector< reco::GenParticle >& prunedGenParticles, int ttXid_)
{
    ...
    for (const auto& p : prunedGenParticles) {
        ...
        // find the W bosons from the top decays
            ...
            if (lastW && fromT) {
                if (p.pdgId() == 24)
                    ...
                    wminus = p;
                for (uint i = 0; i < p.numberOfDaughters(); i++) {
                    if (p.pdgId() == 24 && abs(p.daughter(i)->pdgId()) <= 16) { wplus_decay_products.push_back(*(reco::GenParticle*) p.daughter(i)); }
            ...
            // for THW
                ...
                w_not_from_top = p;
                for (uint i = 0; i < p.numberOfDaughters(); i++) {
                    if (abs(p.daughter(i)->pdgId()) <= 16) { w_not_from_top_decay_products.push_back(*(reco::GenParticle*) p.daughter(i)); }
                }
            }
        }
        // find the Higgs boson and its decay products
            ...
            if (lastH) {
                higgs = p;
                for (uint i = 0; i < p.numberOfDaughters(); i++) {
                    if (p.pdgId() == 25 && abs(p.daughter(i)->pdgId()) != 25) { higgs_decay_products.push_back(*(reco::GenParticle*) p.daughter(i)); }
                }
            }
        }
        // for THQ
    ...
    //       }
    //   }
    if ((top.energy() < 1 && topbar.energy() < 1) || (wplus.energy() < 1 && wminus.energy() < 1) ||
        (top_decay_quark.energy() < 1 && topbar_decay_quark.energy() < 1))
...
{
    for (auto p = ps.begin(); p != ps.end(); p++) { PrintParticle(*p); }
}
...
reco::GenParticle GenTopEvent::GetLepton() const
{
    ...
    if (topIsHadronic && !topbarIsHadronic) {
        for (auto p = wminus_decay_products.begin(); p != wminus_decay_products.end(); p++) {
            if (p->pdgId() == 11 || p->pdgId() == 13 || p->pdgId() == 15) return *p;
        }
...
reco::GenParticle GenTopEvent::GetNeutrino() const
{
    ...
    if (topIsHadronic && !topbarIsHadronic) {
        for (auto p = wminus_decay_products.begin(); p != wminus_decay_products.end(); p++) {
            if (abs(p->pdgId()) == 12 || abs(p->pdgId()) == 14 || abs(p->pdgId()) == 16) return *p;
        }
        std::cerr << "no lepton found" << std::endl;
        return reco::GenParticle();
    }
...
std::vector< reco::GenParticle > GenTopEvent::GetQuarks() const
{
    ...
    for (auto p = wplus_decay_products.begin(); p != wplus_decay_products.end(); p++) { quarks.push_back(*p); }
    for (auto p = wminus_decay_products.begin(); p != wminus_decay_products.end(); p++) { quarks.push_back(*p); }
    for (auto p = higgs_decay_products.begin(); p != higgs_decay_products.end(); p++) { quarks.push_back(*p); }
    return quarks;
}
...
std::vector< reco::GenParticle > GenTopEvent::GetAllTopHads() const
{
    ...
    assert(isFilled);
    if (isFilled && topIsHadronic && foundT) tophads.push_back(top);
    if (isFilled && topbarIsHadronic && foundTbar) tophads.push_back(topbar);
    return tophads;
}
// always top first, tobar second if both exist
std::vector< reco::GenParticle > GenTopEvent::GetAllTopLeps() const
{
    ...
    assert(isFilled);
    if (isFilled && !topIsHadronic && foundT) topleps.push_back(top);
    if (isFilled && !topbarIsHadronic && foundTbar) topleps.push_back(topbar);
    return topleps;
}
// always top first, tobar second if both exist
std::vector< reco::GenParticle > GenTopEvent::GetAllTopHadDecayQuarks() const
{
    ...
    std::vector< reco::GenParticle > quarks;
    assert(isFilled);
    if (isFilled && topIsHadronic && foundT) quarks.push_back(top_decay_quark);
    if (isFilled && topbarIsHadronic && foundTbar) quarks.push_back(topbar_decay_quark);
    return quarks;
}
// always top first, tobar second if both exist
...
std::vector< reco::GenParticle > GenTopEvent::GetAllWhads() const
{
    ...
    assert(isFilled);
    if (isFilled && topIsHadronic && foundT) ws.push_back(wplus);
    if (isFilled && topbarIsHadronic && foundTbar) ws.push_back(wminus);
    return ws;
}
// always top first, tobar second if both exist
...
std::vector< reco::GenParticle > GenTopEvent::GetAllLeptons() const
{
    ...
    for (int i = 0; i + (isFilled && !topIsHadronic) + (isFilled && !topbarIsHadronic) < 2; i++) { leptons.push_back(reco::GenParticle()); }
...
std::vector< reco::GenParticle > GenTopEvent::GetAllNeutrinos() const
{
    ...
    for (int i = 0; i + (isFilled && !topIsHadronic) + (isFilled && !topbarIsHadronic) < 2; i++) { neutrinos.push_back(reco::GenParticle()); }
    return neutrinos;
}
// always top first, tobar second if both exist
std::vector< reco::GenParticle > GenTopEvent::GetAllWQuarks() const
{
    ...
    for (int i = 0; i + (isFilled && !topIsHadronic) + (isFilled && !topbarIsHadronic) < 2; i++) { quarks.push_back(reco::GenParticle()); }
    return quarks;
}
// always top first, tobar second if both exist
...
std::vector< math::XYZTLorentzVector > GenTopEvent::GetAllTopHadVecs() const { return GetLVs(GetAllTopHads()); }
std::vector< math::XYZTLorentzVector > GenTopEvent::GetAllTopLepVecs() const { return GetLVs(GetAllTopLeps()); }
std::vector< math::XYZTLorentzVector > GenTopEvent::GetAllTopHadDecayQuarkVecs() const { return GetLVs(GetAllTopHadDecayQuarks()); }
std::vector< math::XYZTLorentzVector > GenTopEvent::GetAllTopLepDecayQuarkVecs() const { return GetLVs(GetAllTopLepDecayQuarks()); }
std::vector< math::XYZTLorentzVector > GenTopEvent::GetAllWhadVecs() const { return GetLVs(GetAllWhads()); }
std::vector< math::XYZTLorentzVector > GenTopEvent::GetAllWlepVecs() const { return GetLVs(GetAllWleps()); }
std::vector< math::XYZTLorentzVector > GenTopEvent::GetAllLeptonVecs() const { return GetLVs(GetAllLeptons()); }
std::vector< math::XYZTLorentzVector > GenTopEvent::GetAllNeutrinoVecs() const { return GetLVs(GetAllNeutrinos()); }
std::vector< math::XYZTLorentzVector > GenTopEvent::GetAllWQuarkVecs() const { return GetLVs(GetAllWQuarks()); }
std::vector< math::XYZTLorentzVector > GenTopEvent::GetAllWAntiQuarkVecs() const { return GetLVs(GetAllWAntiQuarks()); }

math::XYZTLorentzVector                GenTopEvent::GetHiggsVec() const { return GetLV(GetHiggs()); }
math::XYZTLorentzVector                GenTopEvent::GetTopVec() const { return GetLV(GetTop()); }
math::XYZTLorentzVector                GenTopEvent::GetTopBarVec() const { return GetLV(GetTopBar()); }
math::XYZTLorentzVector                GenTopEvent::GetTopHadVec() const { return GetLV(GetTopHad()); }
math::XYZTLorentzVector                GenTopEvent::GetTopLepVec() const { return GetLV(GetTopLep()); }
math::XYZTLorentzVector                GenTopEvent::GetWplusVec() const { return GetLV(GetWplus()); }
math::XYZTLorentzVector                GenTopEvent::GetWminusVec() const { return GetLV(GetWminus()); }
math::XYZTLorentzVector                GenTopEvent::GetWhadVec() const { return GetLV(GetWhad()); }
math::XYZTLorentzVector                GenTopEvent::GetWlepVec() const { return GetLV(GetWlep()); }
math::XYZTLorentzVector                GenTopEvent::GetLeptonVec() const { return GetLV(GetLepton()); }
math::XYZTLorentzVector                GenTopEvent::GetNeutrinoVec() const { return GetLV(GetNeutrino()); }
std::vector< math::XYZTLorentzVector > GenTopEvent::GetQuarkVecs() const { return GetLVs(GetQuarks()); }
std::vector< math::XYZTLorentzVector > GenTopEvent::GetHiggsDecayProductVecs() const { return GetLVs(GetHiggsDecayProducts()); }
math::XYZTLorentzVector                GenTopEvent::GetTopDecayQuarkVec() const { return GetLV(GetTopDecayQuark()); }
math::XYZTLorentzVector                GenTopEvent::GetTopBarDecayQuarkVec() const { return GetLV(GetTopBarDecayQuark()); }
math::XYZTLorentzVector                GenTopEvent::GetTopHadDecayQuarkVec() const { return GetLV(GetTopHadDecayQuark()); }
math::XYZTLorentzVector                GenTopEvent::GetTopLepDecayQuarkVec() const { return GetLV(GetTopLepDecayQuark()); }
std::vector< math::XYZTLorentzVector > GenTopEvent::GetWLeptonsVecs() const { return GetLVs(GetWLeptons()); }
std::vector< math::XYZTLorentzVector > GenTopEvent::GetWQuarksVecs() const { return GetLVs(GetWQuarks()); }
std::vector< math::XYZTLorentzVector > GenTopEvent::GetWplusDecayProductsVecs() const { return GetLVs(GetWplusDecayProducts()); }
std::vector< math::XYZTLorentzVector > GenTopEvent::GetWminusDecayProductsVecs() const { return GetLVs(GetWminusDecayProducts()); }

bool                    GenTopEvent::IsTTbar() const { return isTTbar; }
bool                    GenTopEvent::IsTTH() const { return isTTH; }
bool                    GenTopEvent::IsAllHadron() const { return topIsHadronic && topbarIsHadronic; }
bool                    GenTopEvent::IsDiLepton() const { return !topIsHadronic && !topbarIsHadronic; }
bool                    GenTopEvent::IsSemiLepton() const { return (topIsHadronic && !topbarIsHadronic) || (!topIsHadronic && topbarIsHadronic); }
math::XYZTLorentzVector GenTopEvent::GetLV(const reco::GenParticle& p) const { return math::XYZTLorentzVector(p.px(), p.py(), p.pz(), p.energy()); }
...
std::vector< math::XYZTLorentzVector > GenTopEvent::GetLVs(const std::vector< reco::GenParticle >& ps) const
{
    ...
    for (auto p = ps.begin(); p != ps.end(); p++) { vecs.push_back(GetLV(*p)); }
    return vecs;
}

int GenTopEvent::GetTTxIdFromProducer() const { return ttXid % 100; }

int GenTopEvent::GetTTxIdFromProducerLong() const { return ttXid; }
