...
Synchronizer::~Synchronizer()
{
    ...
    for (auto f = dumpFiles.begin(); f != dumpFiles.end(); f++) { (*f)->close(); }
    cout << "Systematics used: ";
    for (const auto& s : systematics) { cout << s; }
    cout << endl;
    cout << "Cutflows SL: ";
    for (auto& c : cutflowsSL) { c.Print(cout); }
    cout << endl;
    cout << "Cutflows DL: ";
    for (auto& c : cutflowsDL) { c.Print(cout); }
    cout << endl;
    for (auto& sel : selectionsSL) { delete sel; }
    for (auto& sel : selectionsDL) { delete sel; }
}
...
void Synchronizer::DumpSyncExe(const InputCollections& input, std::ostream& out, Cutflow& cutflowSL, Cutflow& cutflowDL, bool addExtendedInfo,
                               std::vector< int > dumpAlwaysEvents)
{
    ...
    trig_elel = input.triggerInfo.IsTriggered("HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL_v*") ||
                input.triggerInfo.IsTriggered("HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL_DZ_v*");
...
%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f") %
                   run % lumi % event % is_e % is_mu % is_ee % is_emu % is_mumu % n_jets % n_btags % lep1_pt % lep1_eta % lep1_iso % lep1_pdgId % lep1_idSF %
                   lep1_isoSF % lep1_seed % lep2_pt % lep2_eta % lep2_iso % lep2_pdgId % lep2_idSF % lep2_isoSF % lep2_seed % jet1_pt % jet1_eta % jet1_phi %
                   jet1_jesSF % jet1_jesSF_up % jet1_jesSF_down % jet1_jesSF_PileUpDataMC_down % jet1_jesSF_RelativeFSR_up % jet1_jerSF_nominal % jet1_csv %
                   jet1_PUJetId % jet1_PUJetDiscriminant % jet1_seed % jet2_pt % jet2_eta % jet2_phi % jet2_jesSF % jet2_jesSF_up % jet2_jesSF_down %
                   jet2_jesSF_PileUpDataMC_down % jet2_jesSF_RelativeFSR_up % jet2_jerSF_nominal % jet2_csv % jet2_PUJetId % jet2_PUJetDiscriminant %
                   jet2_seed % MET_pt % MET_phi % mll % ttHFCategory % ttHFGenFilterTag % n_interactions % puWeight % csvSF % csvSF_lf_up % csvSF_hf_down %
                   csvSF_cErr1_down % pdf_up % pdf_down % me_up % me_down % isr_up % isr_down % fsr_up % fsr_down % triggerSF % top_pt_weight % bdt_output %
                   dnn_ttH_output % dnn_ttbb_output;
        if (addExtendedInfo) {
            out << boost::format(",%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,"
                                 "%.4f,%.4f,%.4f") %
                       jet3_pt % jet3_eta % jet3_csv % jet4_pt % jet4_eta % jet4_csv % jet5_pt % jet5_eta % jet5_csv % jet6_pt % jet6_eta % jet6_csv % jet7_pt %
                       jet7_eta % jet7_csv;
            ...
            out << boost::format(",%.4f,%.4f,%.4f,%i,%.4f,%.4f,%.4f,%i") % raw_el1_pt % raw_el1_eta % raw_el1_iso % raw_el1_pdgId % raw_el2_pt % raw_el2_eta %
                       raw_el2_iso % raw_el2_pdgId;
            out << boost::format(",%.4f,%.4f,%.4f,%i,%.4f,%.4f,%.4f,%i") % raw_mu1_pt % raw_mu1_eta % raw_mu1_iso % raw_mu1_pdgId % raw_mu2_pt % raw_mu2_eta %
                       raw_mu2_iso % raw_mu2_pdgId;
            out << boost::format(",%i,%i,%i,%i,%i") % pass_FilterSelection % pass_VertexSelection % pass_LeptonSelection % pass_DiLeptonSelection %
                       pass_METSelection;
        }
        out << "\n";
    }
...
{
    for (uint i = 0; i < inputs.size(); i++) { DumpSyncExe(inputs[i], *(dumpFiles[i]), cutflowsSL[i], cutflowsDL[i], addExtendedInfo, dumpAlwaysEvents); }
}
                        ...
                        LeptonSFHelper* leptonsfhelper_, bool dumpExtended)
{
    ...
    for (auto& c : cutflowsDL) {
        for (auto& s : selectionsDL) { s->InitCutflow(c); }
    }
    ...
    for (auto& c : cutflowsSL) {
        for (auto& s : selectionsSL) { s->InitCutflow(c); }
    }
