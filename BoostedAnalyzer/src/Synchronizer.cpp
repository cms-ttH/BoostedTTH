...
void Synchronizer::DumpSyncExe(const InputCollections& input,
	...
	jet1_csv=CSVHelper::GetJetCSV(input.selectedJets.at(0));
	...
	jet2_csv=CSVHelper::GetJetCSV(input.selectedJets.at(1));
    ...
    if(is_SL) {
        ...
        for(auto j=input.selectedJets.begin(); j!=input.selectedJets.end(); j++){
            jetcsvs.push_back(CSVHelper::GetJetCSV(*j));
            jetcsvs_dnn.push_back(CSVHelper::GetJetCSV_DNN(*j));
        }
        for(auto j=input.selectedJetsLoose.begin(); j!=input.selectedJetsLoose.end(); j++){
            loose_jetcsvs.push_back(CSVHelper::GetJetCSV(*j));
        }
        double out_P_4b=-1;
        double out_P_2b=-1;
        double eth_blr=-1;
