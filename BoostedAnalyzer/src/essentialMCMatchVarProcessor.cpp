...
void essentialMCMatchVarProcessor::Process(const InputCollections& input, VariableContainer& vars)
{
    ...
    if (input.genTopEvt.IsFilled()) { vars.FillVar("GenEvt_TTxId_FromProducer", input.genTopEvt.GetTTxIdFromProducer()); }
    int nGoodTagsM = 0;
    int nMisTagsM  = 0;
    ...
    if (input.genTopEvt.IsFilled() && input.genTopEvt.TTxIsFilled() && input.genTopEvt.IsSemiLepton()) {
        ...
        for (auto j = input.selectedJets.begin(); j != input.selectedJets.end(); j++) {
            ...
            if (std::abs(BoostedUtils::DeltaR(genj->p4(), bhiggs_genjet1.p4())) < dR_threshold ||
                std::abs(BoostedUtils::DeltaR(genj->p4(), bhiggs_genjet2.p4())) < dR_threshold) {
                if (first_higgs_jet) {
                    // has_bH
