...
void essentialMCMatchVarProcessor::Init(const InputCollections& input,
...
void essentialMCMatchVarProcessor::Process(const InputCollections& input,
                                           VariableContainer& vars) {
  ...
  if (input.genTopEvt.IsFilled()) {
    vars.FillVar("GenEvt_TTxId_FromProducer",
  ...
  // if(bhiggs_quarkss.size()>2)std::cout<<"MORE THAN TWO HIGGS
  // PRODUCTS"<<std::endl;
  bool dfirst = true;
  ...
  //
  //     for(std::vector<math::XYZTLorentzVector>::iterator itJetVec =
  //     jetvecs.begin() ; itJetVec != jetvecs.end(); ++itJetVec){
  ...
  if (input.genTopEvt.IsFilled() && input.genTopEvt.TTxIsFilled() &&
      input.genTopEvt.IsSemiLepton()) {
    // get gen particles
    // b quark from had top
    std::vector<reco::GenParticle> bhad_quark =
        input.genTopEvt.GetAllTopHadDecayQuarks();
    // if(bhad_quark.size() > 1){ std::cout<<"MORE THAN ONE B FROM
    // HADTOP"<<std::endl; }

    // b quark from lep top
    std::vector<reco::GenParticle> blep_quark =
        input.genTopEvt.GetAllTopLepDecayQuarks();
    // if(blep_quark.size() > 1){ std::cout<<"MORE THAN ONE B FROM
    // LEPTOP"<<std::endl; }

    // b quarks from higgs
    std::vector<reco::GenParticle> bhiggs_quarks =
    ...
    // if(!(hadw_quarks.size() == 2)){ std::cout<<"UNEQUAL TWO QUARKS FROM
    // HADRONIC W"<<std::endl; }

    // get gen jets
    // b jet from had top
    std::vector<reco::GenJet> bhad_genjet =
        input.genTopEvt.GetAllTopHadBGenJets();
    // if(bhad_genjet.size() > 1){ std::cout<<"MORE THAN ONE bJET FROM
    // HADTOP"<<std::endl; }

    // b jet from lep top
    std::vector<reco::GenJet> blep_genjet =
        input.genTopEvt.GetAllTopLepBGenJets();
    // if(blep_genjet.size() > 1){ std::cout<<"MORE THAN ONE bJET FROM
    // HADTOP"<<std::endl; }
    ...
    // if(!(hadw_genjets.size() == 2)){ std::cout<<"UNEQUAL TWO JETS FROM
    // HADRONIC W"<<std::endl; }
    ...
    // ---------------------------------------------------------------------------------------------------
    // loop over reco jets, get GenParticle, perform dR Matching with
    // GenParticle Objects from genTopEvent
    for (auto j = input.selectedJets.begin(); j != input.selectedJets.end();
         j++) {
      ...
      for (uint i = 0; i < bhad_quark.size(); i++) {
        if (std::abs(BoostedUtils::DeltaR(genp->p4(), bhad_quark[i].p4())) <
            dR_threshold) {
          // has_bhad
      ...
      for (uint i = 0; i < blep_quark.size(); i++) {
        if (std::abs(BoostedUtils::DeltaR(genp->p4(), blep_quark[i].p4())) <
            dR_threshold) {
          // has_blep
      ...
      for (uint i = 0; i < hadw_quarks.size(); i++) {
        if (std::abs(BoostedUtils::DeltaR(genp->p4(), hadw_quarks[i].p4())) <
            dR_threshold) {
          if (first_W_to_q) {
            // has_lj
      ...
      for (uint i = 0; i < bhiggs_quarks.size(); i++) {
        if (std::abs(BoostedUtils::DeltaR(genp->p4(), bhiggs_quarks[i].p4())) <
            dR_threshold) {
          if (first_higgs_to_b) {
            // has_bH
    ...
    // math::XYZTLorentzVector w_vec = hadw_quarks[0].p4() +
    // hadw_quarks[1].p4(); float inv_w_mass = w_vec.M();
    ...
    jet_was_matched = false;
    // ---------------------------------------------------------------------------------------------------
    // loop over reco jets, get GenJet, perform dR Matching with GenJet Objects
    // from genTopEvent
    for (auto j = input.selectedJets.begin(); j != input.selectedJets.end();
         j++) {
      jet_was_matched = false;
      ...
      for (uint i = 0; i < bhad_genjet.size(); i++) {
        if (std::abs(BoostedUtils::DeltaR(genj->p4(), bhad_genjet[i].p4())) <
            dR_threshold) {
          // has_bhad
      ...
      for (uint i = 0; i < blep_genjet.size(); i++) {
        if (std::abs(BoostedUtils::DeltaR(genj->p4(), blep_genjet[i].p4())) <
            dR_threshold) {
          // has_blep
      ...
      for (uint i = 0; i < hadw_genjets.size(); i++) {
        if (std::abs(BoostedUtils::DeltaR(genj->p4(), hadw_genjets[i].p4())) <
            dR_threshold) {
          if (first_Wjet) {
            // has_lj
      ...
      if (std::abs(BoostedUtils::DeltaR(genj->p4(), bhiggs_genjet1.p4())) <
              dR_threshold ||
          std::abs(BoostedUtils::DeltaR(genj->p4(), bhiggs_genjet2.p4())) <
              dR_threshold) {
        if (first_higgs_jet) {
          // has_bH
      ...
      if (!(jet_was_matched) &&
          CSVHelper::PassesCSV(*genj, "DeepJet", CSVHelper::CSVwp::Medium,
                               input.era) &&
          iBB > 0) {
