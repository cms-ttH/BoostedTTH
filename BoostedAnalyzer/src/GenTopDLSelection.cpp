...
void GenTopDLSelection::InitCutflow(Cutflow& cutflow)
{
...
bool GenTopDLSelection::IsSelected(const InputCollections& input, Cutflow& cutflow)
{
    if (!initialized) cerr << "GenTopDLSelection not initialized" << endl;
    ...
    if (!(tophad.size() == 0 and toplep.size() == 2)) { return false; }
    else {
