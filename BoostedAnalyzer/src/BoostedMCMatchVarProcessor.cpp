...
void BoostedMCMatchVarProcessor::Init(const InputCollections& input,
...
void BoostedMCMatchVarProcessor::Process(const InputCollections& input,
                                         VariableContainer& vars) {
  ...
  if (input.genTopEvt.IsFilled() && input.genTopEvt.TTxIsFilled()) {
    ...
    if (additional_bs.size() > 0) b1 = additional_bs[0];
    if (additional_bs.size() > 1) b2 = additional_bs[1];
  }
  ...
  for (size_t i = 0; i < input.selectedBoostedJets.size(); i++) {
    if (higgs.pt() > 0)
      vars.FillVars("BoostedJet_Dr_GenHiggs", i,
                    BoostedUtils::DeltaR(
                        higgs.p4(), input.selectedBoostedJets[i].fatjet.p4()));

    if (b1.pt() > 0.) {
      vars.FillVars("BoostedJet_Dr_GenB1", i,
                    BoostedUtils::DeltaR(
                        b1.p4(), input.selectedBoostedJets[i].fatjet.p4()));
      if (b2.pt() > 0.)
        vars.FillVars("BoostedJet_Dr_GenB2", i,
                      BoostedUtils::DeltaR(
      ...
      for (size_t j = 0; j < input.selectedBoostedJets[i].subjets.size(); j++) {
        ...
        vars.FillVars(
            "BoostedJet_Dr_Subjet" + to_string(j + 1) + "_GenB1", i,
            BoostedUtils::DeltaR(input.selectedBoostedJets[i].subjets[j].p4(),
                                 b1.p4()));
        vars.FillVars(
            "BoostedJet_Dr_Subjet" + to_string(j + 1) + "_GenB2", i,
            BoostedUtils::DeltaR(input.selectedBoostedJets[i].subjets[j].p4(),
                                 b2.p4()));
      }
      ...
      for (size_t j = 0; j < input.selectedBoostedJets[i].filterjets.size();
        ...
        vars.FillVars(
            "BoostedJet_Dr_Filterjet" + to_string(j + 1) + "_GenB1", i,
            BoostedUtils::DeltaR(
                input.selectedBoostedJets[i].filterjets[j].p4(), b1.p4()));
        vars.FillVars(
            "BoostedJet_Dr_Filterjet" + to_string(j + 1) + "_GenB2", i,
            BoostedUtils::DeltaR(
                input.selectedBoostedJets[i].filterjets[j].p4(), b2.p4()));
      }
      ...
      for (size_t j = 0; j < input.selectedBoostedJets[i].prunedsubjets.size();
        ...
        vars.FillVars(
            "BoostedJet_Dr_PrunedSubjet" + to_string(j + 1) + "_GenB1", i,
            BoostedUtils::DeltaR(
                input.selectedBoostedJets[i].prunedsubjets[j].p4(), b1.p4()));
        vars.FillVars(
            "BoostedJet_Dr_PrunedSubjet" + to_string(j + 1) + "_GenB2", i,
            BoostedUtils::DeltaR(
                input.selectedBoostedJets[i].prunedsubjets[j].p4(), b2.p4()));
      }
      ...
      for (size_t j = 0; j < input.selectedBoostedJets[i].sdsubjets.size();
        ...
        vars.FillVars(
            "BoostedJet_Dr_SoftdropSubjet" + to_string(j + 1) + "_GenB1", i,
            BoostedUtils::DeltaR(input.selectedBoostedJets[i].sdsubjets[j].p4(),
                                 b1.p4()));
        vars.FillVars(
            "BoostedJet_Dr_SoftdropSubjet" + to_string(j + 1) + "_GenB2", i,
            BoostedUtils::DeltaR(input.selectedBoostedJets[i].sdsubjets[j].p4(),
                                 b2.p4()));
      }

      if (minDrB1_Softdrop < 999)
        vars.FillVars("BoostedJet_Dr_GenB1_SoftdropSubjet", i,
                      minDrB1_Softdrop);
      if (minDrB2_Softdrop < 999)
        vars.FillVars("BoostedJet_Dr_GenB2_SoftdropSubjet", i,
                      minDrB2_Softdrop);
    }
      ...
      if (Dr_temp < minDr_WHad) {
        ...
        for (size_t k = 0; k < input.selectedBoostedJets[i].filterjets.size();
          ...
          if (minDr_Filterjet_Q[k] < 999)
            vars.FillVars("BoostedJet_Dr_Filterjet" + to_string(k + 1) + "_Q",
                          i, minDr_Filterjet_Q[k]);
        }

        // Pruned subjets
        float minDr_PrunedSubjet_Q[2] = {999., 999.};
        for (size_t k = 0;
             k < input.selectedBoostedJets[i].prunedsubjets.size(); k++) {
          ...
          if (minDr_PrunedSubjet_Q[k] < 999)
            vars.FillVars(
                "BoostedJet_Dr_PrunedSubjet" + to_string(k + 1) + "_Q", i,
                minDr_PrunedSubjet_Q[k]);
        }

        // Soft drop subjets
        float minDr_SoftdropSubjet_Q[input.selectedBoostedJets[i]
                                         .sdsubjets.size()];
        for (size_t j = 0; j < input.selectedBoostedJets[i].sdsubjets.size();
             j++)
        ...
        for (size_t k = 0; k < input.selectedBoostedJets[i].sdsubjets.size();
          ...
          if (minDr_SoftdropSubjet_Q[k] < 999)
            vars.FillVars(
                "BoostedJet_Dr_SoftdropSubjet" + to_string(k + 1) + "_Q", i,
                minDr_SoftdropSubjet_Q[k]);
        }
      }
    }
    ...
    if (minDr_Q1_PrunedSubjet < 999)
      vars.FillVars("BoostedJet_Dr_GenQ1_PrunedSubjet", i,
                    minDr_Q1_PrunedSubjet);
    if (minDr_Q2_PrunedSubjet < 999)
      vars.FillVars("BoostedJet_Dr_GenQ2_PrunedSubjet", i,
                    minDr_Q2_PrunedSubjet);
    if (minDr_Q1_SoftdropSubjet < 999)
      vars.FillVars("BoostedJet_Dr_GenQ1_SoftdropSubjet", i,
                    minDr_Q1_SoftdropSubjet);
    if (minDr_Q2_SoftdropSubjet < 999)
      vars.FillVars("BoostedJet_Dr_GenQ2_SoftdropSubjet", i,
                    minDr_Q2_SoftdropSubjet);
    ...
    for (size_t j = 0; j < tophad.size(); j++) {
      if (filterJets.size() > 0 && filterJets[0].pt() > 0) {
        dr_GenQ1_CSV1 =
            fmin(BoostedUtils::DeltaR(filterJets[0].p4(), q1[j].p4()),
                 dr_GenQ1_CSV1);
        dr_GenQ2_CSV1 =
            fmin(BoostedUtils::DeltaR(filterJets[0].p4(), q2[j].p4()),
                 dr_GenQ2_CSV1);
        dr_GenB_CSV1 =
            fmin(BoostedUtils::DeltaR(filterJets[0].p4(), bhad[j].p4()),
                 dr_GenB_CSV1);
      }
      if (filterJets.size() > 1 && filterJets[1].pt() > 0) {
        dr_GenQ1_CSV2 =
            fmin(BoostedUtils::DeltaR(filterJets[1].p4(), q1[j].p4()),
                 dr_GenQ1_CSV2);
        dr_GenQ2_CSV2 =
            fmin(BoostedUtils::DeltaR(filterJets[1].p4(), q2[j].p4()),
                 dr_GenQ2_CSV2);
        dr_GenB_CSV2 =
            fmin(BoostedUtils::DeltaR(filterJets[1].p4(), bhad[j].p4()),
                 dr_GenB_CSV2);
      }
    }
