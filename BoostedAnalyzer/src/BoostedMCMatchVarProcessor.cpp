...
void BoostedMCMatchVarProcessor::Process(const InputCollections& input, VariableContainer& vars)
{
    ...
    for (size_t i = 0; i < input.selectedBoostedJets.size(); i++) {
        ...
        if (minDr_TopLep < 999) { vars.FillVars("BoostedJet_Dr_GenTopLep", i, minDr_TopLep); }
    }
    ...
    for (size_t i = 0; i < input.selectedBoostedJets.size(); i++) {
        if (higgs.pt() > 0) vars.FillVars("BoostedJet_Dr_GenHiggs", i, BoostedUtils::DeltaR(higgs.p4(), input.selectedBoostedJets[i].fatjet.p4()));
        ...
        if (b1.pt() > 0.) {
            ...
            for (size_t j = 0; j < input.selectedBoostedJets[i].subjets.size(); j++) {
                ...
                vars.FillVars("BoostedJet_Dr_Subjet" + to_string(j + 1) + "_GenB1", i,
                              BoostedUtils::DeltaR(input.selectedBoostedJets[i].subjets[j].p4(), b1.p4()));
                vars.FillVars("BoostedJet_Dr_Subjet" + to_string(j + 1) + "_GenB2", i,
                              BoostedUtils::DeltaR(input.selectedBoostedJets[i].subjets[j].p4(), b2.p4()));
            }
