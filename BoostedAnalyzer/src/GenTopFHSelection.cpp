...
bool GenTopFHSelection::IsSelected(const InputCollections& input, Cutflow& cutflow)
{
    ...
    if (!(tophad.size() == 2 and toplep.size() == 0)) { return false; }
    else {
        cutflow.EventSurvivedStep("Event is FullHadronic ttbar", input.weights.at("Weight"));
        return true;
    }
}
