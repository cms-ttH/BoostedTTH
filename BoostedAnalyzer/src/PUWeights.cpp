...
PUWeights::PUWeights()
{
  ...
  nominalWeightIsInit_ =
    false; // this means, it returns the default of 1 as set above
}

void
PUWeights::init(const edm::ParameterSet& iConfig)
{
  if (iConfig.existsAs<edm::ParameterSet>("nominalPUWeight", true)) {
    const edm::ParameterSet nominalPUWeightPS =
      iConfig.getParameter<edm::ParameterSet>("nominalPUWeight");
    initNominalWeight(
      nominalPUWeightPS.getParameter<std::string>("fileNameMCNPU"),
  ...
  if (iConfig.existsAs<edm::VParameterSet>("additionalPUWeights", true)) {
    const edm::VParameterSet additionalPUWeightsPS =
      iConfig.getParameter<edm::VParameterSet>("additionalPUWeights");
    for (edm::VParameterSet::const_iterator iPS = additionalPUWeightsPS.begin();
         iPS != additionalPUWeightsPS.end();
         ++iPS) {
      initAdditionalWeight(
        iPS->getParameter<std::string>("namePUWeight"),
...
void
PUWeights::initNominalWeight(const std::string& fileNameMCNPU,
                             ...
                             const std::string& histNameDataNPUEstimated)
{
  ...
  nominalWeight_.producer_.initWeights(fileNameMCNPU,
                                       histNameMCNPU,
                                       fileNameDataNPUEstimated,
                                       histNameDataNPUEstimated);
  nominalWeightIsInit_ = true;
}

void
PUWeights::initAdditionalWeight(const std::string& name,
                                ...
                                const std::string& histNameDataNPUEstimated)
{
  ...
  std::cout << "Initializing additional PU weight '" << name << "'"
            << std::endl;
  PUWeights::Weight weight;
  weight.name_ = name;
  weight.producer_.initWeights(fileNameMCNPU,
                               histNameMCNPU,
                               fileNameDataNPUEstimated,
                               histNameDataNPUEstimated);
