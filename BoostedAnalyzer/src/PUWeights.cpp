...
void PUWeights::init(const edm::ParameterSet& iConfig)
{
    if (iConfig.existsAs< edm::ParameterSet >("nominalPUWeight", true)) {
        ...
        initNominalWeight(nominalPUWeightPS.getParameter< std::string >("fileNameMCNPU"), nominalPUWeightPS.getParameter< std::string >("histNameMCNPU"),
    ...
    if (iConfig.existsAs< edm::VParameterSet >("additionalPUWeights", true)) {
        ...
        for (edm::VParameterSet::const_iterator iPS = additionalPUWeightsPS.begin(); iPS != additionalPUWeightsPS.end(); ++iPS) {
            initAdditionalWeight(iPS->getParameter< std::string >("namePUWeight"), iPS->getParameter< std::string >("fileNameMCNPU"),
                                 iPS->getParameter< std::string >("histNameMCNPU"), iPS->getParameter< std::string >("fileNameDataNPUEstimated"),
                                 iPS->getParameter< std::string >("histNameDataNPUEstimated"));
        }
    }
}

void PUWeights::initNominalWeight(const std::string& fileNameMCNPU, const std::string& histNameMCNPU, const std::string& fileNameDataNPUEstimated,
                                  const std::string& histNameDataNPUEstimated)
{
    if (nominalWeightIsInit_) { throw cms::Exception("PU weight already initialized") << "Trying to set up second producer for nominal weight"; }
...
void PUWeights::initAdditionalWeight(const std::string& name, const std::string& fileNameMCNPU, const std::string& histNameMCNPU,
                                     const std::string& fileNameDataNPUEstimated, const std::string& histNameDataNPUEstimated)
{
    for (const auto weight : additionalWeights_) {
        if (name == weight.name()) {
            throw cms::Exception("PU weight already initialized")
                << "Trying to set up second producer with '" << name << "' for" << fileNameDataNPUEstimated << "' and '" << fileNameMCNPU << "'";
        }
    }
...
{
    if (nominalWeightIsInit_) { nominalWeight_.compute(npu); }
    for (std::vector< PUWeights::Weight >::iterator it = additionalWeights_.begin(); it != additionalWeights_.end(); ++it) { it->compute(npu); }
}
