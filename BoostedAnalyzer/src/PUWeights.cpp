...
void PUWeights::init(const edm::ParameterSet& iConfig) {
  if (iConfig.existsAs<edm::ParameterSet>("nominalPUWeight", true)) {
    ...
    initNominalWeight(
        ...
        nominalPUWeightPS.getParameter<std::string>(
...
void PUWeights::initNominalWeight(const std::string& fileNameMCNPU,
  ...
  nominalWeight_.producer_.initWeights(fileNameMCNPU, histNameMCNPU,
                                       fileNameDataNPUEstimated,
                                       histNameDataNPUEstimated);
  nominalWeightIsInit_ = true;
}

void PUWeights::initAdditionalWeight(
    const std::string& name, const std::string& fileNameMCNPU,
    const std::string& histNameMCNPU,
    const std::string& fileNameDataNPUEstimated,
    const std::string& histNameDataNPUEstimated) {
  ...
  weight.name_ = name;
  weight.producer_.initWeights(fileNameMCNPU, histNameMCNPU,
                               fileNameDataNPUEstimated,
                               histNameDataNPUEstimated);
