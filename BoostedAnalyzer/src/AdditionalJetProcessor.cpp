...
void AdditionalJetProcessor::Init(const InputCollections& input,
  ...
  vars.InitVars("AdditionalLightGenJet_RecoJetPt", -9.,
                "N_AdditionalLightGenJets");
  vars.InitVars("AdditionalLightGenJet_RecoJetCSV", -9.,
                "N_AdditionalLightGenJets");
...
const pat::Jet* AdditionalJetProcessor::GetPatJet(
    const reco::GenJet& genjet, const std::vector<pat::Jet>& patjets) {
  for (auto patjet = patjets.begin(); patjet != patjets.end(); patjet++) {
    if (patjet->pt() > 0 && genjet.pt() > 0 &&
        BoostedUtils::DeltaR(patjet->p4(), genjet.p4()) < 0.2) {
      return &(*patjet);
      break;
    }
  }
...
void AdditionalJetProcessor::Process(const InputCollections& input,
                                     VariableContainer& vars) {
  ...
  if (input.genTopEvt.TTxIsFilled()) {
    ...
    std::vector<reco::GenJet> additional_c_genjets =
        input.genTopEvt.GetAdditionalCGenJets();
    std::vector<reco::GenJet> additional_light_genjets =
        input.genTopEvt.GetAdditionalLightGenJets();
    ...
    for (auto genbjet = additional_b_genjets.begin();
         genbjet != additional_b_genjets.end(); genbjet++) {
      additional_b_genjets_recojets.push_back(
          GetPatJet(*genbjet, input.selectedJets));
    }
    for (auto gencjet = additional_c_genjets.begin();
         gencjet != additional_c_genjets.end(); gencjet++) {
      additional_c_genjets_recojets.push_back(
          GetPatJet(*gencjet, input.selectedJets));
    }
    for (auto genjet = additional_light_genjets.begin();
         genjet != additional_light_genjets.end(); genjet++) {
      additional_light_genjets_recojets.push_back(
          GetPatJet(*genjet, input.selectedJets));
    }

    std::vector<int> additional_b_genjets_fromtop =
        input.genTopEvt.GetAdditionalBGenJetsFromTopType();
    std::vector<int> additional_c_genjets_fromtop =
        input.genTopEvt.GetAdditionalCGenJetsFromTopType();
    std::vector<int> additional_b_genjets_nhadrons =
        input.genTopEvt.GetAdditionalBGenJetsNHadrons();
    std::vector<int> additional_c_genjets_nhadrons =
        input.genTopEvt.GetAdditionalCGenJetsNHadrons();

    std::vector<reco::GenParticle> additional_b_genjets_hadron =
        input.genTopEvt.GetAdditionalBGenJetsHadron();
    std::vector<reco::GenParticle> additional_c_genjets_hadron =
        input.genTopEvt.GetAdditionalCGenJetsHadron();
    std::vector<reco::GenParticle> additional_b_genjets_hadron2 =
        input.genTopEvt.GetAdditionalBGenJetsHadron2();
    std::vector<reco::GenParticle> additional_c_genjets_hadron2 =
        input.genTopEvt.GetAdditionalCGenJetsHadron2();
    ...
    if (input.genTopEvt.IsFilled()) {
      ...
      std::vector<reco::GenParticle> bhads =
          input.genTopEvt.GetAllTopHadDecayQuarks();
      std::vector<reco::GenParticle> bleps =
          input.genTopEvt.GetAllTopLepDecayQuarks();
      std::vector<reco::GenParticle> bhiggs =
          input.genTopEvt.GetHiggsDecayProducts();
    ...
    for (uint i = 0; i < additional_b_genjets.size(); i++) {
      ...
      vars.FillVars("AdditionalGenBJet_FromTopType", i,
                    additional_b_genjets_fromtop[i]);
      vars.FillVars("AdditionalGenBJet_NHadrons", i,
                    additional_b_genjets_nhadrons[i]);
      vars.FillVars("AdditionalGenBJet_HadronPt", i,
                    additional_b_genjets_hadron[i].pt());
      vars.FillVars("AdditionalGenBJet_RecoJetPt", i,
                    additional_b_genjets_recojets[i] != 0
                        ? additional_b_genjets_recojets[i]->pt()
                        : -1);
      vars.FillVars(
          "AdditionalGenBJet_RecoJetCSV", i,
          additional_b_genjets_recojets[i] != 0
              ? CSVHelper::GetJetCSV(*additional_b_genjets_recojets[i], btagger)
              : -2);
      vars.FillVars("AdditionalGenBJet_HadronId", i,
                    additional_b_genjets_hadron[i].pdgId());
      float drB = 5;
      for (uint k = 0; k < btth.size(); k++) {
        if (abs(btth[k].pdgId()) == 5 && btth[k].pt() > 1 &&
            additional_b_genjets[i].pt() > 1) {
          float dr =
              BoostedUtils::DeltaR(btth[k].p4(), additional_b_genjets[i].p4());
          if (dr > 0 && dr < drB) {
            drB = dr;
          }
        }
      ...
      for (uint k = 0; k < qtth.size(); k++) {
        if (qtth[k].pt() > 1 && additional_b_genjets[i].pt() > 1) {
          float dr =
              BoostedUtils::DeltaR(qtth[k].p4(), additional_b_genjets[i].p4());
          if (dr > 0 && dr < drQ) {
            drQ = dr;
          }
        }
    ...
    for (uint i = 0; i < additional_c_genjets.size(); i++) {
      ...
      vars.FillVars("AdditionalGenCJet_FromTopType", i,
                    additional_c_genjets_fromtop[i]);
      vars.FillVars("AdditionalGenCJet_NHadrons", i,
                    additional_c_genjets_nhadrons[i]);
      vars.FillVars("AdditionalGenCJet_HadronPt", i,
                    additional_c_genjets_hadron[i].pt());
      vars.FillVars("AdditionalGenCJet_RecoJetPt", i,
                    additional_c_genjets_recojets[i] != 0
                        ? additional_c_genjets_recojets[i]->pt()
                        : -1);
      vars.FillVars(
          "AdditionalGenCJet_RecoJetCSV", i,
          additional_c_genjets_recojets[i] != 0
              ? CSVHelper::GetJetCSV(*additional_c_genjets_recojets[i], btagger)
              : -2);
      vars.FillVars("AdditionalGenCJet_HadronId", i,
                    additional_c_genjets_hadron[i].pdgId());
      float drQ = 5;
      for (uint k = 0; k < q1.size(); k++) {
        if (abs(q1[k].pdgId()) == 4 && q1[k].pt() > 1 &&
            additional_c_genjets[i].pt() > 0) {
          float dr =
              BoostedUtils::DeltaR(q1[k].p4(), additional_c_genjets[i].p4());
          if (dr > 0 && dr < drQ) {
            drQ = dr;
          }
        }
      ...
      for (uint k = 0; k < q2.size(); k++) {
        if (abs(q2[k].pdgId()) == 4 && q2[k].pt() > 1 &&
            additional_c_genjets[i].pt() > 0) {
          float dr =
              BoostedUtils::DeltaR(q2[k].p4(), additional_c_genjets[i].p4());
          if (dr > 0 && dr < drQ) {
            drQ = dr;
          }
        }
    ...
    for (uint i = 0; i < additional_light_genjets.size(); i++) {
      vars.FillVars("AdditionalLightGenJet_Pt", i,
                    additional_light_genjets[i].pt());
      vars.FillVars("AdditionalLightGenJet_Eta", i,
                    additional_light_genjets[i].eta());
      vars.FillVars("AdditionalLightGenJet_Phi", i,
                    additional_light_genjets[i].phi());
      vars.FillVars("AdditionalLightGenJet_E", i,
                    additional_light_genjets[i].energy());
      vars.FillVars("AdditionalLightGenJet_RecoJetPt", i,
                    additional_light_genjets_recojets[i] != 0
                        ? additional_light_genjets_recojets[i]->pt()
                        : -1);
      vars.FillVars("AdditionalLightGenJet_RecoJetCSV", i,
                    additional_light_genjets_recojets[i] != 0
                        ? CSVHelper::GetJetCSV(
                              *additional_light_genjets_recojets[i], btagger)
                        : -2);
    }

    vars.FillVar("GenEvt_TTxId_FromGenTopEvt", input.genTopEvt.GetTTxId());
    vars.FillVar("GenEvt_TTxId_w_PseudoAdditional",
                 input.genTopEvt.GetTTxId(true));
    vars.FillVar("GenEvt_TTxId_wo_PseudoAdditional",
                 input.genTopEvt.GetTTxId(false));

    std::vector<reco::GenParticle> bhadrons =
        input.genTopEvt.GetAdditionalBHadrons();
    std::vector<int> bmothers = input.genTopEvt.GetAdditionalBHadronMothers();
    std::vector<int> btopdecaytype =
        input.genTopEvt.GetAdditionalBHadronAfterTopType();
    ...
    std::vector<reco::GenParticle> chadrons =
        input.genTopEvt.GetAdditionalCHadrons();
    std::vector<int> cmothers = input.genTopEvt.GetAdditionalCHadronMothers();
    std::vector<int> ctopdecaytype =
        input.genTopEvt.GetAdditionalCHadronAfterTopType();
    ...
    if (additional_b_genjets.size() > 1 &&
      ...
      if (hadr1.pt() > 0. && hadr2.pt() > 0.)
        drbhad = BoostedUtils::DeltaR(hadr1.p4(), hadr2.p4());
      mbhadjets =
          (additional_b_genjets[0].p4() + additional_b_genjets[1].p4()).M();
      drbhadjets = BoostedUtils::DeltaR(additional_b_genjets[0].p4(),
                                        additional_b_genjets[1].p4());
    }
    if (additional_c_genjets.size() == 1 &&
        additional_c_genjets_hadron.size() == 1) {
    ...
    if (additional_c_genjets.size() > 1 &&
      ...
      if (hadr1.pt() > 0. && hadr2.pt() > 0.)
        drchad = BoostedUtils::DeltaR(hadr1.p4(), hadr2.p4());
      mchadjets =
          (additional_c_genjets[0].p4() + additional_c_genjets[1].p4()).M();
      drchadjets = BoostedUtils::DeltaR(additional_c_genjets[0].p4(),
                                        additional_c_genjets[1].p4());
    }
