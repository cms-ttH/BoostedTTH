...
void
DiLeptonVarProcessor::Init(const InputCollections& input,
...
void
DiLeptonVarProcessor::Process(const InputCollections& input,
                              VariableContainer& vars)
{
  if (!initialized)
  ...
  if (input.selectedMuonsDL.size() >= 2) {
    ...
    vars.FillVar("DiMuon_SS",
                 input.selectedMuonsDL[0].charge() *
