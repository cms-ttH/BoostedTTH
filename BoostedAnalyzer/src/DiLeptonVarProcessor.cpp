...
void DiLeptonVarProcessor::Init(const InputCollections& input,
...
void DiLeptonVarProcessor::Process(const InputCollections& input,
                                   VariableContainer& vars) {
  if (!initialized) cerr << "tree processor not initialized" << endl;

  if (input.selectedMuonsDL.size() >= 2) {
    math::XYZTLorentzVector DiMuonVec =
        input.selectedMuonsDL[0].p4() + input.selectedMuonsDL[1].p4();
    ...
    vars.FillVar("DiMuon_SS", input.selectedMuonsDL[0].charge() *
                                      input.selectedMuonsDL[1].charge() >
                                  0);
  }

  if (input.selectedElectronsDL.size() >= 2) {
    math::XYZTLorentzVector DiElectronVec =
        input.selectedElectronsDL[0].p4() + input.selectedElectronsDL[1].p4();
    ...
    vars.FillVar("DiElectron_SS",
                 input.selectedElectronsDL[0].charge() *
                         input.selectedElectronsDL[1].charge() >
                     0);
  }

  if (input.selectedElectronsDL.size() >= 1 &&
      input.selectedMuonsDL.size() >= 1) {
    math::XYZTLorentzVector ElMuVec =
        input.selectedElectronsDL[0].p4() + input.selectedMuonsDL[0].p4();
    ...
    vars.FillVar("ElMu_SS", input.selectedElectronsDL[0].charge() *
                                    input.selectedMuonsDL[1].charge() >
                                0);
  }
}
