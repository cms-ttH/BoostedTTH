...
void CSVHelper::fillCSVhistos(TFile* fileHF, TFile* fileLF){
...
double CSVHelper::get_csv_wgt( const std::vector<pat::Jet>& Jets, int iSys, double &csvWgtHF, double &csvWgtLF, double &csvWgtCF ){
  int iSysHF = 0;
  switch(iSys){
 ...
 for(std::vector<pat::Jet>::const_iterator itJet = Jets.begin(); itJet != Jets.end(); ++itJet){
    ...
    else if (jetPt >=160 && jetPt<10000) iPt = 5; // change back to && jetPt<10000
