...
void BoostedJetVarProcessor::FillSFJetVars(const InputCollections& input, VariableContainer& vars)
{
    for (size_t i = 0; i < input.selectedBoostedJets.size(); i++) {
        ...
        for (std::vector< pat::Jet >::const_iterator itSub = input.selectedBoostedJets[i].subjets.begin();
             itSub != input.selectedBoostedJets[i].subjets.end() && itSub - input.selectedBoostedJets[i].subjets.begin() < 2; ++itSub) {
        ...
        for (std::vector< pat::Jet >::const_iterator itSub = input.selectedBoostedJets[i].filterjets.begin();
             itSub != input.selectedBoostedJets[i].filterjets.end() && itSub - input.selectedBoostedJets[i].filterjets.begin() < 3; ++itSub) {
        ...
        if (input.selectedBoostedJets[i].filterjets.size() > 1) {
            ...
            vars.FillVars("BoostedJet_Dr_Filterjet12", i,
                          BoostedUtils::DeltaR(input.selectedBoostedJets[i].filterjets[0], input.selectedBoostedJets[i].filterjets[1]));
            vars.FillVars("BoostedJet_CosThetaStar_Filterjet12", i,
                          BoostedUtils::CosThetaStar(input.selectedBoostedJets[i].filterjets[0].p4(), input.selectedBoostedJets[i].filterjets[1].p4()));
        }

        if (input.selectedBoostedJets[i].filterjets.size() > 2) {
            math::XYZTLorentzVector filterjet123 = input.selectedBoostedJets[i].filterjets[0].p4() + input.selectedBoostedJets[i].filterjets[1].p4() +
            ...
            vars.FillVars("BoostedJet_Dr_Filterjet23", i,
                          BoostedUtils::DeltaR(input.selectedBoostedJets[i].filterjets[1], input.selectedBoostedJets[i].filterjets[2]));
            vars.FillVars("BoostedJet_Dr_Filterjet13", i,
                          BoostedUtils::DeltaR(input.selectedBoostedJets[i].filterjets[0], input.selectedBoostedJets[i].filterjets[2]));
            vars.FillVars("BoostedJet_CosThetaStar_Filterjet23", i,
                          BoostedUtils::CosThetaStar(input.selectedBoostedJets[i].filterjets[1].p4(), input.selectedBoostedJets[i].filterjets[2].p4()));
            vars.FillVars("BoostedJet_CosThetaStar_Filterjet13", i,
                          BoostedUtils::CosThetaStar(input.selectedBoostedJets[i].filterjets[0].p4(), input.selectedBoostedJets[i].filterjets[2].p4()));
        }
        ...
        if (input.selectedBoostedJets[i].filterjets.size() > 1) {
            ...
            if (higgsjets.size() > 2) { bbgjet += higgsjets.at(2).p4(); }
