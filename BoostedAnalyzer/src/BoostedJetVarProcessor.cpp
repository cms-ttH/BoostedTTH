...
BoostedJetVarProcessor::BoostedJetVarProcessor(MiniAODHelper* helper_)
    : btagger("DeepJet") {
  toptagger.insert(map<string, TopTagger>::value_type(
      "HEP", TopTagger(TopTag::HEP, TopTag::Pt, "")));
  toptagger.insert(map<string, TopTagger>::value_type(
      "HEPCSV", TopTagger(TopTag::HEP, TopTag::CSV, "")));
  toptagger.insert(map<string, TopTagger>::value_type(
      "Likelihood", TopTagger(TopTag::Likelihood, TopTag::CSV,
                              "toplikelihoodtaggerhistos.root")));
  toptagger.insert(map<string, TopTagger>::value_type(
      "BDT_Std", TopTagger(TopTag::TMVA, TopTag::CSV,
                           "BDTTopTagger_BDTG_Std.weights.xml")));
  toptagger.insert(map<string, TopTagger>::value_type(
      "BDT_PSO", TopTagger(TopTag::TMVA, TopTag::CSV,
                           "BDTTopTagger_BDTG_PSO.weights.xml")));

  higgstagger.insert(map<string, HiggsTagger>::value_type(
      "SecondCSV", HiggsTagger(HiggsTag::SecondCSV, "")));
  higgstagger.insert(map<string, HiggsTagger>::value_type(
      "DoubleCSV", HiggsTagger(HiggsTag::DoubleCSV, "")));
}

BoostedJetVarProcessor::~BoostedJetVarProcessor() {}

void BoostedJetVarProcessor::Init(const InputCollections& input,
...
void BoostedJetVarProcessor::Process(const InputCollections& input,
                                     VariableContainer& vars) {
...
void BoostedJetVarProcessor::FillFatJetVars(const InputCollections& input,
                                            VariableContainer& vars) {
  // Get Objects
  math::XYZTLorentzVector primlepvec = math::XYZTLorentzVector(0., 0., 0., 0.);
  if (input.selectedElectronsLoose.size() > 0 ||
      input.selectedMuonsLoose.size() > 0)
    primlepvec = BoostedUtils::GetPrimLepVec(input.selectedElectronsLoose,
                                             input.selectedMuonsLoose);
  ...
  for (size_t i = 0; i < input.selectedBoostedJets.size(); i++) {
    ...
    vars.FillVars("BoostedJet_Dr_Lepton", i,
                  primlepvec.pt() > 5
                      ? BoostedUtils::DeltaR(primlepvec, fatjet.p4())
                      : -1);
  }
}

void BoostedJetVarProcessor::FillHTTJetVars(const InputCollections& input,
                                            VariableContainer& vars) {
  for (size_t i = 0; i < input.selectedBoostedJets.size(); i++) {
    // Get BoostedJet Objects
    pat::Jet fatjet = input.selectedBoostedJets[i].fatjet;
    ...
    const math::XYZTLorentzVector WVec =
        input.selectedBoostedJets[i].GetWJetVec();
    const math::XYZTLorentzVector topVec =
        input.selectedBoostedJets[i].GetTopJetVec();
    ...
    vars.FillVars("BoostedJet_Dr_B_Top", i,
                  BoostedUtils::DeltaR(nonW.p4(), topVec));
    vars.FillVars("BoostedJet_Dr_W1_Top", i,
                  BoostedUtils::DeltaR(W1.p4(), topVec));
    vars.FillVars("BoostedJet_Dr_W2_Top", i,
                  BoostedUtils::DeltaR(W2.p4(), topVec));
    vars.FillVars("BoostedJet_Dr_W_Top", i, BoostedUtils::DeltaR(WVec, topVec));
    vars.FillVars("BoostedJet_Dr_W_B", i,
                  BoostedUtils::DeltaR(WVec, nonW.p4()));
    vars.FillVars("BoostedJet_Dr_W1_B", i,
                  BoostedUtils::DeltaR(W1.p4(), nonW.p4()));
    vars.FillVars("BoostedJet_Dr_W2_B", i,
                  BoostedUtils::DeltaR(W2.p4(), nonW.p4()));
    ...
    vars.FillVars("BoostedJet_Dr_W1_W2", i,
                  BoostedUtils::DeltaR(W1.p4(), W2.p4()));
    ...
    vector<math::XYZTLorentzVector> topvecs_bycsv =
        BoostedUtils::GetJetVecs(subjets);
    ...
    vars.FillVars("BoostedJet_Bbtag_CSV", i,
                  CSVHelper::GetJetCSV(subjets[0], btagger));
    ...
    vars.FillVars("BoostedJet_W1btag_CSV", i,
                  CSVHelper::GetJetCSV(subjets[1], btagger));
    ...
    vars.FillVars("BoostedJet_W2btag_CSV", i,
                  CSVHelper::GetJetCSV(subjets[2], btagger));

    vars.FillVars("BoostedJet_Wbtag_E", i,
                  (topvecs_bycsv[1] + topvecs_bycsv[2]).E());
    vars.FillVars("BoostedJet_Wbtag_Pt", i,
                  (topvecs_bycsv[1] + topvecs_bycsv[2]).Pt());
    vars.FillVars("BoostedJet_Wbtag_Eta", i,
                  (topvecs_bycsv[1] + topvecs_bycsv[2]).Eta());
    vars.FillVars("BoostedJet_Wbtag_Phi", i,
                  (topvecs_bycsv[1] + topvecs_bycsv[2]).Phi());
    vars.FillVars("BoostedJet_Wbtag_M", i,
                  (topvecs_bycsv[1] + topvecs_bycsv[2]).M());

    vars.FillVars("BoostedJet_BW1btag_M", i,
                  (topvecs_bycsv[0] + topvecs_bycsv[1]).M());
    vars.FillVars("BoostedJet_BW2btag_M", i,
                  (topvecs_bycsv[0] + topvecs_bycsv[2]).M());

    vars.FillVars("BoostedJet_Dr_W1btag_Bbtag", i,
                  BoostedUtils::DeltaR(subjets[1], subjets[0]));
    vars.FillVars("BoostedJet_Dr_W2btag_Bbtag", i,
                  BoostedUtils::DeltaR(subjets[2], subjets[0]));
    vars.FillVars("BoostedJet_Dr_W1btag_W2btag", i,
                  BoostedUtils::DeltaR(subjets[1], subjets[2]));

    vars.FillVars("BoostedJet_ERatio_Bbtag", i,
                  subjets[0].energy() / fatjet.energy());
    vars.FillVars("BoostedJet_ERatio_W1btag", i,
                  subjets[1].energy() / fatjet.energy());
    vars.FillVars("BoostedJet_ERatio_W2btag", i,
                  subjets[2].energy() / fatjet.energy());
    vars.FillVars("BoostedJet_ERatio_Wbtag", i,
                  (topvecs_bycsv[1] + topvecs_bycsv[2]).E() / fatjet.energy());
    vars.FillVars("BoostedJet_ERatio_Bbtag_Top", i,
                  subjets[0].energy() / topVec.E());
    vars.FillVars("BoostedJet_ERatio_W1btag_Top", i,
                  subjets[1].energy() / topVec.E());
    vars.FillVars("BoostedJet_ERatio_W2btag_Top", i,
                  subjets[2].energy() / topVec.E());
    vars.FillVars("BoostedJet_ERatio_Wbtag_Top", i,
                  (topvecs_bycsv[1] + topvecs_bycsv[2]).E() / topVec.E());

    vars.FillVars("BoostedJet_PtRatio_Bbtag", i, subjets[0].pt() / fatjet.pt());
    vars.FillVars("BoostedJet_PtRatio_W1btag", i,
                  subjets[1].pt() / fatjet.pt());
    vars.FillVars("BoostedJet_PtRatio_W2btag", i,
                  subjets[2].pt() / fatjet.pt());
    vars.FillVars("BoostedJet_PtRatio_Wbtag", i,
                  (topvecs_bycsv[1] + topvecs_bycsv[2]).pt() / fatjet.pt());
    vars.FillVars("BoostedJet_PtRatio_Bbtag_Top", i,
                  subjets[0].pt() / topVec.Pt());
    vars.FillVars("BoostedJet_PtRatio_W1btag_Top", i,
                  subjets[1].pt() / topVec.Pt());
    vars.FillVars("BoostedJet_PtRatio_W2btag_Top", i,
                  subjets[2].pt() / topVec.Pt());
    vars.FillVars("BoostedJet_PtRatio_Wbtag_Top", i,
                  (topvecs_bycsv[1] + topvecs_bycsv[2]).pt() / topVec.Pt());

    vars.FillVars("BoostedJet_MRatio_Bbtag", i,
                  subjets[0].mass() / fatjet.mass());
    vars.FillVars("BoostedJet_MRatio_W1btag", i,
                  subjets[1].mass() / fatjet.mass());
    vars.FillVars("BoostedJet_MRatio_W2btag", i,
                  subjets[2].mass() / fatjet.mass());
    vars.FillVars("BoostedJet_MRatio_Wbtag", i,
                  (topvecs_bycsv[1] + topvecs_bycsv[2]).M() / fatjet.mass());
    vars.FillVars("BoostedJet_MRatio_Bbtag_Top", i,
                  subjets[0].mass() / topVec.M());
    vars.FillVars("BoostedJet_MRatio_W1btag_Top", i,
                  subjets[1].mass() / topVec.M());
    vars.FillVars("BoostedJet_MRatio_W2btag_Top", i,
                  subjets[2].mass() / topVec.M());
    vars.FillVars("BoostedJet_MRatio_Wbtag_Top", i,
                  (topvecs_bycsv[1] + topvecs_bycsv[2]).M() / topVec.M());

    vars.FillVars("BoostedJet_FatJet_Mass", i,
                  input.selectedBoostedJets[i].fatjetMass);
    vars.FillVars("BoostedJet_FatJet_Pt", i,
                  input.selectedBoostedJets[i].fatjetPt);
    vars.FillVars("BoostedJet_FatJet_Eta", i,
                  input.selectedBoostedJets[i].fatjetEta);
    vars.FillVars("BoostedJet_FatJet_Phi", i,
                  input.selectedBoostedJets[i].fatjetPhi);

    vars.FillVars("BoostedJet_TopMass", i,
                  input.selectedBoostedJets[i].topMass);
    vars.FillVars("BoostedJet_UnfilteredMass", i,
                  input.selectedBoostedJets[i].unfilteredMass);
    vars.FillVars("BoostedJet_PrunedMass", i,
                  input.selectedBoostedJets[i].prunedMass);
    vars.FillVars("BoostedJet_fRec", i, input.selectedBoostedJets[i].fRec);
    vars.FillVars("BoostedJet_MassRatioPassed", i,
                  input.selectedBoostedJets[i].massRatioPassed);

    vars.FillVars("BoostedJet_Ropt", i, input.selectedBoostedJets[i].Ropt);
    vars.FillVars("BoostedJet_RoptCalc", i,
                  input.selectedBoostedJets[i].RoptCalc);
    vars.FillVars("BoostedJet_PtForRoptCalc", i,
                  input.selectedBoostedJets[i].ptForRoptCalc);

    vars.FillVars("BoostedJet_Tau1Unfiltered", i,
                  input.selectedBoostedJets[i].tau1Unfiltered);
    vars.FillVars("BoostedJet_Tau2Unfiltered", i,
                  input.selectedBoostedJets[i].tau2Unfiltered);
    vars.FillVars("BoostedJet_Tau3Unfiltered", i,
                  input.selectedBoostedJets[i].tau3Unfiltered);
    vars.FillVars("BoostedJet_Tau1Filtered", i,
                  input.selectedBoostedJets[i].tau1Filtered);
    vars.FillVars("BoostedJet_Tau2Filtered", i,
                  input.selectedBoostedJets[i].tau2Filtered);
    vars.FillVars("BoostedJet_Tau3Filtered", i,
                  input.selectedBoostedJets[i].tau3Filtered);

    vars.FillVars("BoostedJet_QWeight", i,
                  input.selectedBoostedJets[i].qWeight);
    vars.FillVars("BoostedJet_QEpsilon", i,
                  input.selectedBoostedJets[i].qEpsilon);
    vars.FillVars("BoostedJet_QSigmaM", i,
                  input.selectedBoostedJets[i].qSigmaM);
    ...
    for (size_t j = 0; j < jetvecs.size() && w2vec.pt() > 0; j++) {
      if (((int)j) != idxB && ((int)j) != idxW1 &&
          BoostedUtils::DeltaR(jetvecs[j], w2vec) < 0.2) {
        idxW2 = j;
        break;
      }
    }
...
void BoostedJetVarProcessor::FillTopTagVars(const InputCollections& input,
                                            VariableContainer& vars) {
  for (size_t i = 0; i < input.selectedBoostedJets.size(); i++) {
    vars.FillVars(
        "BoostedJet_TopTag_HEP", i,
        toptagger.at("HEP").GetTopTaggerOutput(input.selectedBoostedJets[i]));
    vars.FillVars("BoostedJet_TopTag_HEPCSV", i,
                  toptagger.at("HEPCSV").GetTopTaggerOutput(
                      input.selectedBoostedJets[i]));
    vars.FillVars("BoostedJet_TopTag_Likelihood", i,
                  toptagger.at("Likelihood")
                      .GetTopTaggerOutput(input.selectedBoostedJets[i]));
    vars.FillVars("BoostedJet_TopTag_BDT_Std", i,
                  toptagger.at("BDT_Std").GetTopTaggerOutput(
                      input.selectedBoostedJets[i]));
    vars.FillVars("BoostedJet_TopTag_BDT_PSO", i,
                  toptagger.at("BDT_PSO").GetTopTaggerOutput(
                      input.selectedBoostedJets[i]));
  }
}

void BoostedJetVarProcessor::FillSFJetVars(const InputCollections& input,
                                           VariableContainer& vars) {
  for (size_t i = 0; i < input.selectedBoostedJets.size(); i++) {
    vars.FillVars("N_BoostedJet_Subjets", i,
                  input.selectedBoostedJets[i].subjets.size());

    for (std::vector<pat::Jet>::const_iterator itSub =
             input.selectedBoostedJets[i].subjets.begin();
         itSub != input.selectedBoostedJets[i].subjets.end() &&
         itSub - input.selectedBoostedJets[i].subjets.begin() < 2;
         ++itSub) {
      TString subjetName = "Subjet";
      subjetName +=
          ((itSub - input.selectedBoostedJets[i].subjets.begin()) + 1);
      ...
      vars.FillVars("BoostedJet_" + subjetName + "_CSV", i,
                    CSVHelper::GetJetCSV(*itSub, btagger));
      vars.FillVars(
          "BoostedJet_" + subjetName + "_Dr", i,
          BoostedUtils::DeltaR(*itSub, input.selectedBoostedJets[i].fatjet));
      vars.FillVars("BoostedJet_" + subjetName + "_Flav", i,
                    itSub->partonFlavour());
      vars.FillVars("BoostedJet_" + subjetName + "_Charge", i,
                    itSub->jetCharge());
      vars.FillVars("BoostedJet_" + subjetName + "_PtRatio", i,
                    itSub->pt() / input.selectedBoostedJets[i].fatjet.pt());
      vars.FillVars(
          "BoostedJet_" + subjetName + "_ERatio", i,
          itSub->energy() / input.selectedBoostedJets[i].fatjet.energy());
    }

    if (input.selectedBoostedJets[i].subjets.size() > 1) {
      math::XYZTLorentzVector subjet12 =
          input.selectedBoostedJets[i].subjets[0].p4() +
          input.selectedBoostedJets[i].subjets[1].p4();
    ...
    vars.FillVars("N_BoostedJet_Filterjets", i,
                  input.selectedBoostedJets[i].filterjets.size());

    for (std::vector<pat::Jet>::const_iterator itSub =
             input.selectedBoostedJets[i].filterjets.begin();
         itSub != input.selectedBoostedJets[i].filterjets.end() &&
         itSub - input.selectedBoostedJets[i].filterjets.begin() < 3;
         ++itSub) {
      TString subjetName = "Filterjet";
      subjetName +=
          ((itSub - input.selectedBoostedJets[i].filterjets.begin()) + 1);
    ...
    if (input.selectedBoostedJets[i].filterjets.size() > 1) {
      math::XYZTLorentzVector filterjet12 =
          input.selectedBoostedJets[i].filterjets[0].p4() +
          input.selectedBoostedJets[i].filterjets[1].p4();
      ...
      vars.FillVars(
          "BoostedJet_Dr_Filterjet12", i,
          BoostedUtils::DeltaR(input.selectedBoostedJets[i].filterjets[0],
                               input.selectedBoostedJets[i].filterjets[1]));
      vars.FillVars("BoostedJet_CosThetaStar_Filterjet12", i,
                    BoostedUtils::CosThetaStar(
                        input.selectedBoostedJets[i].filterjets[0].p4(),
                        input.selectedBoostedJets[i].filterjets[1].p4()));
    }

    if (input.selectedBoostedJets[i].filterjets.size() > 2) {
      math::XYZTLorentzVector filterjet123 =
          input.selectedBoostedJets[i].filterjets[0].p4() +
          input.selectedBoostedJets[i].filterjets[1].p4() +
          input.selectedBoostedJets[i].filterjets[2].p4();
      ...
      vars.FillVars(
          "BoostedJet_Dr_Filterjet23", i,
          BoostedUtils::DeltaR(input.selectedBoostedJets[i].filterjets[1],
                               input.selectedBoostedJets[i].filterjets[2]));
      vars.FillVars(
          "BoostedJet_Dr_Filterjet13", i,
          BoostedUtils::DeltaR(input.selectedBoostedJets[i].filterjets[0],
                               input.selectedBoostedJets[i].filterjets[2]));
      vars.FillVars("BoostedJet_CosThetaStar_Filterjet23", i,
                    BoostedUtils::CosThetaStar(
                        input.selectedBoostedJets[i].filterjets[1].p4(),
                        input.selectedBoostedJets[i].filterjets[2].p4()));
      vars.FillVars("BoostedJet_CosThetaStar_Filterjet13", i,
                    BoostedUtils::CosThetaStar(
                        input.selectedBoostedJets[i].filterjets[0].p4(),
                        input.selectedBoostedJets[i].filterjets[2].p4()));
    }

    vars.FillVars("BoostedJet_IsGoodTopJet", i,
                  input.selectedBoostedJets[i].isGoodTopJet);
    vars.FillVars("BoostedJet_IsGoodHiggsJet", i,
                  input.selectedBoostedJets[i].isGoodHiggsJet);

    if (input.selectedBoostedJets[i].filterjets.size() > 1) {
      vector<pat::Jet> higgsjets =
          BoostedUtils::GetHiggsFilterJets(input.selectedBoostedJets[i], 2, 3);

      math::XYZTLorentzVector bbjet =
          higgsjets.at(0).p4() + higgsjets.at(1).p4();
      ...
      for (std::vector<pat::Jet>::const_iterator itSub = higgsjets.begin();
           itSub != higgsjets.end() && itSub - higgsjets.begin() < 3; ++itSub) {
    ...
    std::vector<pat::Jet> filterJets =
        BoostedUtils::GetHiggsFilterJets(input.selectedBoostedJets[i], 4, -1);
    if (filterJets.size() > 0)
      vars.FillVars("BoostedJet_CSV1", i,
                    CSVHelper::GetJetCSV(filterJets[0], btagger));
    if (filterJets.size() > 1)
      vars.FillVars("BoostedJet_CSV2", i,
                    CSVHelper::GetJetCSV(filterJets[1], btagger));
    if (filterJets.size() > 2)
      vars.FillVars("BoostedJet_CSV3", i,
                    CSVHelper::GetJetCSV(filterJets[2], btagger));
    if (filterJets.size() > 3)
      vars.FillVars("BoostedJet_CSV4", i,
                    CSVHelper::GetJetCSV(filterJets[3], btagger));

    vector<math::XYZTLorentzVector> jetvecs =
        BoostedUtils::GetJetVecs(input.selectedJets);
    vector<math::XYZTLorentzVector> filtvecs =
        BoostedUtils::GetJetVecs(input.selectedBoostedJets[i].filterjets);
    int idx1 = -1;
    int idx2 = -1;
    int idx3 = -1;
    ...
    for (size_t j = 0; j < jetvecs.size() && filtvecs.size() > 1; j++) {
      if (idx1 != ((int)j) &&
          BoostedUtils::DeltaR(jetvecs[j], filtvecs[1]) < 0.2) {
        idx2 = j;
        break;
      }
    }
    for (size_t j = 0; j < jetvecs.size() && filtvecs.size() > 2; j++) {
      if (idx2 != ((int)j) && idx1 != ((int)j) &&
          BoostedUtils::DeltaR(jetvecs[j], filtvecs[2]) < 0.2) {
        idx3 = j;
        break;
      }
    }
...
void BoostedJetVarProcessor::FillPrunedJetVars(const InputCollections& input,
                                               VariableContainer& vars) {
  for (size_t i = 0; i < input.selectedBoostedJets.size(); i++) {
    std::vector<pat::Jet> subjets =
        input.selectedBoostedJets.at(i).prunedsubjets;
    std::sort(subjets.begin(), subjets.end(), BoostedUtils::FirstJetIsHarder);

    vars.FillVars("N_BoostedJet_Pruned_Subjets", i, subjets.size());

    for (std::vector<pat::Jet>::const_iterator itSub = subjets.begin();
         itSub != subjets.end() && itSub - subjets.begin() < 2; ++itSub) {
      TString SubjetName = "Subjet";
      SubjetName += ((itSub - subjets.begin()) + 1);

      vars.FillVars("BoostedJet_Pruned_" + SubjetName + "_E", i,
                    itSub->energy());
      vars.FillVars("BoostedJet_Pruned_" + SubjetName + "_Pt", i, itSub->pt());
      vars.FillVars("BoostedJet_Pruned_" + SubjetName + "_M", i, itSub->mass());
      vars.FillVars("BoostedJet_Pruned_" + SubjetName + "_Eta", i,
                    itSub->eta());
      vars.FillVars("BoostedJet_Pruned_" + SubjetName + "_Phi", i,
                    itSub->phi());
      vars.FillVars("BoostedJet_Pruned_" + SubjetName + "_CSV", i,
                    CSVHelper::GetJetCSV(*itSub, btagger));
      vars.FillVars(
          "BoostedJet_Pruned_" + SubjetName + "_Dr", i,
          BoostedUtils::DeltaR(*itSub, input.selectedBoostedJets[i].fatjet));
      vars.FillVars("BoostedJet_Pruned_" + SubjetName + "_Flav", i,
                    itSub->partonFlavour());
      vars.FillVars("BoostedJet_Pruned_" + SubjetName + "_Charge", i,
                    itSub->jetCharge());
      vars.FillVars("BoostedJet_Pruned_" + SubjetName + "_PtRatio", i,
                    itSub->pt() / input.selectedBoostedJets[i].fatjet.pt());
      vars.FillVars(
          "BoostedJet_Pruned_" + SubjetName + "_ERatio", i,
          itSub->energy() / input.selectedBoostedJets[i].fatjet.energy());
    }
    ...
    if (subjets.size() > 1) {
      ...
      vars.FillVars("BoostedJet_Dr_Pruned_Subjet12", i,
                    BoostedUtils::DeltaR(subjets[0], subjets[1]));
    }

    std::sort(subjets.begin(), subjets.end(), BoostedUtils::FirstHasHigherCSV);

    if (subjets.size() > 0)
      vars.FillVars("BoostedJet_Pruned_CSV1", i,
                    CSVHelper::GetJetCSV(subjets.at(0), btagger));
    if (subjets.size() > 1)
      vars.FillVars("BoostedJet_Pruned_CSV2", i,
                    CSVHelper::GetJetCSV(subjets.at(1), btagger));
  }
}

void BoostedJetVarProcessor::FillSDJetVars(const InputCollections& input,
                                           VariableContainer& vars) {
  for (size_t i = 0; i < input.selectedBoostedJets.size(); i++) {
    ...
    for (std::vector<pat::Jet>::const_iterator itSub = subjets.begin();
         itSub != subjets.end() && itSub - subjets.begin() < 3; ++itSub) {
      TString SubjetName = "Subjet";
      SubjetName += ((itSub - subjets.begin()) + 1);
      ...
      vars.FillVars("BoostedJet_SD_" + SubjetName + "_CSV", i,
                    CSVHelper::GetJetCSV(*itSub, btagger));
      vars.FillVars(
          "BoostedJet_SD_" + SubjetName + "_Dr", i,
          BoostedUtils::DeltaR(*itSub, input.selectedBoostedJets[i].fatjet));
      vars.FillVars("BoostedJet_SD_" + SubjetName + "_Flav", i,
                    itSub->partonFlavour());
      vars.FillVars("BoostedJet_SD_" + SubjetName + "_Charge", i,
                    itSub->jetCharge());
      vars.FillVars("BoostedJet_SD_" + SubjetName + "_PtRatio", i,
                    itSub->pt() / input.selectedBoostedJets[i].fatjet.pt());
      vars.FillVars(
          "BoostedJet_SD_" + SubjetName + "_ERatio", i,
          itSub->energy() / input.selectedBoostedJets[i].fatjet.energy());
    }
    ...
    if (subjets.size() > 1) {
      ...
      vars.FillVars("BoostedJet_Dr_SD_Subjet12", i,
                    BoostedUtils::DeltaR(subjets[0], subjets[1]));
      if (subjets.size() > 2) {
        vars.FillVars("BoostedJet_Dr_SD_Subjet13", i,
                      BoostedUtils::DeltaR(subjets[0], subjets[2]));
        vars.FillVars("BoostedJet_Dr_SD_Subjet23", i,
                      BoostedUtils::DeltaR(subjets[1], subjets[2]));
      }
    }
    ...
    if (subjets.size() > 0)
      vars.FillVars("BoostedJet_SD_CSV1", i,
                    CSVHelper::GetJetCSV(subjets.at(0), btagger));
    if (subjets.size() > 1)
      vars.FillVars("BoostedJet_SD_CSV2", i,
                    CSVHelper::GetJetCSV(subjets.at(1), btagger));
    if (subjets.size() > 2)
      vars.FillVars("BoostedJet_SD_CSV3", i,
                    CSVHelper::GetJetCSV(subjets.at(2), btagger));

    vars.FillVars("BoostedJet_SD_Tau1", i,
                  input.selectedBoostedJets.at(i).tau1Softdrop);
    vars.FillVars("BoostedJet_SD_Tau2", i,
                  input.selectedBoostedJets.at(i).tau2Softdrop);
    vars.FillVars("BoostedJet_SD_Tau3", i,
                  input.selectedBoostedJets.at(i).tau3Softdrop);
  }
}

void BoostedJetVarProcessor::FillHiggsTagVars(const InputCollections& input,
                                              VariableContainer& vars) {
  for (size_t i = 0; i < input.selectedBoostedJets.size(); i++) {
    vars.FillVars("BoostedJet_HiggsTag_SecondCSV", i,
                  higgstagger.at("SecondCSV")
                      .GetHiggsTaggerOutput(input.selectedBoostedJets[i]));
    vars.FillVars("BoostedJet_HiggsTag_DoubleCSV", i,
                  higgstagger.at("DoubleCSV")
                      .GetHiggsTaggerOutput(input.selectedBoostedJets[i]));
  }
}
