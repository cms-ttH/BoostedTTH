...
DiLeptonJetTagSelection::DiLeptonJetTagSelection(std::vector<int> minjets_,std::vector<int> maxjets_, std::vector<int> mintags_, std::vector<int> maxtags_):minJets(minjets_), maxJets(maxjets_), minTags(mintags_), maxTags(maxtags_){
...
DiLeptonJetTagSelection::DiLeptonJetTagSelection(const edm::ParameterSet& iConfig):DiLeptonJetTagSelection(iConfig.getParameter< std::vector<int> >("minJets"),
...
bool DiLeptonJetTagSelection::IsSelected(const InputCollections& input,Cutflow& cutflow){
  ...
  for(size_t i=0; i<input.selectedJets.size();i++){
    ...
    if(input.selectedJets.at(i).pt()>=ptCut){
      njets++;
    }
  }
