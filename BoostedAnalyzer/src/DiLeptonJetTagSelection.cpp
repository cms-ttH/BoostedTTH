...
DiLeptonJetTagSelection::DiLeptonJetTagSelection(std::vector<int> minjets_,
    ...
    : DiLeptonJetTagSelection(minjets_, std::vector<int>(), mintags_,
                              std::vector<int>()) {}
...
DiLeptonJetTagSelection::DiLeptonJetTagSelection(
    ...
    : DiLeptonJetTagSelection(
          iConfig.getParameter<std::vector<int> >("minJets"),
...
bool DiLeptonJetTagSelection::IsSelected(const InputCollections& input,
                                         Cutflow& cutflow) {
  ...
  for (size_t i = 0; i < input.selectedJetsLoose.size(); i++) {
    if (CSVHelper::PassesCSV(input.selectedJetsLoose.at(i), "DeepJet",
