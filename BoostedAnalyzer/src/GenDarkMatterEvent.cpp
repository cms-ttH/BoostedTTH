...
void GenDarkMatterEvent::Initialize(
    std::vector<reco::GenParticle> prunedGenParticles_,
    std::vector<pat::PackedGenParticle> packedGenParticles_) {
  ...
  for (size_t i = 0; i < prunedGenParticles.size(); i++) {
    if (prunedGenParticles[i].pdgId() == 1000022 or
        abs(prunedGenParticles[i].pdgId()) == 18) {
      hasDarkMatter = true;
      // break;
    }
    if ((abs(prunedGenParticles[i].pdgId()) == 11 or
         abs(prunedGenParticles[i].pdgId()) == 12 or
         abs(prunedGenParticles[i].pdgId()) == 13 or
         abs(prunedGenParticles[i].pdgId()) == 14 or
         abs(prunedGenParticles[i].pdgId()) == 15 or
         abs(prunedGenParticles[i].pdgId()) == 16) and
...
GenDarkMatterEvent::ReturnPackedGenParticles() const {
...
void GenDarkMatterEvent::Fill() {
  ...
  for (size_t i = 0; i < prunedGenParticles.size(); i++) {
    ...
    if ((genparticle.pdgId() == 1000022 or abs(genparticle.pdgId()) == 18) and
        genparticle.status() == 1) {
      Neutralinos.push_back(genparticle);
    }
    if ((genparticle.pdgId() == 23 || genparticle.pdgId() == 25 ||
         genparticle.pdgId() == 55) and
        genparticle.isLastCopy()) {
      Mediator = genparticle;
    }
    if ((abs(genparticle.pdgId()) == 12 or abs(genparticle.pdgId()) == 14 or
...
void GenDarkMatterEvent::FillBoson() {
  ...
  for (size_t i = 0; i < prunedGenParticles.size(); i++) {
    ...
    // Z Bosons
    if ((abs(genparticle.pdgId()) == 12 or abs(genparticle.pdgId()) == 14 or
         abs(genparticle.pdgId()) == 16 or abs(genparticle.pdgId()) == 11 or
    ...
    // W Bosons
    if ((abs(genparticle.pdgId()) == 11 or abs(genparticle.pdgId()) == 12 or
         abs(genparticle.pdgId()) == 13 or abs(genparticle.pdgId()) == 14) and
        genparticle.isPromptFinalState()) {  // or abs(daughter->pdgId()) == 15
                                             // or abs(daughter->pdgId()) == 16
      decay_prodW.push_back(genparticle);
    }

    if (abs(genparticle.pdgId()) == 22 and genparticle.status() == 1 and
        !genparticle.statusFlags().isPrompt()) {
      radiated_photons.push_back(genparticle);
    }
  }
  if (decay_prodW.size() == 2) {
    // std::cout << "filling W Boson" << std::endl;
    if ((decay_prodW.at(0).pdgId()) * (decay_prodW.at(1).pdgId()) < 0 and
        abs(abs(decay_prodW.at(0).pdgId()) - abs(decay_prodW.at(1).pdgId())) ==
            1) {
      for (size_t k = 0; k < decay_prodW.size(); k++) {
        if (abs(decay_prodW.at(k).pdgId()) == 11 or
            abs(decay_prodW.at(k).pdgId()) == 13) {
          for (size_t l = 0; l < radiated_photons.size(); l++) {
            if (reco::deltaR(radiated_photons.at(l).p4(),
                             decay_prodW.at(k).p4()) < 0.1) {
              decay_prodW.at(k).setP4(decay_prodW.at(k).p4() +
                                      radiated_photons.at(l).p4());
            }
          }
        }
      }
  ...
  if (decay_prodZ.size() == 2) {
    ...
    if ((decay_prodZ.at(0).pdgId()) + (decay_prodZ.at(1).pdgId()) == 0) {
      for (size_t k = 0; k < decay_prodZ.size(); k++) {
        if (abs(decay_prodZ.at(k).pdgId()) == 11 or
            abs(decay_prodZ.at(k).pdgId()) == 13) {
          for (size_t l = 0; l < radiated_photons.size(); l++) {
            if (reco::deltaR(radiated_photons.at(l).p4(),
                             decay_prodZ.at(k).p4()) < 0.1) {
              decay_prodZ.at(k).setP4(decay_prodZ.at(k).p4() +
                                      radiated_photons.at(l).p4());
            }
          }
        }
      ...
      ZBosonisFilled = true;
      if (abs(decay_prodZ.at(0).pdgId()) == 12 or
          abs(decay_prodZ.at(0).pdgId()) == 14 or
          abs(decay_prodZ.at(0).pdgId()) == 16) {
        isZnunu = true;
      } else
        isZnunu = false;
    }
