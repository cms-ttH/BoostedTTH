...
void PhotonVarProcessor::Init(const InputCollections& input,
...
void PhotonVarProcessor::Process(const InputCollections& input,
                                 VariableContainer& vars) {
  ...
  for (size_t i = 0; i < input.selectedPhotons.size(); i++) {
    ...
    if (input.selectedPhotons.at(i).hasUserFloat("IdentificationSF")) {
      vars.FillVars("Photon_IdentificationSF", i,
                    input.selectedPhotons.at(i).userFloat("IdentificationSF"));
      vars.FillVars(
          "Photon_IdentificationSFUp", i,
          input.selectedPhotons.at(i).userFloat("IdentificationSFUp"));
      vars.FillVars(
          "Photon_IdentificationSFDown", i,
          input.selectedPhotons.at(i).userFloat("IdentificationSFDown"));
    }
  }
}
