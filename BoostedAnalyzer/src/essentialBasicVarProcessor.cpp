...
void
essentialBasicVarProcessor::Init(const InputCollections& input,
...
void
essentialBasicVarProcessor::Process(const InputCollections& input,
                                    VariableContainer& vars)
{
  if (!initialized)
  ...
  for (std::vector<pat::Jet>::const_iterator itJet = input.selectedJets.begin();
    ...
    vars.FillVars(
      "Jet_CSV_DNN", iJet, MiniAODHelper::GetJetCSV_DNN(*itJet, btagger));
    ...
    if (itJet->hasUserInt("pileupJetIdUpdated:fullId")) {
      vars.FillVars(
        "Jet_PileUpID", iJet, itJet->userInt("pileupJetIdUpdated:fullId"));
    }
    if (itJet->hasUserFloat("pileupJetIdUpdated:fullDiscriminant")) {
      vars.FillVars("Jet_PileUpMVA",
                    iJet,
  ...
  //   input.selectedJetsLoose.begin() ; itJet != input.selectedJetsLoose.end();
  //   ++itJet){
  ...
  //     vars.FillVars(
  //     "LooseJet_CSV",iJet,MiniAODHelper::GetJetCSV(*itJet,btagger) );
  ...
  //     vars.FillVars(
  //     "LooseJet_PileUpID",iJet,itJet->userFloat("pileupJetId:fullDiscriminant"));
  //     if(itJet->genJet()!=NULL){
  ...
  // Tagged Jets
  //   for(std::vector<pat::Jet>::iterator itTaggedJet =
  //   selectedTaggedJets.begin() ; itTaggedJet != selectedTaggedJets.end();
  //   ++itTaggedJet){
  ...
  for (std::vector<pat::Electron>::const_iterator itEle =
       ...
       itEle != input.selectedElectronsLoose.end();
       ++itEle) {
    ...
    if (itEle->hasUserFloat("ptBeforeRun2Calibration")) {
      vars.FillVars("Electron_Pt_BeforeRun2Calibration",
                    iEle,
                    itEle->userFloat("ptBeforeRun2Calibration"));
    }
    vars.FillVars(
      "Electron_Eta_Supercluster", iEle, itEle->superCluster()->eta());
  }
  for (std::vector<pat::Muon>::const_iterator itMu =
         input.selectedMuonsLoose.begin();
       itMu != input.selectedMuonsLoose.end();
       ++itMu) {
  ...
  for (std::vector<math::XYZTLorentzVector>::iterator itJetVec1 =
    ...
    for (std::vector<math::XYZTLorentzVector>::iterator itJetVec2 =
         ...
         ++itJetVec2) {
      for (std::vector<math::XYZTLorentzVector>::iterator itJetVec3 =
             itJetVec2 + 1;
           itJetVec3 != jetvecs.end();
           ++itJetVec3) {
  ...
  //   float m3tagged = -1.;
  float maxpttagged = -1.;
  for (std::vector<pat::Jet>::iterator itJetUntagged1 =
         selectedUntaggedJets.begin();
       itJetUntagged1 != selectedUntaggedJets.end();
       ++itJetUntagged1) {
    for (std::vector<pat::Jet>::iterator itJetUntagged2 = itJetUntagged1 + 1;
         itJetUntagged2 != selectedUntaggedJets.end();
         ++itJetUntagged2) {
      for (std::vector<pat::Jet>::iterator itJetTagged =
             selectedTaggedJets.begin();
           itJetTagged != selectedTaggedJets.end();
           ++itJetTagged) {

        math::XYZTLorentzVector m3vec =
          itJetUntagged1->p4() + itJetUntagged2->p4() + itJetTagged->p4();
  ...
  if (input.selectedElectrons.size() > 0 || input.selectedMuons.size() > 0) {
    mtw =
      sqrt(2 * (primLepVec.Pt() * input.correctedMET.corPt(pat::MET::Type1XY) -
                primLepVec.Px() * input.correctedMET.corPx(pat::MET::Type1XY) -
                primLepVec.Py() * input.correctedMET.corPy(pat::MET::Type1XY)));
  }
  ...
  std::sort(
    csvJetsSorted_DNN.begin(), csvJetsSorted_DNN.end(), std::greater<double>());
  ...
  for (std::vector<double>::iterator itCSV = csvJetsSorted_DNN.begin();
       itCSV != csvJetsSorted_DNN.end();
       ++itCSV) {
