...
void essentialBasicVarProcessor::Init(const InputCollections& input,
...
void essentialBasicVarProcessor::Process(const InputCollections& input,
                                         VariableContainer& vars) {
  ...
  for (std::vector<pat::Jet>::const_iterator itJet = input.selectedJets.begin();
       itJet != input.selectedJets.end(); ++itJet) {
  ...
  vars.FillVar("N_TightLeptons",
               input.selectedElectrons.size() + input.selectedMuons.size());
  vars.FillVar("N_LooseLeptons", input.selectedElectronsLoose.size() +
                                     input.selectedMuonsLoose.size());
  ...
  for (std::vector<pat::Jet>::const_iterator itJet = input.selectedJets.begin();
    ...
    if (itJet->hasUserInt("pileupJetIdUpdated:fullId"))
      vars.FillVars("Jet_PileUpID", iJet,
                    itJet->userInt("pileupJetIdUpdated:fullId"));
    if (itJet->hasUserFloat("pileupJetIdUpdated:fullDiscriminant"))
      vars.FillVars("Jet_PileUpMVA", iJet,
                    itJet->userFloat("pileupJetIdUpdated:fullDiscriminant"));
    ...
    vars.FillVars("Jet_DeepCSV_b", iJet,
                  CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepCSVJetTags:probb"));
    vars.FillVars("Jet_DeepCSV_bb", iJet,
                  CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepCSVJetTags:probbb"));
    vars.FillVars("Jet_DeepCSV_c", iJet,
                  CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepCSVJetTags:probc"));
    vars.FillVars(
        "Jet_DeepCSV_udsg", iJet,
        CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepCSVJetTags:probudsg"));

    vars.FillVars("Jet_DeepJetCSV", iJet,
                  CSVHelper::GetJetCSV_DNN(*itJet, "DeepJet"));
    vars.FillVars(
        "Jet_DeepJet_b", iJet,
        CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepFlavourJetTags:probb"));
    vars.FillVars(
        "Jet_DeepJet_bb", iJet,
        CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepFlavourJetTags:probbb"));
    vars.FillVars(
        "Jet_DeepJet_lepb", iJet,
        CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepFlavourJetTags:problepb"));
    vars.FillVars(
        "Jet_DeepJet_c", iJet,
        CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepFlavourJetTags:probc"));
    vars.FillVars(
        "Jet_DeepJet_uds", iJet,
        CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepFlavourJetTags:probuds"));
    vars.FillVars(
        "Jet_DeepJet_g", iJet,
        CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepFlavourJetTags:probg"));
  }

  for (std::vector<pat::Jet>::const_iterator itJet =
           input.selectedJetsLoose.begin();
       itJet != input.selectedJetsLoose.end(); ++itJet) {
    ...
    if (itJet->hasUserInt("pileupJetIdUpdated:fullId"))
      vars.FillVars("LooseJet_PileUpID", iJet,
                    itJet->userInt("pileupJetIdUpdated:fullId"));
    if (itJet->hasUserFloat("pileupJetIdUpdated:fullDiscriminant"))
      vars.FillVars("LooseJet_PileUpMVA", iJet,
                    itJet->userFloat("pileupJetIdUpdated:fullDiscriminant"));
    ...
    vars.FillVars("LooseJet_DeepCSV_b", iJet,
                  CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepCSVJetTags:probb"));
    vars.FillVars("LooseJet_DeepCSV_bb", iJet,
                  CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepCSVJetTags:probbb"));
    vars.FillVars("LooseJet_DeepCSV_c", iJet,
                  CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepCSVJetTags:probc"));
    vars.FillVars(
        "LooseJet_DeepCSV_udsg", iJet,
        CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepCSVJetTags:probudsg"));

    vars.FillVars("LooseJet_DeepJetCSV", iJet,
                  CSVHelper::GetJetCSV_DNN(*itJet, "DeepJet"));
    vars.FillVars(
        "LooseJet_DeepJet_b", iJet,
        CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepFlavourJetTags:probb"));
    vars.FillVars(
        "LooseJet_DeepJet_bb", iJet,
        CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepFlavourJetTags:probbb"));
    vars.FillVars(
        "LooseJet_DeepJet_lepb", iJet,
        CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepFlavourJetTags:problepb"));
    vars.FillVars(
        "LooseJet_DeepJet_c", iJet,
        CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepFlavourJetTags:probc"));
    vars.FillVars(
        "LooseJet_DeepJet_uds", iJet,
        CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepFlavourJetTags:probuds"));
    vars.FillVars(
        "LooseJet_DeepJet_g", iJet,
        CSVHelper::GetJetCSV_DNN(*itJet, "pfDeepFlavourJetTags:probg"));
  }

  // Tagged Jets
  for (std::vector<pat::Jet>::iterator itTaggedJet = selectedTaggedJets.begin();
       itTaggedJet != selectedTaggedJets.end(); ++itTaggedJet) {
    ...
    vars.FillVars("TaggedJet_CSV", iTaggedJet,
                  CSVHelper::GetJetCSV(*itTaggedJet, btagger));
    vars.FillVars("TaggedJet_DeepJetCSV", iTaggedJet,
                  CSVHelper::GetJetCSV_DNN(*itTaggedJet, "DeepJet"));
    vars.FillVars(
        "TaggedJet_DeepJet_b", iTaggedJet,
        CSVHelper::GetJetCSV_DNN(*itTaggedJet, "pfDeepFlavourJetTags:probb"));
    vars.FillVars(
        "TaggedJet_DeepJet_bb", iTaggedJet,
        CSVHelper::GetJetCSV_DNN(*itTaggedJet, "pfDeepFlavourJetTags:probbb"));
    vars.FillVars("TaggedJet_DeepJet_lepb", iTaggedJet,
                  CSVHelper::GetJetCSV_DNN(*itTaggedJet,
                                           "pfDeepFlavourJetTags:problepb"));
    vars.FillVars(
        "TaggedJet_DeepJet_c", iTaggedJet,
        CSVHelper::GetJetCSV_DNN(*itTaggedJet, "pfDeepFlavourJetTags:probc"));
    vars.FillVars(
        "TaggedJet_DeepJet_uds", iTaggedJet,
        CSVHelper::GetJetCSV_DNN(*itTaggedJet, "pfDeepFlavourJetTags:probuds"));
    vars.FillVars(
        "TaggedJet_DeepJet_g", iTaggedJet,
        CSVHelper::GetJetCSV_DNN(*itTaggedJet, "pfDeepFlavourJetTags:probg"));
  }

  // Fill Lepton Variables
  std::vector<math::XYZTLorentzVector> tightLeptonVecs =
      BoostedUtils::GetLepVecs(input.selectedElectrons, input.selectedMuons);
  for (auto itLep = tightLeptonVecs.begin(); itLep != tightLeptonVecs.end();
       ++itLep) {
  ...
  std::vector<math::XYZTLorentzVector> looseLeptonVecs =
      BoostedUtils::GetLepVecs(input.selectedElectronsLoose,
                               input.selectedMuonsLoose);
  for (std::vector<math::XYZTLorentzVector>::iterator itLep =
           looseLeptonVecs.begin();
       itLep != looseLeptonVecs.end(); ++itLep) {
  ...
  for (std::vector<pat::Electron>::const_iterator itEle =
           input.selectedElectronsLoose.begin();
       itEle != input.selectedElectronsLoose.end(); ++itEle) {
    ...
    if (itEle->hasUserFloat("ptBeforeRun2Calibration"))
      vars.FillVars("LooseElectron_Pt_BeforeRun2Calibration", iEle,
                    itEle->userFloat("ptBeforeRun2Calibration"));

    vars.FillVars("LooseElectron_Eta_Supercluster", iEle,
                  itEle->superCluster()->eta());
    if (itEle->hasUserFloat("CorrFactor_ScaleUp")) {
      // electron energy scale combined up/down
      vars.FillVars("LooseElectron_CorrFactor_ScaleUp", iEle,
                    itEle->userFloat("CorrFactor_ScaleUp"));
      vars.FillVars("LooseElectron_CorrFactor_ScaleDown", iEle,
                    itEle->userFloat("CorrFactor_ScaleDown"));
      // electron energy smearing combined up/down
      vars.FillVars("LooseElectron_CorrFactor_SigmaUp", iEle,
                    itEle->userFloat("CorrFactor_SigmaUp"));
      vars.FillVars("LooseElectron_CorrFactor_SigmaDown", iEle,
                    itEle->userFloat("CorrFactor_SigmaDown"));
    }
    if (itEle->hasUserFloat("IdentificationSF")) {
      vars.FillVars("LooseElectron_IdentificationSF", iEle,
                    itEle->userFloat("IdentificationSF"));
      vars.FillVars("LooseElectron_IdentificationSFUp", iEle,
                    itEle->userFloat("IdentificationSFUp"));
      vars.FillVars("LooseElectron_IdentificationSFDown", iEle,
                    itEle->userFloat("IdentificationSFDown"));
      vars.FillVars("LooseElectron_ReconstructionSF", iEle,
                    itEle->userFloat("ReconstructionSF"));
      vars.FillVars("LooseElectron_ReconstructionSFUp", iEle,
                    itEle->userFloat("ReconstructionSFUp"));
      vars.FillVars("LooseElectron_ReconstructionSFDown", iEle,
                    itEle->userFloat("ReconstructionSFDown"));
    }
  }
  for (std::vector<pat::Electron>::const_iterator itEle =
           input.selectedElectrons.begin();
       itEle != input.selectedElectrons.end(); ++itEle) {
    ...
    if (itEle->hasUserFloat("ptBeforeRun2Calibration"))
      vars.FillVars("Electron_Pt_BeforeRun2Calibration", iEle,
                    itEle->userFloat("ptBeforeRun2Calibration"));

    vars.FillVars("Electron_Eta_Supercluster", iEle,
                  itEle->superCluster()->eta());
    if (itEle->hasUserFloat("CorrFactor_ScaleUp")) {
      // electron energy scale combined up/down
      vars.FillVars("Electron_CorrFactor_ScaleUp", iEle,
                    itEle->userFloat("CorrFactor_ScaleUp"));
      vars.FillVars("Electron_CorrFactor_ScaleDown", iEle,
                    itEle->userFloat("CorrFactor_ScaleDown"));
      // electron energy smearing combined up/down
      vars.FillVars("Electron_CorrFactor_SigmaUp", iEle,
                    itEle->userFloat("CorrFactor_SigmaUp"));
      vars.FillVars("Electron_CorrFactor_SigmaDown", iEle,
                    itEle->userFloat("CorrFactor_SigmaDown"));
    }
    if (itEle->hasUserFloat("IdentificationSF")) {
      vars.FillVars("Electron_IdentificationSF", iEle,
                    itEle->userFloat("IdentificationSF"));
      vars.FillVars("Electron_IdentificationSFUp", iEle,
                    itEle->userFloat("IdentificationSFUp"));
      vars.FillVars("Electron_IdentificationSFDown", iEle,
                    itEle->userFloat("IdentificationSFDown"));
      vars.FillVars("Electron_ReconstructionSF", iEle,
                    itEle->userFloat("ReconstructionSF"));
      vars.FillVars("Electron_ReconstructionSFUp", iEle,
                    itEle->userFloat("ReconstructionSFUp"));
      vars.FillVars("Electron_ReconstructionSFDown", iEle,
                    itEle->userFloat("ReconstructionSFDown"));
    }
  }

  for (std::vector<pat::Muon>::const_iterator itMu =
           input.selectedMuonsLoose.begin();
       itMu != input.selectedMuonsLoose.end(); ++itMu) {
    ...
    if (itMu->hasUserFloat("PtbeforeRC"))
      vars.FillVars("LooseMuon_Pt_BeForeRC", iMu,
                    itMu->userFloat("PtbeforeRC"));
    ...
    if (itMu->hasUserFloat("roccorSFDown"))
      vars.FillVars("LooseMuon_roccorSFDown", iMu,
                    itMu->userFloat("roccorSFDown"));

    if (itMu->hasUserFloat("IdentificationSF")) {
      vars.FillVars("LooseMuon_IdentificationSF", iMu,
                    itMu->userFloat("IdentificationSF"));
      vars.FillVars("LooseMuon_IdentificationSFUp", iMu,
                    itMu->userFloat("IdentificationSFUp"));
      vars.FillVars("LooseMuon_IdentificationSFDown", iMu,
                    itMu->userFloat("IdentificationSFDown"));
    }
    if (itMu->hasUserFloat("IsolationSF")) {
      vars.FillVars("LooseMuon_IsolationSF", iMu,
                    itMu->userFloat("IsolationSF"));
      vars.FillVars("LooseMuon_IsolationSFUp", iMu,
                    itMu->userFloat("IsolationSFUp"));
      vars.FillVars("LooseMuon_IsolationSFDown", iMu,
                    itMu->userFloat("IsolationSFDown"));
    }
  }
  for (std::vector<pat::Muon>::const_iterator itMu =
           input.selectedMuons.begin();
       itMu != input.selectedMuons.end(); ++itMu) {
    ...
    if (itMu->hasUserFloat("IdentificationSF")) {
      vars.FillVars("Muon_IdentificationSF", iMu,
                    itMu->userFloat("IdentificationSF"));
      vars.FillVars("Muon_IdentificationSFUp", iMu,
                    itMu->userFloat("IdentificationSFUp"));
      vars.FillVars("Muon_IdentificationSFDown", iMu,
                    itMu->userFloat("IdentificationSFDown"));
    }
    if (itMu->hasUserFloat("IsolationSF")) {
      vars.FillVars("Muon_IsolationSF", iMu, itMu->userFloat("IsolationSF"));
      vars.FillVars("Muon_IsolationSFUp", iMu,
                    itMu->userFloat("IsolationSFUp"));
      vars.FillVars("Muon_IsolationSFDown", iMu,
                    itMu->userFloat("IsolationSFDown"));
    }
  }
  ...
  for (std::vector<double>::iterator itCSV = csvJetsSorted.begin();
       itCSV != csvJetsSorted.end(); ++itCSV) {
