...
METSelection::METSelection(const edm::ParameterSet& iConfig)
    : METSelection(iConfig.getParameter<double>("minMET"),
                   iConfig.getParameter<double>("maxMET")) {}
METSelection::METSelection(float minMET_, float maxMET_)
...
bool METSelection::IsSelected(const InputCollections& input, Cutflow& cutflow) {
  ...
  // set the 4-vector of the hadronic recoil equals to the MET 4-vector and
  // correct it in the following for leptons and photons
  ...
  // check if MET/hadronic recoil is greater than a lower threshold and smaller
  // than a maximum threshold check in addition if a quality criteria for MET is
  // fulfilled to reduce fake MET events if the events satisfies the conditions,
  // return true
  // if((hadr_recoil>minMET&&hadr_recoil<maxMET)&&(fabs(calomet-met)/calomet<0.5)){
  if (met > minMET || hadr_recoil > minMET) {
