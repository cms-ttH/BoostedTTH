...
void
ProcessorTemplate::Init(const InputCollections& input, VariableContainer& vars)
{
  vars.InitVar("HTjet");             // initialize a float variable
  vars.InitVar("N_Jets", "I");       // initialize an integer variable
  vars.InitVars("Jet_Pt", "N_Jets"); // initialize a float-array variable of
                                     // length N_Jets (notice the *s* in
                                     // InitVars)
...
void
ProcessorTemplate::Process(const InputCollections& input,
                           VariableContainer& vars)
{
  ...
  for (uint i = 0; i < input.selectedJets.size(); i++) {
    vars.FillVars("Jet_Pt", i, input.selectedJets[i].pt()); // fill array entry
                                                            // i of Jet_Pt
                                                            // (notice the *s*
                                                            // in FillVars)
