...
void
essentialMVAVarProcessor::Init(const InputCollections& input,
...
void
essentialMVAVarProcessor::Process(const InputCollections& input,
                                  VariableContainer& vars)
{
  if (!initialized)
  ...
  for (std::vector<pat::Jet>::iterator itTaggedJet = selectedTaggedJets.begin();
       itTaggedJet != selectedTaggedJets.end();
       ++itTaggedJet) {
  ...
  for (std::vector<pat::Jet>::iterator itTaggedJet1 =
       ...
       itTaggedJet1 != selectedTaggedJets.end();
       ++itTaggedJet1) {
    ...
    for (std::vector<pat::Jet>::iterator itTaggedJet2 = itTaggedJet1 + 1;
         itTaggedJet2 != selectedTaggedJets.end();
         ++itTaggedJet2) {
  ...
  for (std::vector<pat::Jet>::iterator itUntaggedJet1 =
       ...
       itUntaggedJet1 != selectedUntaggedJets.end();
       ++itUntaggedJet1) {
    ...
    for (std::vector<pat::Jet>::iterator itUntaggedJet2 = itUntaggedJet1 + 1;
         itUntaggedJet2 != selectedUntaggedJets.end();
         ++itUntaggedJet2) {
  ...
  //
  //     for(std::vector<math::XYZTLorentzVector>::iterator itJetVec1 =
  //     jetvecs.begin();itJetVec1 != jetvecs.end() && itJetVec1-jetvecs.begin()
  //     < maxjets;++itJetVec1){
  //       for(std::vector<math::XYZTLorentzVector>::iterator itJetVec2 =
  //       itJetVec1+1;itJetVec2 != jetvecs.end() && itJetVec2-jetvecs.begin() <
  //       maxjets;++itJetVec2){
  //         m2JetsAverage += (*itJetVec1+*itJetVec2).M();
  //         drJetsAverage += BoostedUtils::DeltaR(*itJetVec1,*itJetVec2);
  //         dktJetsAverage += BoostedUtils::DeltaKt(*itJetVec1,*itJetVec2);
  ...
  std::sort(
    jetsByCSV.begin(), jetsByCSV.end(), BoostedUtils::FirstHasHigherCSV);
  ...
  //     int nPairsJets=0;
  //
  //     for(std::vector<pat::Jet>::iterator itJet1 = jetsByCSV.begin();itJet1
  //     != jetsByCSV.end() && itJet1-jetsByCSV.begin() < maxjets;++itJet1){
  //       math::XYZTLorentzVector jetVec1 = itJet1->p4();
  //
  //       for(std::vector<pat::Jet>::iterator itJet2 = itJet1+1;itJet2 !=
  //       jetsByCSV.end() && itJet2-jetsByCSV.begin() < maxjets;++itJet2){
  ...
  for (std::vector<pat::Jet>::const_iterator itJet =
       ...
       itJet != input.selectedJetsLoose.end();
       ++itJet) {
    if (itJet->pt() >= 30)
      continue;
  ...
  //   float aplanarity=-1;
  //   float sphericity=-1;
  //   TLorentzVector primLepVecTL =
  //   BoostedUtils::GetTLorentzVector(primLepVec);
  //   TLorentzVector
  //   metvecTL(input.correctedMET.px(),input.correctedMET.py(),0,input.correctedMET.energy());
  ...
  //   bdtvar.getBestHiggsMass(primLepVecTL,metvecTL,jetvecsTL,csvJets,minChi,dRbb,bjet1,bjet2,
  //   jet_loose_vecsTL,jetCSV_loose);
  //   TLorentzVector dummy_metv;
  //   double minChiStudy, chi2lepW, chi2leptop, chi2hadW, chi2hadtop,
  //   mass_lepW, mass_leptop, mass_hadW, mass_hadtop, dRbbStudy, testquant1,
  //   testquant2, testquant3, testquant4, testquant5, testquant6, testquant7;
  ...
  //   }
  //   bdtvar.study_tops_bb_syst (input.correctedMET.pt(),
  //   input.correctedMET.phi(), dummy_metv, primLepVecTL, jets_vvdouble,
  //   csvJets, minChiStudy, chi2lepW, chi2leptop, chi2hadW, chi2hadtop,
  //   mass_lepW, mass_leptop, mass_hadW, mass_hadtop, dRbbStudy, testquant1,
  //   testquant2, testquant3, testquant4, testquant5, testquant6, testquant7,
  //   b1, b2);
  ...
  if (input.selectedJets.size() > 3)
    eth_blr = mem->GetBTagLikelihoodRatio(
      jettvecs, jetcsvs, out_best_perm, out_P_4b, out_P_2b);
