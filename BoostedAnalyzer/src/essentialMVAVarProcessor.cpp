...
essentialMVAVarProcessor::essentialMVAVarProcessor() { pointerToMVAvars.reset(new MVAvars()); }
...
void essentialMVAVarProcessor::Init(const InputCollections &input, VariableContainer &vars)
{
    ...
    for (auto it = varMap.begin(); it != varMap.end(); it++) vars.InitVar(it->first);
...
void essentialMVAVarProcessor::Process(const InputCollections &input, VariableContainer &vars)
{
    ...
    for (auto it = varMap.begin(); it != varMap.end(); it++) vars.FillVar(it->first, it->second);
}
