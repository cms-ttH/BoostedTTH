...
void GenJetOrderedJetCollectionProcessor::Init(const InputCollections& input,
...
void GenJetOrderedJetCollectionProcessor::Process(const InputCollections& input,
                                                  VariableContainer& vars) {
  if (!initialized)
    throw cms::Exception("BadProcessor")
        << "GenJetOrderedJetCollectionProcessor not initialized";
...
bool GenJetOrderedJetCollectionProcessor::getClosestJet(
    const reco::GenJet& genJet, const pat::JetCollection& jets,
    pat::Jet& closestJet, double& deltaRMin) const {
  double dRMin = 1000;
  int idxMin = -1;
...
double GenJetOrderedJetCollectionProcessor::deltaR(const reco::GenJet& genJet,
