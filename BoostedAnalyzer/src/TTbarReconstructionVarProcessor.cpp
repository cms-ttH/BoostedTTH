...
TTbarReconstructionVarProcessor::TTbarReconstructionVarProcessor()
    : generator(InterpretationGenerator(IntType::tt, 0, 8)) {}
TTbarReconstructionVarProcessor::~TTbarReconstructionVarProcessor() {}

void TTbarReconstructionVarProcessor::Init(const InputCollections& input,
...
void TTbarReconstructionVarProcessor::Process(const InputCollections& input,
                                              VariableContainer& vars) {
  ...
  TVector2 metvec(input.correctedMET.corP4(pat::MET::Type1XY).px(),
  ...
  if (input.genTopEvt.IsFilled() && input.genTopEvt.IsSemiLepton()) {
    ...
    if (bs_true.size() == 2) {
      ...
      mcmatcher.Setup(bhad_true, qs_true[0], qs_true[1], blep_true, lep_true,
                      nu_true, bs_true[0], bs_true[1]);
    }
  }
