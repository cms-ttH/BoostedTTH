...
using namespace std;
FilterInfo::FilterInfo(std::map< std::string, bool > filters_) { filters = filters_; }

bool FilterInfo::Exists(std::string filtername) const
{
    if (filters.find(filtername) == filters.end()) { return false; }
    return true;
}
...
bool FilterInfo::Pass(std::string filtername) const
{
    ...
    if (asterix == int(std::string::npos)) { return Exists(filtername) && filters.at(filtername); }
    else {
...
std::map< std::string, bool > FilterInfo::GetFilters() const { return filters; }

FilterInfoProducer::FilterInfoProducer(const edm::ParameterSet& iConfig, edm::ConsumesCollector&& iC)
{
...
void FilterInfo::Print() const
{
    ...
    for (auto f : filters) { std::cout << f.first << " " << f.second << "\n"; }
}
