...
bool
FilterInfo::Exists(std::string filtername) const
{
...
bool
FilterInfo::Pass(std::string filtername) const
{
  if (filtername == "any" || filtername == "Any" || filtername == "none" ||
      filtername == "None")
    return true;
...
bool
FilterInfo::PassAll(std::vector<std::string> filternames) const
{
...
{
  filterBitsToken = iC.consumes<edm::TriggerResults>(
    iConfig.getParameter<edm::InputTag>("filterBits"));
  for (auto& tag :
       iConfig.getParameter<std::vector<edm::InputTag>>("additionalFilters")) {
...
FilterInfo
FilterInfoProducer::Produce(const edm::Event& iEvent) const
{
