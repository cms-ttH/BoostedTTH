...
bool BoostedUtils::FirstIsHarder(math::XYZTLorentzVector vec1, math::XYZTLorentzVector vec2) { return vec1.Pt() > vec2.Pt(); }

bool BoostedUtils::FirstJetIsHarder(pat::Jet jet1, pat::Jet jet2) { return jet1.pt() > jet2.pt(); }
...
bool BoostedUtils::FirstHasHigherCSV(pat::Jet jet1, pat::Jet jet2) { return CSVHelper::GetJetCSV(jet1, "DeepJet") > CSVHelper::GetJetCSV(jet2, "DeepJet"); }
...
std::vector< math::XYZTLorentzVector > BoostedUtils::GetLepVecs(const std::vector< pat::Electron >& selectedElectrons,
                                                                const std::vector< pat::Muon >      selectedMuons)
{
    ...
    for (std::vector< pat::Muon >::const_iterator itMu = selectedMuons.begin(); itMu != selectedMuons.end(); ++itMu) { leptonVecs.push_back(itMu->p4()); }
...
vector< math::XYZTLorentzVector > BoostedUtils::GetJetVecs(const std::vector< pat::Jet >& jets)
{
    ...
    for (std::vector< pat::Jet >::const_iterator itJet = jets.begin(); itJet != jets.end(); ++itJet) { jetVecs.push_back(itJet->p4()); }
...
bool BoostedUtils::PassesCSV(const pat::Jet& jet, const char workingPoint)
{
    ...
    switch (workingPoint) {
        case 'L':
            if (csvValue > CSVLwp) { return true; }
            break;
        case 'M':
            if (csvValue > CSVMwp) { return true; }
            break;
        case 'T':
            if (csvValue > CSVTwp) { return true; }
            break;
...
std::vector< pat::Jet > BoostedUtils::GetHiggsFilterJets(const std::vector< pat::Jet >& higgsDecayJets, const int& nCSVJets, const int& nPtJets)
{
    ...
    if ((int) subJets.size() > (nCSVJets + 1)) { std::sort(subJets.begin() + nCSVJets, subJets.end(), BoostedUtils::FirstJetIsHarder); }
...
float BoostedUtils::GetHiggsMass(const boosted::BoostedJet& boostedJet, const int& nJets, const int& nCSVJets, const int& nPtJets)
{
    ...
    for (std::vector< math::XYZTLorentzVector >::const_iterator itFiltJet = filterJetVecs.begin() + 1;
         itFiltJet != filterJetVecs.end() && itFiltJet - filterJetVecs.begin() < nJets; ++itFiltJet) {
        sumVec += *itFiltJet;
    }
...
std::vector< pat::Jet > BoostedUtils::GetSingleTopJets(const std::vector< pat::Jet >& centralJets, const std::vector< pat::Jet >& forwardJets, float etacut)
{
    ...
    for (auto cj = centralJets.begin(); cj != centralJets.end(); cj++) {
        if (fabs(cj->eta()) < etacut) { singleTopJets.push_back(*cj); }
    }
    for (auto fj = forwardJets.begin(); fj != forwardJets.end(); fj++) {
        if (fabs(fj->eta()) > etacut) { singleTopJets.push_back(*fj); }
    }
...
std::vector< pat::MET > BoostedUtils::GetCorrectedMET(const std::vector< pat::Jet >& cleanIdJetsForMET, const std::vector< pat::Jet >& correctedJets,
