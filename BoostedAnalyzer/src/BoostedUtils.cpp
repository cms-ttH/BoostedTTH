...
TLorentzVector BoostedUtils::GetTLorentzVector(
...
vector<TLorentzVector> BoostedUtils::GetTLorentzVectors(
...
math::XYZTLorentzVector BoostedUtils::GetXYZTLorentzVector(
...
bool BoostedUtils::FirstIsHarder(math::XYZTLorentzVector vec1,
...
bool BoostedUtils::FirstBoostedJetIsHarder(boosted::BoostedJet jet1,
...
bool BoostedUtils::FirstHasHigherCSV(pat::Jet jet1, pat::Jet jet2) {
  return CSVHelper::GetJetCSV(jet1, "DeepJet") >
         CSVHelper::GetJetCSV(jet2, "DeepJet");
}

float BoostedUtils::DeltaEta(const math::XYZTLorentzVector& vec1,
...
float BoostedUtils::DeltaPhi(const math::XYZTLorentzVector& vec1,
                             const math::XYZTLorentzVector& vec2) {
  if (vec1.Pt() < 0.001 || vec2.Pt() < 0.001) return -2;
...
float BoostedUtils::DeltaR(const math::XYZTLorentzVector& vec1,
                           const math::XYZTLorentzVector& vec2) {
...
float BoostedUtils::DeltaKt(const math::XYZTLorentzVector& vec1,
                            const math::XYZTLorentzVector& vec2) {
  float dr = BoostedUtils::DeltaR(vec1, vec2);
...
float BoostedUtils::CosThetaStar(const math::XYZTLorentzVector& vec1,
                                 const math::XYZTLorentzVector& vec2) {
  if (vec1.Pt() < 0.001 || vec2.Pt() < 0.001) return -2;
...
float BoostedUtils::CosThetaCM(const math::XYZTLorentzVector& vec,
...
std::vector<math::XYZTLorentzVector> BoostedUtils::GetLepVecs(
...
math::XYZTLorentzVector BoostedUtils::GetPrimLepVec(
    const std::vector<pat::Electron>& selectedElectrons,
    const std::vector<pat::Muon> selectedMuons) {
  std::vector<math::XYZTLorentzVector> leptonVecs =
...
void BoostedUtils::GetNuVecs(const math::XYZTLorentzVector& lepvec,
  ...
  double a = (mu * lepvec.Pz()) /
             (lepvec.E() * lepvec.E() - lepvec.Pz() * lepvec.Pz());
  ...
  nu1.SetPxPyPzE(metvec.Px(), metvec.Py(), pz1,
                 sqrt(metvec.Mod2() + pz1 * pz1));
  nu2.SetPxPyPzE(metvec.Px(), metvec.Py(), pz2,
                 sqrt(metvec.Mod2() + pz2 * pz2));
}

vector<math::XYZTLorentzVector> BoostedUtils::GetJetVecs(
...
boosted::BoostedJetCollection BoostedUtils::GetSortedByPt(
    boosted::BoostedJetCollection const& boostedJets) {
...
float BoostedUtils::GetClosestLepJetID(int& idJet,
...
float BoostedUtils::GetJetAverageJetEtaMax(const std::vector<pat::Jet>& jets1,
                                           const std::vector<pat::Jet>& jets2) {
  int count = 0;
  float avgval = 0.;
  for (std::vector<pat::Jet>::const_iterator itJet1 = jets1.begin();
       itJet1 != jets1.end(); ++itJet1) {
    for (std::vector<pat::Jet>::const_iterator itJet2 = itJet1 + 1;
         itJet2 != jets1.end(); ++itJet2) {
...
boosted::JetType BoostedUtils::GetBoostedJetType(
    const boosted::BoostedJet boostedJet, const BoostedJetDisc::Mode mode) {
  switch (mode) {
...
std::vector<pat::Jet> BoostedUtils::GetHiggsFilterJets(
    const boosted::BoostedJet& boostedJet, const int& nCSVJets,
    const int& nPtJets) {
...
std::vector<pat::Jet> BoostedUtils::GetHiggsFilterJets(
    const std::vector<pat::Jet>& higgsDecayJets, const int& nCSVJets,
    const int& nPtJets) {
  ...
  if ((int)subJets.size() > (nCSVJets + 1)) {
    std::sort(subJets.begin() + nCSVJets, subJets.end(),
...
float BoostedUtils::GetHiggsMass(const boosted::BoostedJet& boostedJet,
                                 const int& nJets, const int& nCSVJets,
                                 const int& nPtJets) {
...
std::vector<pat::Jet> BoostedUtils::GetSingleTopJets(
    const std::vector<pat::Jet>& centralJets,
    const std::vector<pat::Jet>& forwardJets, float etacut) {
...
std::vector<pat::MET> BoostedUtils::GetCorrectedMET(
    const std::vector<pat::Jet>& cleanIdJetsForMET,
  ...
  for (std::vector<pat::MET>::const_iterator oldMET = correctedMETs.begin();
       oldMET != correctedMETs.end(); ++oldMET) {
    pat::MET outMET = *oldMET;

    if (i == 0) {
      // get old MET p4
      TLorentzVector oldMETVec;
      oldMETVec.SetPxPyPzE(oldMET->p4().Px(), oldMET->p4().Py(),
                           oldMET->p4().Pz(), oldMET->p4().E());
      ...
      for (std::vector<pat::Jet>::const_iterator itJet =
           ...
           itJet != cleanIdJetsForMET.end(); ++itJet) {
        TLorentzVector cleanJETVec;
        cleanJETVec.SetPtEtaPhiE(itJet->pt(), itJet->eta(), itJet->phi(),
                                 itJet->energy());
        TLorentzVector PTcleanJETVec;
        PTcleanJETVec.SetPxPyPzE(cleanJETVec.Px(), cleanJETVec.Py(), 0.0,
                                 cleanJETVec.Et());
      ...
      for (std::vector<pat::Jet>::const_iterator itJet = correctedJets.begin();
           itJet != correctedJets.end(); ++itJet) {
        TLorentzVector correctedJETVec;
        correctedJETVec.SetPtEtaPhiE(itJet->pt(), itJet->eta(), itJet->phi(),
                                     itJet->energy());
        TLorentzVector PTcorrectedJETVec;
        PTcorrectedJETVec.SetPxPyPzE(correctedJETVec.Px(), correctedJETVec.Py(),
                                     0.0, correctedJETVec.Et());
