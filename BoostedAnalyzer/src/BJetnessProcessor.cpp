...
{
  bjetness_ =
    iC.consumes<vector<float>>(edm::InputTag("BJetness", "BJetnessValue"));
}
...
void
BJetnessProcessor::Init(const InputCollections& input, VariableContainer& vars)
{
...
void
BJetnessProcessor::Process(const InputCollections& input,
                           VariableContainer& vars)
{
  ...
  /*
    (*bjetness)[0]; is the pt of the leading jet after correction. This value is
    saved as a reference value in the comparison of events.
    (*bjetness)[1]; is the number of lepton of the bjetness jet.
    (*bjetness)[2]; is the ratio between non primary vertex and primary vertex
    tracks.
