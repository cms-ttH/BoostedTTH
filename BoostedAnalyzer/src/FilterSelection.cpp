...
void
FilterSelection::InitCutflow(Cutflow& cutflow)
{
...
bool
FilterSelection::IsSelected(const InputCollections& input, Cutflow& cutflow)
{
  if (!initialized)
    cerr << "FilterSelection not initialized" << endl;
  for (auto const filter : filters) {
    if (!input.filterInfo.Pass(filter))
      return false;
    else
      cutflow.EventSurvivedStep(string("Filter ") + filter,
