...
{
    for (auto it = input.weights.begin(); it != input.weights.end(); ++it) { vars.InitVar(it->first); }
...
void WeightProcessor::Process(const InputCollections& input, VariableContainer& vars)
{
    ...
    for (auto it = input.weights.begin(); it != input.weights.end(); ++it) { vars.FillVar(it->first, it->second); }
}
