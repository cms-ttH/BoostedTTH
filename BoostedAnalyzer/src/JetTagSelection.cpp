...
JetTagSelection::JetTagSelection(std::vector<int> minjets_,
    ...
    : JetTagSelection(minjets_, std::vector<int>(), mintags_,
                      std::vector<int>()) {}
...
bool JetTagSelection::IsSelected(const InputCollections& input,
                                 Cutflow& cutflow) {
  ...
  for (size_t i = 0; i < input.selectedJets.size(); i++) {
    if (CSVHelper::PassesCSV(input.selectedJets.at(i), "DeepJet",
                             CSVHelper::CSVwp::Medium, input.era)) {
      ntags++;
    }
  }
