...
JetTagSelection::JetTagSelection(std::vector< int > minjets_, std::vector< int > mintags_) :
...
JetTagSelection::JetTagSelection(std::vector< int > minjets_, std::vector< int > maxjets_, std::vector< int > mintags_, std::vector< int > maxtags_) :
    minJets(minjets_),
    maxJets(maxjets_),
    minTags(mintags_),
    maxTags(maxtags_)
...
JetTagSelection::JetTagSelection(const edm::ParameterSet& iConfig) :
    JetTagSelection(iConfig.getParameter< std::vector< int > >("minJets"), iConfig.getParameter< std::vector< int > >("maxJets"),
...
bool JetTagSelection::IsSelected(const InputCollections& input, Cutflow& cutflow)
{
    ...
    for (size_t i = 0; i < input.selectedJets.size(); i++) {
        if (CSVHelper::PassesCSV(input.selectedJets.at(i), "DeepJet", CSVHelper::CSVwp::Medium, input.era)) { ntags++; }
    }
