...
LeptonSelection::LeptonSelection(std::vector<std::string> electronTriggers_,
...
LeptonSelection::LeptonSelection(std::string electronTrigger,
                                 std::string muonTrigger, std::string channel_,
                                 int step_)
    : LeptonSelection(std::vector<std::string>(1, (electronTrigger)),
                      std::vector<std::string>(1, muonTrigger), channel_,
                      step_) {}

LeptonSelection::LeptonSelection(const edm::ParameterSet& iConfig, int step_)
    : LeptonSelection(
          iConfig.getParameter<edm::ParameterSet>("LeptonSelection")
              .getParameter<std::vector<std::string> >("electronTriggers"),
          iConfig.getParameter<edm::ParameterSet>("LeptonSelection")
              .getParameter<std::vector<std::string> >("muonTriggers"),
          iConfig.getParameter<edm::ParameterSet>("LeptonSelection")
              .getParameter<std::string>("channel"),
          step_) {}
...
bool LeptonSelection::IsSelected(const InputCollections& input,
                                 Cutflow& cutflow) {
  ...
  if (channel == "both") {
    ...
    if (step < 0 || step == 3) {
      if (!((muonTriggered && nmuonsloose == 1 && nmuons == 1) ||
            (electronTriggered && nelectronsloose == 1 && nelectrons == 1)))
        return false;
      else
