...
bool VariableContainer::DoesVarExist(const TString& name) const {
  return (intMap.find(name) != intMap.end() ||
          floatMap.find(name) != floatMap.end() ||
          arrayMap.find(name) != arrayMap.end());
}
...
void VariableContainer::InitVar(const TString& name, float defaultValue,
...
void VariableContainer::FillIntVar(const TString& name, long value,
                                   bool checkIfExists) {
...
void VariableContainer::FillFloatVar(const TString& name, float value,
                                     bool checkIfExists) {
...
void VariableContainer::InitVars(const TString& name, float defaultValue,
...
void VariableContainer::InitVars(const TString& name,
                                 const TString& nEntryVariable,
                                 int maxentries) {
...
void VariableContainer::ConnectTree(TTree* tree) {
  ...
  auto itI = intMap.begin();
  while (itI != intMap.end()) {
    tree->Branch(itI->first, &(itI->second),
                 itI->first + "/" + intMapType[itI->first]);
    itI++;
  }
  auto itA = arrayMap.begin();
  while (itA != arrayMap.end()) {
    tree->Branch(itA->first, itA->second,
