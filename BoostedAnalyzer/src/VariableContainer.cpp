...
VariableContainer::~VariableContainer()
{
    for (std::map< TString, Float_t* >::iterator it = arrayMap.begin(); it != arrayMap.end(); it++) { delete[] it->second; }
}
...
{
    return floatMapFilled.find(name) != floatMapFilled.end() || intMapFilled.find(name) != intMapFilled.end() ||
...
void VariableContainer::FillVar(const TString& name, double value)
{
    ...
    if (isFloat) { FillFloatVar(name, value, false); }
    else if (isInt) {
...
void VariableContainer::FillIntVar(const TString& name, long value, bool checkIfExists)
{
    ...
    if (intMapFilled[name]) { cerr << name << " already filled, replacing value!" << endl; }
    intMap[name]       = value;
...
void VariableContainer::FillFloatVar(const TString& name, float value, bool checkIfExists)
{
    ...
    if (floatMapFilled[name]) { cerr << name << " already filled, replacing value!" << endl; }
    floatMap[name]       = value;
...
void VariableContainer::InitVars(const TString& name, const TString& nEntryVariable, int maxentries) { InitVars(name, -9., nEntryVariable, maxentries); }

void VariableContainer::FillVars(const TString& name, int index, float value)
{
    if (arrayMap.find(name) == arrayMap.end()) { cerr << name << " does not exist!" << endl; }
...
void VariableContainer::SetDefaultValues()
{
    ...
    while (itA != arrayMap.end()) {
        for (int i = 0; i < itAmaxEntriesArrays->second; ++i) itA->second[i] = itAdefault->second;
...
void VariableContainer::PrintArrayValue(const TString& name)
{
    ...
    for (int i = 0; i < nEntries; ++i) { cout << name + "[" << i << "] : " << printArray[i] << std::endl; }
}
