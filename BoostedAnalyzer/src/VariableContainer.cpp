...
VariableContainer::~VariableContainer()
{
  for (std::map<TString, Float_t*>::iterator it = arrayMap.begin();
       it != arrayMap.end();
       it++) {
    delete[] it->second;
  }
...
bool
VariableContainer::DoesVarExist(const TString& name) const
{
  return (intMap.count(name) > 0 || floatMap.count(name) > 0 ||
          arrayMap.count(name) > 0);
}

bool
VariableContainer::IsVarFilled(const TString& name) const
{
  return floatMapFilled.find(name) != floatMapFilled.end() ||
         intMapFilled.find(name) != intMapFilled.end() ||
         arrayMapFilled.find(name) != arrayMapFilled.end();
}

void
VariableContainer::InitVar(const TString& name,
                           float defaultValue,
...
void
VariableContainer::InitVar(const TString& name, const std::string& type)
{
  checkIfVariableAlreadyInit(name);
  InitVar(name, -9., type);
}
...
void
VariableContainer::FillVar(const TString& name, double value)
{
...
void
VariableContainer::FillIntVar(const TString& name,
                              long value,
                              bool checkIfExists)
{
...
void
VariableContainer::FillFloatVar(const TString& name,
                                float value,
                                bool checkIfExists)
{
...
void
VariableContainer::InitVars(const TString& name,
                            float defaultValue,
                            ...
                            int maxentries)
{
  ...
  if (!DoesVarExist(nEntryVariable)) {
    cerr << nEntryVariable << " not filled, needed for " << name
         << ", will not initialize" << endl;
    return;
  }
...
void
VariableContainer::FillVars(const TString& name, int index, float value)
{
...
void
VariableContainer::SetDefaultValues()
{
...
void
VariableContainer::ConnectTree(TTree* tree)
{
  auto itF = floatMap.begin();
  while (itF != floatMap.end()) {
  ...
  auto itA = arrayMap.begin();
  while (itA != arrayMap.end()) {
    tree->Branch(itA->first,
                 itA->second,
                 itA->first + "[" + entryVariableOf[itA->first] + "]/F");
    itA++;
  }
}

void
VariableContainer::PrintArrayValue(const TString& name)
{
...
void
VariableContainer::Dump() const
{
  auto itF = floatMap.begin();
  auto itFdefault = floatMapDefaults.begin();
  cout << "floats: " << endl;
  while (itF != floatMap.end()) {
    cout << itF->first << " : " << itF->second << " : " << itFdefault->second
         << endl;
    ++itF;
    ++itFdefault;
  }
  ...
  cout << "ints: " << endl;
  while (itI != intMap.end()) {
    cout << itI->first << " : " << itI->second << " : " << itIdefault->second
         << endl;
    ++itI;
    ++itIdefault;
  }
...
float*
VariableContainer::GetFloatVarPointer(const TString& name)
{
...
float*
VariableContainer::GetArrayVarPointer(const TString& name, int entry)
{
  if (arrayMap.count(name) == 0) {
    cerr << name << " does not exist!" << endl;
    return 0;
...
long*
VariableContainer::GetIntVarPointer(const TString& name)
{
...
float
VariableContainer::GetFloatVar(const TString& name)
{
...
float
VariableContainer::GetArrayVar(const TString& name, int entry)
{
...
long
VariableContainer::GetIntVar(const TString& name)
{
...
void
VariableContainer::checkIfVariableAlreadyInit(const TString& name) const
{
  if (intMap.count(name) > 0 || floatMap.count(name) > 0 ||
      arrayMap.count(name) > 0) {
    throw cms::Exception("BadProcessor")
      << "Variable '" << name << "' already initialized!";
  }
}
