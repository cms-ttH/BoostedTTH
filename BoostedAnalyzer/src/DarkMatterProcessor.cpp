...
void DarkMatterProcessor::Init(const InputCollections& input,
...
void DarkMatterProcessor::Process(const InputCollections& input,
                                  VariableContainer& vars) {
  ...
  if (input.correctedMET.genMET() != 0) {
    if (input.correctedMET.genMET()->pt() <
        1) {  // fix for broken GenMET in MadGraphMonoJetSamples
      if (input.genDarkMatterEvt.IsFilled()) {
        ...
        gen_hadr_recoil_p4 =
            BoostedUtils::GetXYZTLorentzVector(DM_Evt.ReturnNaiveMET4Vector());
      }
    } else {
  ...
  if (input.systematic == Systematics::JESup) {
    ...
    met_p4 = input.correctedMET.shiftedP4(pat::MET::UnclusteredEnUp,
                                          pat::MET::Type1XY);
  } else if (input.systematic == Systematics::METUnclENdown) {
    met_p4 = input.correctedMET.shiftedP4(pat::MET::UnclusteredEnDown,
                                          pat::MET::Type1XY);
  } else {
  ...
  vars.FillVar("Evt_Pt_MET_UnclEnUp",
               input.correctedMET.shiftedPt(pat::MET::UnclusteredEnUp,
                                            pat::MET::Type1XY));
  vars.FillVar("Evt_Pt_MET_UnclEnDown",
               input.correctedMET.shiftedPt(pat::MET::UnclusteredEnDown,
                                            pat::MET::Type1XY));
  vars.FillVar("CaloMET", input.correctedMET.caloMETPt());
  vars.FillVar("CaloMET_PFMET_ratio",
               fabs(met_p4.pt() - input.correctedMET.caloMETPt()) /
                   input.correctedMET.caloMETPt());

  for (size_t i = 0; i < input.selectedJets.size(); i++) {
    vars.FillVars("DeltaPhi_Jet_MET", i,
                  fabs(TVector2::Phi_mpi_pi(met_p4.phi() -
                                            input.selectedJets.at(i).phi())));
  }
  ...
  vars.FillVar("CaloMET_Hadr_Recoil_ratio",
               fabs(hadr_recoil_p4.pt() - input.correctedMET.caloMETPt()) /
                   input.correctedMET.caloMETPt());

  for (size_t i = 0; i < input.selectedJets.size(); i++) {
    vars.FillVars("DeltaPhi_Jet_Hadr_Recoil", i,
                  fabs(TVector2::Phi_mpi_pi(hadr_recoil_p4.phi() -
                                            input.selectedJets.at(i).phi())));
  }

  // naive calculation for hadronic recoil in analogy to MET calculation,
  // however not all jets included in loose collection. Should probably use
  // slimmedJETs collection
  ...
  if (input.selectedMuonsLoose.size() == 2) {
    auto Zmumu =
        input.selectedMuonsLoose[0].p4() + input.selectedMuonsLoose[1].p4();
    ...
    vars.FillVar("Zmumu_Pt_Hadr_Recoil_Pt_ratio",
                 fabs(Zmumu.Pt() - hadr_recoil_p4.pt()) / hadr_recoil_p4.pt());
  }
  ...
  if (input.genDarkMatterEvt.IsFilled()) {
    ...
    std::vector<TLorentzVector> Neutralinos_p4 =
        DM_Evt.ReturnNeutralino4Vectors();
    ...
    for (std::vector<TLorentzVector>::const_iterator itNeutralino =
             Neutralinos_p4.begin();
         itNeutralino != Neutralinos_p4.end(); ++itNeutralino) {
    ...
    if (Neutralinos_p4.size() >= 2) {
      ...
      vars.FillVar(
          "cos_theta_N1_N2",
          TMath::Cos(
              (Neutralinos_p4[0].Vect()).Angle(Neutralinos_p4[1].Vect())));
      vars.FillVar(
          "cos_theta_Med_N1",
          TMath::Cos((Neutralinos_p4[0].Vect()).Angle(Mediator_p4.Vect())));
      vars.FillVar(
          "cos_theta_Med_N2",
          TMath::Cos((Neutralinos_p4[1].Vect()).Angle(Mediator_p4.Vect())));

      vars.FillVar("delta_phi_N1_N2",
                   Neutralinos_p4[0].DeltaPhi(Neutralinos_p4[1]));
      ...
      vars.FillVar("delta_R_N1_N2",
                   Neutralinos_p4[0].DeltaR(Neutralinos_p4[1]));
    ...
    for (std::vector<TLorentzVector>::const_iterator itNeutrino =
             Neutrinos_p4.begin();
         itNeutrino != Neutrinos_p4.end(); ++itNeutrino) {
