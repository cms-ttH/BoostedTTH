...
DNNVarProcessor::DNNVarProcessor(DNNClassifier_SL* sldnnclassifier_)
...
void
DNNVarProcessor::Init(const InputCollections& input, VariableContainer& vars)
{
...
void
DNNVarProcessor::Process(const InputCollections& input, VariableContainer& vars)
{
  if (!initialized)
  ...
  dnn_output = sldnnclassifier->evaluate(
    jetvecs, jetcsvs, lepvecs[0], metP4, add_features);
