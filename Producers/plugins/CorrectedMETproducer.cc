...
void CorrectedMETproducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    using namespace edm;
    ...
    std::unique_ptr< pat::METCollection > correctedMETs =
        std::make_unique< pat::METCollection >(CorrectMET(*h_oldJets, *h_newJets, *h_oldElectrons, *h_newElectrons, *h_oldMuons, *h_newMuons, *h_inputMETs));
    iEvent.put(std::move(correctedMETs), collectionName);
}

std::vector< pat::MET > CorrectedMETproducer::CorrectMET(const std::vector< pat::Jet >& oldJetsForMET, const std::vector< pat::Jet >& newJetsForMET,
                                                         const std::vector< pat::Electron >& oldElectronsForMET,
                                                         const std::vector< pat::Electron >& newElectronsForMET, const std::vector< pat::Muon >& oldMuonsForMET,
                                                         const std::vector< pat::Muon >& newMuonsForMET, const std::vector< pat::MET >& pfMETs) const
{
...
// module  ------------
void CorrectedMETproducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions)
{
