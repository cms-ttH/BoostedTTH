...
SelectedPhotonProducer::SelectedPhotonProducer(const edm::ParameterSet& iConfig)
    : era{iConfig.getParameter<std::string>("era")},
      ...
      EDMPhotonsToken{consumes<pat::PhotonCollection>(
...

{
  if (era.find("2016") == std::string::npos and
    ...
    std::cerr << "\n\nERROR: Unknown era" << era
              << " in SelectedPhotonProducer " << std::endl;
  ...
  for (size_t i = 0; i < ptMins_.size(); i++) {
    if (ids_.at(i) == "loose")
      ...
      std::cerr << "\n\nERROR: No matching photon ID type found for: "
                << ids_.at(i) << std::endl;
      throw std::exception();
    }
  }
  ...
  PhoID_SF_Loose =
      (TH2F*)TFile::Open(
          TString(std::string(getenv("CMSSW_BASE")) + "/src/" +
                  iConfig.getParameter<std::string>("file_PhoLooseIDSF")))
          ->Get("EGamma_SF2D");
  PhoID_SF_Loose->SetDirectory(0);
  PhoID_SF_Medium =
      (TH2F*)TFile::Open(
          TString(std::string(getenv("CMSSW_BASE")) + "/src/" +
                  iConfig.getParameter<std::string>("file_PhoMediumIDSF")))
          ->Get("EGamma_SF2D");
  PhoID_SF_Medium->SetDirectory(0);
  PhoID_SF_Tight =
      (TH2F*)TFile::Open(
          TString(std::string(getenv("CMSSW_BASE")) + "/src/" +
                  iConfig.getParameter<std::string>("file_PhoTightIDSF")))
          ->Get("EGamma_SF2D");
...
void SelectedPhotonProducer::produce(edm::Event& iEvent,
  ...
  if (not inputPhotons.isValid()) {
    std::cerr << "\n\nERROR: retrieved photon collection is not valid"
              << std::endl;
    throw std::exception();
  }
  ...
  for (size_t i = 0; i < ptMins_.size(); i++) {
    ...
    std::vector<pat::Photon> selectedPhotons = GetSortedByPt(GetSelectedPhotons(
        updatedPhotons, ptMins_.at(i), etaMaxs_.at(i), photonIDs_.at(i)));
    ...
    std::unique_ptr<pat::PhotonCollection> selectedPhotons_ptr =
        std::make_unique<pat::PhotonCollection>(selectedPhotons);
...
// module  ------------
void SelectedPhotonProducer::fillDescriptions(
...
std::vector<pat::Photon> SelectedPhotonProducer::GetSelectedPhotons(
    const std::vector<pat::Photon>& inputPhotons, const float iMinPt,
    const float iMaxEta, const PhotonID photonID) {
  std::vector<pat::Photon> selectedPhotons;

  for (std::vector<pat::Photon>::const_iterator it = inputPhotons.begin(),
                                                ed = inputPhotons.end();
       it != ed; ++it) {
    if (isGoodPhoton(*it, iMinPt, iMaxEta, photonID))
...
bool SelectedPhotonProducer::isGoodPhoton(const pat::Photon& iPhoton,
                                          const float iMinPt,
                                          const float iMaxEta,
                                          const PhotonID iPhotonID) {
  bool passesKinematics =
      (iMinPt <= iPhoton.pt()) and (iMaxEta >= fabs(iPhoton.eta()));
  bool inCrack = false;
  bool passesID = false;
...
void SelectedPhotonProducer::AddPhotonSFs(
    std::vector<pat::Photon>& inputPhotons, const PhotonID iPhotonID) const {
...
std::vector<float> SelectedPhotonProducer::GetPhotonIDSF(
    const pat::Photon& iPhoton, const PhotonID iPhotonID) const {
  // get pt and eta of the electron
  auto pt = iPhoton.hasUserFloat("ptBeforeRun2Calibration")
                ? iPhoton.userFloat("ptBeforeRun2Calibration")
                : iPhoton.pt();
  auto eta = iPhoton.superCluster().isAvailable()
                 ? iPhoton.superCluster()->position().eta()
                 : iPhoton.eta();
  ...
  if (SF_hist == nullptr) {
    std::cerr << "\n\nERROR: Photon ID Scale Factor File could not be loaded"
              << std::endl;
    throw std::exception();
  }
