...

{
    ...
    if (leptonType_ == LeptonType::Electron) {
        ...
        EleID_SF_Medium = (TH2F*) TFile::Open(TString(std::string(getenv("CMSSW_BASE")) + "/src/" + iConfig.getParameter< std::string >("file_EleMediumIDSF")))
                              ->Get("EGamma_SF2D");
        EleID_SF_Medium->SetDirectory(0);
        EleID_SF_Tight = (TH2F*) TFile::Open(TString(std::string(getenv("CMSSW_BASE")) + "/src/" + iConfig.getParameter< std::string >("file_EleTightIDSF")))
                             ->Get("EGamma_SF2D");
        EleID_SF_Tight->SetDirectory(0);
        EleReco_SF_highPt =
            (TH2F*) TFile::Open(TString(std::string(getenv("CMSSW_BASE")) + "/src/" + iConfig.getParameter< std::string >("file_EleRecoSF_highPt")))
                ->Get("EGamma_SF2D");
        EleReco_SF_highPt->SetDirectory(0);
        EleReco_SF_lowPt =
            (TH2F*) TFile::Open(TString(std::string(getenv("CMSSW_BASE")) + "/src/" + iConfig.getParameter< std::string >("file_EleRecoSF_lowPt")))
                ->Get("EGamma_SF2D");
...
void SelectedLeptonProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    if (leptonType_ == LeptonType::Electron) {
        ...
        for (size_t i = 0; i < ptMins_.size(); i++) {
            ...
            std::vector< pat::Electron > selectedElectrons =
                GetSortedByPt(GetSelectedElectrons(updatedElectrons, ptMins_.at(i), electronIDs_.at(i), etaMaxs_.at(i)));
        ...
        // correction)
        if (useMuonRC) { ApplyMuonMomentumCorrection(correctedMuons); }
...
std::vector< pat::Electron > SelectedLeptonProducer::GetSelectedElectrons(const std::vector< pat::Electron >& inputElectrons, const double iMinPt,
                                                                          const ElectronID iElectronID, const double iMaxEta) const
{
    std::vector< pat::Electron > selectedElectrons;

    for (const auto& ele : inputElectrons) {
        if (isGoodElectron(ele, iMinPt, iMaxEta, iElectronID)) { selectedElectrons.push_back(ele); }
    }

    return selectedElectrons;
}
// function to check whether an electron fulfills several requirements
bool SelectedLeptonProducer::isGoodElectron(const pat::Electron& iElectron, const double iMinPt, const double iMaxEta, const ElectronID iElectronID) const
{
    ...
    switch (iElectronID) {
        case ElectronID::None: passesID = true; break;
        case ElectronID::Veto: passesID = iElectron.electronID("cutBasedElectronID-Fall17-94X-V2-veto"); break;
        case ElectronID::Loose: passesID = iElectron.electronID("cutBasedElectronID-Fall17-94X-V2-loose"); break;
        case ElectronID::Medium: passesID = iElectron.electronID("cutBasedElectronID-Fall17-94X-V2-medium"); break;
        case ElectronID::Tight: passesID = iElectron.electronID("cutBasedElectronID-Fall17-94X-V2-tight"); break;
        default: std::cerr << "\n\nERROR: InvalidElectronID" << std::endl; throw std::exception();
    }
...
double SelectedLeptonProducer::GetEletronRelIsolation(const pat::Electron& inputElectron, const IsoCorrType icorrType, const IsoConeSize iconeSize) const
{
    ...
    if (iconeSize == IsoConeSize::R03) {
        if (icorrType == IsoCorrType::deltaBeta)
            ...
            double eta = 0.;
            if (inputElectron.superCluster().isAvailable()) { eta = fabs(inputElectron.superCluster()->position().eta()); }
            else {
...
{
    for (auto& ele : inputElectrons) { ele.addUserFloat("relIso", GetEletronRelIsolation(ele, icorrType, iconeSize)); }
}
...
std::vector< pat::Muon > SelectedLeptonProducer::GetSelectedMuons(const std::vector< pat::Muon >& inputMuons, const double iMinPt, const MuonID iMuonID,
                                                                  const IsoConeSize iconeSize, const IsoCorrType icorrType, const double iMaxEta,
                                                                  const MuonIsolation imuonIso) const
{
    std::vector< pat::Muon > selectedMuons;

    for (const auto& mu : inputMuons) {
        if (isGoodMuon(mu, iMinPt, iMaxEta, iMuonID, iconeSize, icorrType, imuonIso)) { selectedMuons.push_back(mu); }
    }

    return selectedMuons;
}
// function to check whether a muon fulfills several requirements
bool SelectedLeptonProducer::isGoodMuon(const pat::Muon& iMuon, const double iMinPt, const double iMaxEta, const MuonID iMuonID, const IsoConeSize iconeSize,
    ...
    for (size_t i = 0; i < inputMuons.size(); i++) {
        ...
        if (isData) {
            ...
            if (inputMuons.at(i).genLepton()) {
                momentum_sf      = rc.kSpreadMC(inputMuons.at(i).charge(), inputMuons.at(i).pt(), inputMuons.at(i).eta(), inputMuons.at(i).phi(),
                                           inputMuons.at(i).genLepton()->pt(), 0, 0);
                momentum_sfDelta = rc.kSpreadMCerror(inputMuons.at(i).charge(), inputMuons.at(i).pt(), inputMuons.at(i).eta(), inputMuons.at(i).phi(),
                                                     inputMuons.at(i).genLepton()->pt());
            }
            else {
                momentum_sf      = rc.kSmearMC(inputMuons.at(i).charge(), inputMuons.at(i).pt(), inputMuons.at(i).eta(), inputMuons.at(i).phi(),
                                          trackerLayersWithMeasurement, r1, 0, 0);
                momentum_sfDelta = rc.kSmearMCerror(inputMuons.at(i).charge(), inputMuons.at(i).pt(), inputMuons.at(i).eta(), inputMuons.at(i).phi(),
...
{
    for (auto& mu : inputMuons) { mu.addUserFloat("relIso", GetMuonRelIsolation(mu, icorrType, iconeSize)); }
}
...
std::vector< float > SelectedLeptonProducer::GetMuonIDSF(const pat::Muon& iMuon, const MuonID& iMuonID) const
{
    ...
    // load the correct scale factor histogram
    switch (iMuonID) {
        case MuonID::None: break;
        case MuonID::Loose: SF_hist = MuonID_SF_Loose; break;
        case MuonID::Medium: SF_hist = MuonID_SF_Medium; break;
        case MuonID::Tight: SF_hist = MuonID_SF_Tight; break;
        default: std::cerr << "\n\nERROR: InvalidMuonID" << std::endl; throw std::exception();
    }
...
std::vector< float > SelectedLeptonProducer::GetMuonISOSF(const pat::Muon& iMuon, const MuonID& iMuonID, const MuonIsolation& iMuonIso) const
{
    ...
    // load the correct scale factor histogram
    switch (iMuonIso) {
        case MuonIsolation::None: break;
        case MuonIsolation::Loose:
            switch (iMuonID) {
                case MuonID::None: break;
                case MuonID::Loose: SF_hist = MuonISO_SF_LooseRelIso_LooseID; break;
                case MuonID::Medium: SF_hist = MuonISO_SF_LooseRelIso_MediumID; break;
                case MuonID::Tight: SF_hist = MuonISO_SF_LooseRelIso_TightID; break;
                default: std::cerr << "\n\nERROR: InvalidMuonID in GetMuonISOSF" << std::endl; throw std::exception();
            }
            break;
        case MuonIsolation::Medium: break;
        case MuonIsolation::Tight:
            switch (iMuonID) {
                case MuonID::None: break;
                case MuonID::Loose: break;
                case MuonID::Medium: SF_hist = MuonISO_SF_TightRelIso_MediumID; break;
                case MuonID::Tight: SF_hist = MuonISO_SF_TightRelIso_TightID; break;
                default: std::cerr << "\n\nERROR: InvalidMuonID in GetMuonISOSF" << std::endl; throw std::exception();
            }
            break;
        default: std::cerr << "\n\nERROR: InvalidMuonIsolation" << std::endl; throw std::exception();
    }
...
bool SelectedLeptonProducer::isGoodTau(const pat::Tau& iTau, const double iMinPt, const double iMaxEta, const TauID iTauID) const
{
    ...
    bool passesDM         = useDeepTau ? iTau.tauID("decayModeFindingNewDMs") >= 0.5 : iTau.tauID("decayModeFinding") >= 0.5;
    bool passesID         = false;
    ...
    if (useDeepTau) {
        ...
        switch (iTauID) {
            case TauID::None: passesID = true; break;
            case TauID::Veto:
                passesID = iTau.tauID("byVLooseIsolationMVArun2017v2DBoldDMwLT2017") >= 0.5;// and iTau.tauID("againstElectronVLooseMVA6") >= 0.5 and iTau.tauID("againstMuonLoose3") >= 0.5;
                break;
            case TauID::Loose:
                passesID = iTau.tauID("byLooseIsolationMVArun2017v2DBoldDMwLT2017") >= 0.5;// and iTau.tauID("againstElectronLooseMVA6") >= 0.5 and iTau.tauID("againstMuonLoose3") >= 0.5;
                break;
            case TauID::Medium:
                passesID = iTau.tauID("byMediumIsolationMVArun2017v2DBoldDMwLT2017") >= 0.5;// and iTau.tauID("againstElectronMediumMVA6") >= 0.5 and iTau.tauID("againstMuonLoose3") >= 0.5;
                break;
            case TauID::Tight:
                passesID = iTau.tauID("byTightIsolationMVArun2017v2DBoldDMwLT2017") >= 0.5;// and iTau.tauID("againstElectronTightMVA6") >= 0.5 and iTau.tauID("againstMuonTight3") >= 0.5;
                break;
            default: std::cerr << "\n\nERROR: InvalidTauID" << std::endl; throw std::exception();
        }
    }
...
// module  ------------
void SelectedLeptonProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions)
{
