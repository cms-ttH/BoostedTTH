...
SelectedJetProducer::SelectedJetProducer(const edm::ParameterSet& iConfig)
{
  ...
  const std::vector<std::string> s_systematics =
  ...
  for (uint i = 0; i < s_systematics.size(); i++) {
    try {
      ...
      throw cms::Exception("InvalidUncertaintyName")
        << "SelectedJetProducer: systematic name " << s_systematics[i]
  ...
  for (uint i = 0; i < collectionNames.size(); i++) {
    for (uint j = 0; j < systematics.size(); j++) {
      produces<pat::JetCollection>(
        systName(collectionNames[i], systematics[j]));
    }
  }
}
...
SelectedJetProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  const std::vector<pat::Jet> idJets = helper.GetSelectedJets(
    *h_inputJets, 0., 9999., MiniAODHelper::getjetID(JetID), '-');
  ...
  if (applyCorrection) {
    ...
    std::unique_ptr<pat::JetCollection> rawJets_ =
      std::make_unique<pat::JetCollection>(rawJets);
    iEvent.put(std::move(rawJets_), "rawJets");
    // Clean muons and electrons from jets
    std::vector<pat::Jet> cleanJets = helper.GetDeltaRCleanedJets(
      rawJets, *h_inputMuons, *h_inputElectrons, leptonJetDr);
    ...
    for (uint i = 0; i < systematics.size(); i++) {
      unsortedJets.push_back(helper.GetCorrectedJets(
        cleanJets, iEvent, iSetup, h_genJets, systematics[i], true, doJER));
    }
  ...
  // if no correction is to be applied, still remove jets close to a lepton
    ...
    for (uint i = 0; i < systematics.size(); i++) {
      unsortedJets.push_back(helper.GetDeltaRCleanedJets(
        idJets, *h_inputMuons, *h_inputElectrons, leptonJetDr));
    }
  }
  // loop over all jetcollections and each systematic and apply pt,eta as well
  // as pujetid cut on them
  for (uint i = 0; i < ptMins.size(); i++) {
    for (uint j = 0; j < systematics.size(); j++) {
      // Get jet Collection which pass selections
      std::vector<pat::Jet> selectedJets_unsorted =
        helper.GetSelectedJets(unsortedJets[j],
                               ptMins[i],
                               etaMaxs[i],
                               jetID::none,
                               '-',
                               PUJetID::get(PUJetIDMins[i]));
      // sort the selected jets with respect to pt
      std::unique_ptr<pat::JetCollection> selectedJets =
        std::make_unique<pat::JetCollection>(
          helper.GetSortedByPt(selectedJets_unsorted));
...
std::string
SelectedJetProducer::systName(std::string name, Systematics::Type sysType)
{
...
// module  ------------
void
SelectedJetProducer::fillDescriptions(
