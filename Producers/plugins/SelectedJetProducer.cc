...
SelectedJetProducer::SelectedJetProducer(const edm::ParameterSet &iConfig)
    : jetType{iConfig.getParameter<std::string>("JetType")},
      ...
      PUJetIDMins{
          iConfig.getParameter<std::vector<std::string>>("PUJetIDMins")},
      JetID_{iConfig.getParameter<std::vector<std::string>>("JetID")},
      systematics_config{
          iConfig.getParameter<std::vector<std::string>>("systematics")},
      // inputs
      jetsToken{consumes<pat::JetCollection>(
          iConfig.getParameter<edm::InputTag>("jets"))},
      genjetsToken{consumes<reco::GenJetCollection>(
          iConfig.getParameter<edm::InputTag>("miniAODGenJets"))},
      muonsToken{consumes<pat::MuonCollection>(
          iConfig.getParameter<edm::InputTag>("muons"))},
      electronsToken{consumes<pat::ElectronCollection>(
  ...
  for (size_t i = 0; i < systematics_config.size(); i++) {
    try {
      ...
      throw cms::Exception("InvalidUncertaintyName")
          << "SelectedJetProducer: systematic name " << systematics_config.at(i)
  ...
  for (size_t i = 0; i < collectionNames.size(); i++) {
    for (size_t j = 0; j < systematics.size(); j++) {
      produces<pat::JetCollection>(
          systName(collectionNames.at(i), systematics.at(j)));
    }
  }
  ...
  if (JetType_ == JetType::AK4PFCHS) {
    ...
    if (era.find("2016") != std::string::npos) {
      jecUncertaintyTxtFileName = std::string(getenv("CMSSW_BASE")) +
                                  "/src/BoostedTTH/Producers/data/jec/" +
                                  jecFileAK4_2016;
  ...
  if (jecUncertaintyTxtFileName != "") {
    if (!fileExists(jecUncertaintyTxtFileName)) {  // check if JEC uncertainty
                                                   // file exists
      throw cms::Exception("InvalidJECUncertaintyFile")
          << "No JEC uncertainty file '" << jecUncertaintyTxtFileName
          << "' found";
    }
  } else {
    throw cms::Exception("NoJECUncertaintyFile")
        << "No JEC uncertainty file specified";
  }
  ...
  for (size_t i = 0; i < PUJetIDMins.size(); i++) {
    ...
    if (JetID_.at(i) == "none")
      ...
      std::cerr << "\n\nERROR: No matching JetID found for: " << JetID_.at(i)
                << std::endl;
      throw std::exception();
    }
    ...
    if (PUJetIDMins.at(i) == "none")
      ...
      std::cerr << "\n\nERROR: No matching PUJetID_WP found for: "
                << PUJetIDMins.at(i) << std::endl;
      throw std::exception();
    }
  }
...
T SelectedJetProducer::GetSortedByPt(const T &collection) const {
  ...
  std::sort(result.begin(), result.end(),
            [](typename T::value_type a, typename T::value_type b) {
              return ptr(a)->pt() > ptr(b)->pt();
            });
  return result;
}

void SelectedJetProducer::UpdateJetCorrectorUncertainties(
    const edm::EventSetup &iSetup) {
  for (auto &jecUncIt : jecUncertainties_) {
    jecUncIt.second.reset(CreateJetCorrectorUncertainty(
        iSetup, jetTypeLabelForJECUncertainty, jecUncIt.first));
  }
}

void SelectedJetProducer::AddJetCorrectorUncertainty(
    const edm::EventSetup &iSetup, const std::string &uncertaintyLabel) {
  jecUncertainties_[uncertaintyLabel] =
      std::unique_ptr<JetCorrectionUncertainty>(CreateJetCorrectorUncertainty(
          iSetup, jetTypeLabelForJECUncertainty, uncertaintyLabel));
}

// function to generate JetCorrectionUncertainty
JetCorrectionUncertainty *SelectedJetProducer::CreateJetCorrectorUncertainty(
    const edm::EventSetup &iSetup, const std::string &jetTypeLabel,
    const std::string &uncertaintyLabel) const {
  try {
    JetCorrectorParameters jetCorPar;
    if (jecUncertaintyTxtFileName != "") {
      if (uncertaintyLabel ==
          "Uncertainty") {  // this is the key in the database but not in txt...
        jetCorPar = JetCorrectorParameters(jecUncertaintyTxtFileName, "Total");
      } else {
        jetCorPar =
            JetCorrectorParameters(jecUncertaintyTxtFileName, uncertaintyLabel);
      }
    } else {
    ...
    throw cms::Exception("InvalidJECUncertaintyLabel")
        << "No JEC uncertainty with label '" << uncertaintyLabel
        << "' found in event setup";
  }
  return 0;
}

// function to return Jets, which fullfill all IDs
std::vector<pat::Jet> SelectedJetProducer::GetSelectedJets(
    const std::vector<pat::Jet> &inputJets, const float iMinPt,
    const float iMaxAbsEta, const JetID iJetID, const PUJetIDWP wp) const {
...
std::string SelectedJetProducer::systName(
    std::string name, SystematicsHelper::Type sysType) const {
...
bool SelectedJetProducer::isGoodJet(const pat::Jet &iJet, const float iMinPt,
                                    const float iMaxAbsEta, const JetID iJetID,
                                    const PUJetIDWP wp) const {
  ...
  switch (iJetID) {
    case JetID::Loose:
      passesID = iJet.neutralHadronEnergyFraction() < 0.99 &&
                 iJet.neutralEmEnergyFraction() < 0.99;
      if (not(JetType_ == JetType::AK8PFPUPPI ||
              JetType_ == JetType::AK15PFPUPPI))
    ...
    case JetID::Tight:
      ...
      if (fabs(iJet.eta()) < 2.4)
        passesID = passesID && (iJet.chargedHadronEnergyFraction() > 0.0 &&
                                iJet.chargedMultiplicity() > 0);
      if (era.find("2016") != std::string::npos)
        passesID = passesID && iJet.chargedEmEnergyFraction() < 0.99;
      break;
    case JetID::None:
      passesID = true;
      break;
...
std::vector<pat::Jet> SelectedJetProducer::GetUncorrectedJets(
    const std::vector<pat::Jet> &inputJets) const {
...
std::vector<pat::Jet> SelectedJetProducer::GetDeltaRCleanedJets(
    const std::vector<pat::Jet> &inputJets,
    const std::vector<pat::Muon> &inputMuons,
...
std::vector<pat::Jet> SelectedJetProducer::GetCorrectedJets(
    const std::vector<pat::Jet> &inputJets, const edm::Event &event,
    const edm::EventSetup &setup,
    const edm::Handle<reco::GenJetCollection> &genjets,
    const SystematicsHelper::Type iSysType, const bool &doJES,
  ...
  for (const auto &jet : inputJets) {
    outputJets.push_back(GetCorrectedJet(jet, event, setup, genjets, iSysType,
                                         doJES, doJER, corrFactor, uncFactor));
  }
  return outputJets;
}

// function to return one corrected Jet
pat::Jet SelectedJetProducer::GetCorrectedJet(
    const pat::Jet &inputJet, const edm::Event &event,
    const edm::EventSetup &setup,
    const edm::Handle<reco::GenJetCollection> &genjets,
  ...
  ApplyJetEnergyCorrection(outputJet, factor, event, setup, genjets, iSysType,
                           doJES, doJER, addUserFloats, corrFactor, uncFactor);
...
// SmearedJetProducer)
void SelectedJetProducer::ApplyJetEnergyCorrection(
    pat::Jet &jet, double &totalCorrFactor, const edm::Event &event,
    const edm::EventSetup &setup,
    const edm::Handle<reco::GenJetCollection> &genjets,
    const SystematicsHelper::Type iSysType, const bool doJES, const bool doJER,
    const bool addUserFloats, const float corrFactor, const float uncFactor) {
  totalCorrFactor = 1.;
  if (doJES || doJER) {  // check again if JES or JER is demanded
    ...
    if (doJES) {
      ...
      if (corrector) {
        ...
        edm::LogError("SelectedJetProducer")
            << "Trying to use GetCorrectedJets() without setting jet "
               "corrector!";
      }
      ...
      if (addUserFloats) {
        ...
        const double uncUp =
            GetJECUncertainty(jet, setup, SystematicsHelper::JESup);
        const double uncDown =
            GetJECUncertainty(jet, setup, SystematicsHelper::JESdown);
      ...
      if (SystematicsHelper::isJECUncertainty(iSysType)) {
        ...
        if (addUserFloats) {
          jet.addUserFloat("Helper" + SystematicsHelper::toString(iSysType),
                           jecvar);
        }
...
double SelectedJetProducer::GetJECUncertainty(
    const pat::Jet &jet, const edm::EventSetup &iSetup,
    const SystematicsHelper::Type iSysType) {
  const std::string uncertaintyLabel =
      SystematicsHelper::GetJECUncertaintyLabel(iSysType);
...
void SelectedJetProducer::produce(edm::Event &iEvent,
                                  const edm::EventSetup &iSetup) {
  using namespace edm;
  ...
  if (not inputJets.isValid()) {
    std::cerr << "\n\nERROR: retrieved jet collection is not valid"
              << std::endl;
    throw std::exception();
  }
  ...
  if (!isData) {
    ...
    if (not genJets.isValid()) {
      std::cerr << "\n\nERROR: retrieved genjet collection is not valid"
                << std::endl;
      throw std::exception();
    }
  }
  ...
  // Get raw jets
  std::vector<pat::Jet> rawJets_general =
      GetSortedByPt(GetUncorrectedJets(*inputJets));
  std::unique_ptr<pat::JetCollection> rawJets_general_ =
      std::make_unique<pat::JetCollection>(rawJets_general);
  ...
  for (size_t i = 0; i < ptMins.size(); i++) {
    ...
    // )
    std::vector<pat::Jet> idJets =
        GetSelectedJets(*inputJets, 0., 9999., Jet_ID.at(i));

    for (auto &jet : idJets) {
      if (!jet.hasUserFloat("neutralHadronEnergyFraction") &&
          !jet.hasUserFloat("chargedHadronEnergyFraction")) {
        jet.addUserFloat("neutralHadronEnergyFraction",
                         jet.neutralHadronEnergyFraction());
        jet.addUserFloat("chargedHadronEnergyFraction",
    ...
    for (size_t j = 0; j < systematics.size(); j++) {
      ...
      if (applyCorrection) {
        std::vector<pat::Jet> rawJets =
            GetSortedByPt(GetUncorrectedJets(idJets));

        // Clean muons and electrons from jets
        std::vector<pat::Jet> cleanJets = GetDeltaRCleanedJets(
            rawJets, *inputMuons, *inputElectrons, leptonJetDr);
        ...
        unsortedJets = GetCorrectedJets(cleanJets, iEvent, iSetup, genJets,
                                        systematics.at(j), doJES, doJER);

      }
      // if no correction is to be applied, still remove jets close to a lepton
      else {
        unsortedJets = GetDeltaRCleanedJets(idJets, *inputMuons,
                                            *inputElectrons, leptonJetDr);
      }
      ...
      std::vector<pat::Jet> selectedJets_unsorted =
          GetSelectedJets(unsortedJets, ptMins.at(i), etaMaxs.at(i),
                          JetID::None, PUJetID_WP.at(i));
      // sort the selected jets with respect to pt
      std::unique_ptr<pat::JetCollection> selectedJets =
          std::make_unique<pat::JetCollection>(
              GetSortedByPt(selectedJets_unsorted));
...
// module  ------------
void SelectedJetProducer::fillDescriptions(
