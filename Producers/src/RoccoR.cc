...
int RocRes::etaBin(double eta) const
{
    ...
    for (int i = 0; i < NETA - 1; ++i)
        if (abseta < resol[i + 1].eta) return i;
    return NETA - 1;
}
...
int RocRes::trkBin(double x, int h, TYPE T) const
{
    for (int i = 0; i < NTRK - 1; ++i)
        if (x < resol[h].nTrk[T][i + 1]) return i;
    return NTRK - 1;
}
...
double RoccoR::kGenSmear(double pt, double eta, double v, double u, RocRes::TYPE TT, int s, int m) const { return RC[s][m].RR.kSmear(pt, eta, TT, v, u); }
