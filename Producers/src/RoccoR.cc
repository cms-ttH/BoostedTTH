...
#include "BoostedTTH/Producers/interface/RoccoR.h"
#include "TMath.h"
#include "TSystem.h"
#include <fstream>
#include <iostream>
...
int
RocRes::getBin(double x, const int NN, const double* b) const
{
  for (int i = 0; i < NN; ++i)
    if (x < b[i + 1])
      return i;
...
int
RocRes::getEtaBin(double feta) const
{
  return getBin(feta, NETA, BETA);
}

int
RocRes::getNBinDT(double v, int H) const
{
  return getBin(v, NTRK, dtrk[H]);
}

int
RocRes::getNBinMC(double v, int H) const
{
...
RocRes::dumpParams()
{
  ...
  cout << NTRK << endl;
  cout << NETA << endl;
  for (int H = 0; H < NETA + 1; ++H)
    cout << BETA[H] << " ";
  cout << endl;
  for (int H = 0; H < NETA; ++H) {
    for (int F = 0; F < NTRK; ++F) {
      cout << Form(
        "%8.4f %8.4f %8.4f | ", width[H][F], alpha[H][F], power[H][F]);
    }
    cout << endl;
  }
  for (int H = 0; H < NETA; ++H) {
...
void
RocRes::init(std::string filename)
{
...
double
RocRes::Sigma(double pt, int H, int F) const
{
  double dpt = pt - 45;
...
double
RocRes::getUrnd(int H, int F, double w) const
{
...
double
RocRes::kSmear(double pt, double eta, TYPE type, double w, double u, int n)
  const
{
  int H = getBin(fabs(eta), NETA, BETA);
  int F = n > NMIN ? n - NMIN : 0;
  if (type == Data)
    F = getNBinDT(getUrnd(H, F, w), H);
  double K = type == Data ? kDat[H] : kRes[H];
  double x = K * Sigma(pt, H, F) * cb[H][F].invcdf(u);
  return 1.0 / (1.0 + x);
}
...
double
RocRes::kExtra(double pt, double eta, int n, double u, double w) const
{
  int H = getBin(fabs(eta), NETA, BETA);
  int F = n > NMIN ? n - NMIN : 0;
  ...
  if (fabs(r) > 5)
    return 1.0; // protection against too large smearing
...
int
RocOne::getBin(double x, const int NN, const double* b) const
{
  for (int i = 0; i < NN; ++i)
    if (x < b[i + 1])
      return i;
...
bool
RocOne::checkSYS(int iSYS, int iMEM, int kSYS, int kMEM)
{
  if (iSYS == 0 && iMEM == 0)
    return true;
  if (iSYS == kSYS && iMEM == kMEM)
    return true;
  return false;
}

bool
RocOne::checkTIGHT(int iTYPE, int iSYS, int iMEM, int kTYPE, int kSYS, int kMEM)
{
  if (iTYPE != kTYPE)
    return false;
  if (iSYS != kSYS)
...
void
RocOne::init(std::string filename, int iTYPE, int iSYS, int iMEM)
{
  ...
  while (std::getline(in, s)) {
    ...
    if (s.substr(0, 4) == "CPHI") {
      ...
      ss >> tag >> type >> sys >> mem >> isdt >> var >> bin;
      if (!checkTIGHT(type, sys, mem, iTYPE, iSYS, iMEM))
        continue;
      initialized = true;
      if (var == 0)
...
double
RocOne::kScaleDT(int Q, double pt, double eta, double phi) const
{
...
double
RocOne::kScaleMC(int Q, double pt, double eta, double phi, double kSMR) const
{
  int H = getBin(eta, NETA, BETA);
  int F = getBin(phi, NPHI, MPHI, DPHI);
  double m = M[MC][H][F];
...
double
RocOne::kGenSmear(double pt, double eta, double v, double u, RocRes::TYPE TT)
  const
{
  ...
  while (std::getline(in, s)) {
    ...
    for (int m = 0; m < sn; ++m) {
      ...
      if (gSystem->AccessPathName(inputfile.c_str())) {
        std::cout << Form("Missing %8d %3d, using default instead...", si, m)
                  << std::endl;
        v.push_back(
          RocOne(Form("%s/%d.%d.txt", dirname.c_str(), 0, 0), 0, 0, 0));
      } else {
...
double
RoccoR::kScaleDT(int Q, double pt, double eta, double phi, int s, int m) const
{
  return RC[s][m].kScaleDT(Q, pt, eta, phi);
}

double
RoccoR::kScaleAndSmearMC(int Q,
                         double pt,
                         double eta,
                         double phi,
                         int n,
