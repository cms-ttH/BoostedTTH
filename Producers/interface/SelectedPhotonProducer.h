...
class SelectedPhotonProducer : public edm::EDProducer {
 ...
 private:
  ...
  std::vector<pat::Photon> GetSelectedPhotons(
      const std::vector<pat::Photon>& inputPhotons, const float iMinPt,
      const float iMaxEta, const PhotonID photonID);
  bool isGoodPhoton(const pat::Photon& iPhoton, const float iMinPt,
                    const float iMaxEta, const PhotonID iPhotonID);
  void AddPhotonSFs(std::vector<pat::Photon>& inputPhotons,
                    const PhotonID iPhotonID) const;
  std::vector<float> GetPhotonIDSF(const pat::Photon& iPhoton,
                                   const PhotonID iPhotonID) const;
  // ----------member data ---------------------------
