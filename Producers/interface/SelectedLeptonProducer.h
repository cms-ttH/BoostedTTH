...
class SelectedLeptonProducer : public edm::EDProducer {
 public:
  ...
  std::vector<pat::Electron> GetSelectedElectrons(
      const std::vector<pat::Electron>& inputElectrons,
      const double iMinPt = 10., const ElectronID = ElectronID::Loose,
      const double iMaxEta = 2.4) const;
  bool isGoodElectron(const pat::Electron& iElectron, const double iMinPt = 10.,
                      const double iMaxEta = 2.4,
                      const ElectronID iElectronID = ElectronID::Loose) const;

  // Function to calculate electron relative isolation manually, for sync
  // exercises
  double GetEletronRelIsolation(
      const pat::Electron& inputElectron,
      const IsoCorrType icorrType = IsoCorrType::rhoEA,
      const IsoConeSize iconeSize = IsoConeSize::R03) const;
  void AddElectronRelIsolation(std::vector<pat::Electron>& inputElectrons,
                               const IsoCorrType icorrType = IsoCorrType::rhoEA,
                               const IsoConeSize iconeSize = IsoConeSize::R03);

  // Functions to add object scale factors to electrons meaning reconstruction
  // and identification scale factors
  void AddElectronSFs(std::vector<pat::Electron>& inputElectrons,
                      const ElectronID iElectronID) const;
  std::vector<float> GetElectronIDSF(const pat::Electron& iElectron,
                                     const ElectronID iElectronID) const;
  ...
  std::vector<pat::Muon> GetSelectedMuons(
      const std::vector<pat::Muon>& inputMuons, const double iMinPt = 10.,
      const MuonID = MuonID::Loose, const IsoConeSize = IsoConeSize::R04,
      const IsoCorrType = IsoCorrType::deltaBeta, const double iMaxEta = 2.4,
      const MuonIsolation = MuonIsolation::Loose) const;
  bool isGoodMuon(const pat::Muon&, const double iMinPt = 10.,
                  const double iMaxEta = 2.4, const MuonID = MuonID::Loose,
                  const IsoConeSize = IsoConeSize::R04,
                  const IsoCorrType = IsoCorrType::deltaBeta,
                  const MuonIsolation = MuonIsolation::Loose) const;

  // Functions to add object scale factors to muons meaning identification and
  // relative isolation scale factors
  void AddMuonSFs(std::vector<pat::Muon>& inputMuons, const MuonID& iMuonID,
                  const MuonIsolation& iMuonIso) const;
  std::vector<float> GetMuonIDSF(const pat::Muon& iMuon,
                                 const MuonID& iMuonID) const;
  std::vector<float> GetMuonISOSF(const pat::Muon& iMuon, const MuonID& iMuonID,
                                  const MuonIsolation& iMuonIso) const;

  // Function to calculate muon relative isolation manually, for sync exercises
  double GetMuonRelIsolation(
      const pat::Muon& inputMuon,
      const IsoCorrType icorrType = IsoCorrType::deltaBeta,
      const IsoConeSize iconeSize = IsoConeSize::R04) const;
  void AddMuonRelIsolation(std::vector<pat::Muon>& inputMuons,
                           const IsoCorrType icorrType = IsoCorrType::deltaBeta,
                           const IsoConeSize iconeSize = IsoConeSize::R04);
 ...
 private:
  ...
  const bool deterministicSeeds;  // flag to enable or disable deterministic
                                  // seeds for RC
  ...
  // ptrs to 2D histograms which contain the muon scale factors for
  // reconstruction and identification
