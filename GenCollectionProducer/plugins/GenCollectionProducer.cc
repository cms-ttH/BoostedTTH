...
class GenCollectionProducer : public edm::stream::EDProducer<> {
   ...
   private:
    ...
    std::vector< reco::GenParticle > ApplyPtEtaCuts(const std::vector< reco::GenParticle >& GenParticles_, const std::string& type_, const double& pt_min_,
...
void GenCollectionProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    using namespace edm;
...
std::vector< reco::GenJet > GenCollectionProducer::ApplyPtEtaCuts(const std::vector< reco::GenJet >& GenJets_, const double& pt_min_, const double& eta_max_)
{
    ...
    for (size_t i = 0; i < GenJets_.size(); i++) {
        if (GenJets_.at(i).pt() >= pt_min_ && GenJets_.at(i).eta() <= eta_max_) { mod_GenJets.push_back(GenJets_.at(i)); }
    }
    return mod_GenJets;
}
...
std::vector< reco::GenParticle > GenCollectionProducer::ApplyPtEtaCuts(const std::vector< reco::GenParticle >& GenParticles_, const std::string& type_,
                                                                       const double& pt_min_, const double& eta_max_)
{
    ...
    for (size_t i = 0; i < GenParticles_.size(); i++) {
        if (abs(GenParticles_.at(i).pdgId()) == pdgids[type_]) {
            ...
            if (type_ != "Tau") {
                if (GenParticles_.at(i).isPromptFinalState()) { mod_GenParticles.push_back(GenParticles_.at(i)); }
            }
            // prompt taus (is this correct?)
            else {
                if (GenParticles_.at(i).isPromptDecayed()) { mod_GenParticles.push_back(GenParticles_.at(i)); }
            }
        }
    }
    ...
    for (size_t i = 0; i < mod_GenParticles.size(); i++) {
        if (mod_GenParticles.at(i).pt() >= pt_min_ && mod_GenParticles.at(i).eta() <= eta_max_) { mod_GenParticles_pt_eta.push_back(mod_GenParticles.at(i)); }
    }
    //
...
std::vector< reco::GenJet > GenCollectionProducer::DeltaRCleaning(const std::vector< reco::GenJet >& GenJets_, const std::vector< reco::GenParticle >& Leptons,
                                                                  double R)
{
    std::vector< reco::GenJet > cleaned_jets;
    for (size_t i = 0; i < GenJets_.size(); i++) {
        bool isolated = true;
        for (size_t j = 0; j < Leptons.size(); j++) { isolated = isolated && reco::deltaR(GenJets_.at(i), Leptons.at(j)) < R; }
        if (isolated) { cleaned_jets.push_back(GenJets_.at(i)); }
    }
    return cleaned_jets;
}
...
// module  ------------
void GenCollectionProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions)
{
