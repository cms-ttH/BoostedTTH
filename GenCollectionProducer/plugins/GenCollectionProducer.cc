...
//
/**\class GenCollectionProducer GenCollectionProducer.cc
 BoostedTTH/GenCollectionProducer/plugins/GenCollectionProducer.cc
...
class GenCollectionProducer : public edm::stream::EDProducer<> {
 ...
 private:
  ...
  std::vector<reco::GenJet> ApplyPtEtaCuts(
      const std::vector<reco::GenJet>& GenJets_, const double& pt_min_,
      const double& eta_max_);
  std::vector<reco::GenParticle> ApplyPtEtaCuts(
      const std::vector<reco::GenParticle>& GenParticles_,
      const std::string& type_, const double& pt_min_, const double& eta_max_);
  std::vector<reco::GenJet> DeltaRCleaning(
      const std::vector<reco::GenJet>& GenJets_,
      const std::vector<reco::GenParticle>& Leptons, double R);
...
GenCollectionProducer::GenCollectionProducer(const edm::ParameterSet& iConfig) {
  ...
  */
  ...
  GenJetsAK8Token = consumes<std::vector<reco::GenJet>>(
      iConfig.getParameter<edm::InputTag>("genJetsAK8"));
  GenParticlesToken = consumes<std::vector<reco::GenParticle>>(
      iConfig.getParameter<edm::InputTag>("genParticles"));

  // read config
  collection_name =
      iConfig.getParameter<std::vector<std::string>>("collection_name");
  collection_type =
      iConfig.getParameter<std::vector<std::string>>("collection_type");
  ...
  for (size_t i = 0; i < collection_name.size(); i++) {
    if (collection_type.at(i) == "AK4Jet" ||
        collection_type.at(i) == "AK8Jet") {
      produces<std::vector<reco::GenJet>>(collection_name.at(i));
    } else if (collection_type.at(i) == "Electron" ||
               collection_type.at(i) == "Muon" ||
               collection_type.at(i) == "Tau" ||
               collection_type.at(i) == "Photon") {
      produces<std::vector<reco::GenParticle>>(collection_name.at(i));
    } else {
      throw cms::Exception("GenCollectionProducer")
          << "Wrong collection type " << collection_type.at(i) << "\n";
    }
  }
}
...
void GenCollectionProducer::produce(edm::Event& iEvent,
  ...
  */
  ...
  std::vector<reco::GenParticle> electrons =
      ApplyPtEtaCuts(*GenParticles, "Electron", 10., 2.4);
  std::vector<reco::GenParticle> muons =
      ApplyPtEtaCuts(*GenParticles, "Muon", 10., 2.4);
  std::vector<reco::GenParticle> leptons;
  leptons.insert(leptons.end(), electrons.begin(), electrons.end());
  leptons.insert(leptons.end(), muons.begin(), muons.end());

  // get basic collections of the gen particles/jets with pt and eta cuts
  // applied
  for (size_t i = 0; i < collection_name.size(); i++) {
    if (collection_type.at(i) == "AK4Jet") {
      std::vector<reco::GenJet> collection =
          ApplyPtEtaCuts(*GenJets, pt_min.at(i), eta_max.at(i));
      if (doDeltaRCleaning) {
        std::vector<reco::GenJet> collection_cleaned =
            DeltaRCleaning(collection, leptons, 0.4);
        collection = collection_cleaned;
      }
      std::unique_ptr<std::vector<reco::GenJet>> pOut(
          new std::vector<reco::GenJet>(collection));
      std::sort(pOut->begin(), pOut->end(),
      ...
      std::vector<reco::GenJet> collection =
          ApplyPtEtaCuts(*GenJetsAK8, pt_min.at(i), eta_max.at(i));
      if (doDeltaRCleaning) {
        std::vector<reco::GenJet> collection_cleaned =
            DeltaRCleaning(collection, leptons, 0.8);
        collection = collection_cleaned;
      }
      std::unique_ptr<std::vector<reco::GenJet>> pOut(
          new std::vector<reco::GenJet>(collection));
      ...
      std::vector<reco::GenParticle> collection = ApplyPtEtaCuts(
          *GenParticles, collection_type.at(i), pt_min.at(i), eta_max.at(i));
      std::unique_ptr<std::vector<reco::GenParticle>> pOut(
          new std::vector<reco::GenParticle>(collection));
      std::sort(pOut->begin(), pOut->end(),
                [](const auto& a, const auto& b) { return a.pt() > b.pt(); });
      iEvent.put(std::move(pOut), collection_name.at(i));
    }
...
std::vector<reco::GenJet> GenCollectionProducer::ApplyPtEtaCuts(
    const std::vector<reco::GenJet>& GenJets_, const double& pt_min_,
    const double& eta_max_) {
...
// get collection of specified genparticles with pt and eta cuts and in addition
// calculate dressed leptons
std::vector<reco::GenParticle> GenCollectionProducer::ApplyPtEtaCuts(
    const std::vector<reco::GenParticle>& GenParticles_,
    const std::string& type_, const double& pt_min_, const double& eta_max_) {
  ...
  if (type_ == "Electron" || type_ == "Muon") {
    for (size_t i = 0; i < GenParticles_.size(); i++) {
      for (size_t j = 0; j < mod_GenParticles.size(); j++) {
        if (abs(GenParticles_.at(i).pdgId()) == pdgids["Photon"] &&
            GenParticles_.at(i).status() == 1 &&
            !GenParticles_.at(i).statusFlags().isPrompt() &&
            reco::deltaR(GenParticles_.at(i), mod_GenParticles.at(j)) < 0.1) {
          mod_GenParticles.at(j).setP4(mod_GenParticles.at(j).p4() +
                                       GenParticles_.at(i).p4());
        }
      }
    }
  }
  ...
  for (size_t i = 0; i < mod_GenParticles.size(); i++) {
    if (mod_GenParticles.at(i).pt() >= pt_min_ &&
        mod_GenParticles.at(i).eta() <= eta_max_) {
...
std::vector<reco::GenJet> GenCollectionProducer::DeltaRCleaning(
    const std::vector<reco::GenJet>& GenJets_,
    const std::vector<reco::GenParticle>& Leptons, double R) {
...
// or events  ------------
void GenCollectionProducer::beginStream(edm::StreamID) {}

// ------------ method called once each stream after processing all runs, lumis
// and events  ------------
void GenCollectionProducer::endStream() {}
...
/*
void
GenCollectionProducer::beginLuminosityBlock(edm::LuminosityBlock const&,
...
/*
void
GenCollectionProducer::endLuminosityBlock(edm::LuminosityBlock const&,
edm::EventSetup const&)
{
}
*/
...
// module  ------------
void GenCollectionProducer::fillDescriptions(
